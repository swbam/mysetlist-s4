...
async function verifyShowPage() {
  ...
  console.log(
    `   Image: ${show.headliner_artist.image_url ? "✓ Has image" : "✗ No image"}`,
  );
      ...
      if (setlist.setlist_songs && setlist.setlist_songs.length > 0) {
        ...
        setlist.setlist_songs
          ...
          .forEach((item) => {
            ...
            if (song) {
              console.log(
                `        ${item.position}. ${song.title} by ${song.artist}`,
              );
            }
          });
        if (setlist.setlist_songs.length > 5) {
          console.log(
            `        ... and ${setlist.setlist_songs.length - 5} more songs`,
          );
        }
      }
    });
  ...
  if (show.setlists?.length > 0) {
    const totalSongs = show.setlists.reduce(
      (sum, s) => sum + (s.setlist_songs?.length || 0),
      0,
    );
    console.log(
