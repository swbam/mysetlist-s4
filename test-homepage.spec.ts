...
test.describe('MySetlist Homepage Tests', () => {
  ...
  test('Homepage loads without black screen', async ({ page }) => {
    ...
    console.log('✅ Homepage loads successfully without black screen');
  });
  ...
  test('Search functionality works', async ({ page }) => {
    ...
    const searchResults = page.locator('[data-testid="search-results"], .search-results, [role="listbox"]');
    await expect(searchResults.first()).toBeVisible({ timeout: 5000 });
    
    console.log('✅ Search functionality works - results appear');
  });
  ...
  test('Navigation links work', async ({ page }) => {
    ...
    const artistsLink = page.locator('a[href="/artists"], a:has-text("Artists")');
    ...
    if (await showsLink.count() > 0) {
      ...
      console.log('✅ Shows page navigation works');
    ...
    const trendingLink = page.locator('a[href="/trending"], a:has-text("Trending")');
    if (await trendingLink.count() > 0) {
      await trendingLink.first().click();
      await page.waitForLoadState('networkidle');
      expect(page.url()).toContain('/trending');
      console.log('✅ Trending page navigation works');
    }
  });
  ...
  test('Mobile navigation works', async ({ page }) => {
    ...
    if (await mobileMenuButton.count() > 0) {
      ...
      const mobileMenu = page.locator('[data-testid="mobile-nav"], .mobile-nav, [role="dialog"]');
      await expect(mobileMenu.first()).toBeVisible({ timeout: 5000 });
      
      console.log('✅ Mobile navigation opens successfully');
    } else {
      console.log('ℹ️ Mobile menu button not found - possibly responsive design handles this differently');
    }
  });
  ...
  test('Search to artist page flow', async ({ page }) => {
    ...
    const searchResultItem = page.locator('[data-testid="search-result"], .search-result, [role="option"]');
    ...
    if (await searchResultItem.count() > 0) {
      ...
      const isArtistPage = currentUrl.includes('/artists/') || currentUrl.includes('taylor-swift');
      
      expect(isArtistPage).toBeTruthy();
      console.log(`✅ Search to artist page flow works - navigated to: ${currentUrl}`);
    } else {
      console.log('ℹ️ No search results found - may need real data or API connectivity');
    }
  });
});