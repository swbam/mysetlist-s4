...
async function finalVerification() {
  try {
    ...
    console.log(`   Songs Synced: ${artist.song_catalog_synced_at ? '✅' : '❌'}`);
    ...
    if (parseInt(showsCount[0].count) > 0) {
      ...
      sampleShows.forEach(show => {
        console.log(`     - ${show.name || 'TBD'} at ${show.venue_name || 'TBD'} in ${show.city || 'TBD'} on ${show.date}`);
      });
    }
    ...
    if (parseInt(songsCount[0].count) > 0) {
      ...
      sampleSongs.forEach(song => {
        console.log(`     - ${song.name} (${song.album_name}) - Popularity: ${song.popularity}`);
      });
    }
    ...
    console.log(`   Shows: ${parseInt(showsCount[0].count) > 0 ? '✅' : '❌'} ${showsCount[0].count} shows`);
    console.log(`   Songs: ${parseInt(songsCount[0].count) > 0 ? '✅' : '❌'} ${songsCount[0].count} songs`);
    
    const isComplete = artist.import_status === 'completed' || artist.import_status === 'initializing';
    const hasShows = parseInt(showsCount[0].count) > 0;
    const hasSongs = parseInt(songsCount[0].count) > 0;
