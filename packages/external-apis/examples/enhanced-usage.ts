  ...
  try {
    ...
    if (identifiers) {
      ...
      if (identifiers.spotifyId) {
        ...
        const catalogResult = await artistSyncService.syncCatalog(identifiers.spotifyId);
    ...
    const showResults = await showSyncService.syncArtistShows('artist-uuid-here');
    ...
    const setlistResult = await setlistSyncService.ensureInitialSetlists('show-uuid-here', {
      songCount: 5,
    ...
    console.log(`📀 Catalog: ${catalog.totalSongs} songs, ${catalog.totalAlbums} albums`);
...
async function batchProcessingExample() {
  ...
  for (const artistId of artistIds) {
    try {
      ...
      console.log(`✅ Completed ${artistId}: ${catalogResult.totalSongs} songs`);
