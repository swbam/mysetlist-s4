import { db, shows, showArtists, artists, venues } from '@repo/database';
import { TicketmasterClient, TicketmasterEvent } from '../clients/ticketmaster';
import { SetlistFmClient, SetlistFmSetlist } from '../clients/setlistfm';
...
export class ShowSyncService {
    ...
    try {
      ...
      if (event._embedded?.attractions?.[0]) {
        ...
        try {
          ...
          console.error(`Failed to find artist on Spotify: ${attraction.name}`, error);
        }
      }
      ...
      if (event._embedded?.attractions && event._embedded.attractions.length > 1) {
        for (let i = 1; i < event._embedded.attractions.length; i++) {
          ...
          try {
            ...
            console.error(`Failed to add supporting act: ${attraction.name}`, error);
          }
        }
      }
    } catch (error) {
      console.error(`Failed to sync show from Ticketmaster: ${event.name}`, error);
      throw error;
    }
  }
  ...
  async syncShowFromSetlistFm(setlist: SetlistFmSetlist): Promise<void> {
    try {
      ...
      console.error(`Failed to sync show from Setlist.fm: ${setlist.id}`, error);
      throw error;
    }
  }
  ...
  async syncHistoricalSetlists(artistName: string): Promise<void> {
    try {
      ...
      console.error(`Failed to sync historical setlists for ${artistName}:`, error);
      throw error;
    }
  }
  ...
  private mapTicketmasterStatus(code: string): 'upcoming' | 'ongoing' | 'completed' | 'cancelled' {
