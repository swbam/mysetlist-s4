...
import { TicketmasterClient, TicketmasterVenue } from '../clients/ticketmaster';
import { SetlistFmClient, SetlistFmVenue } from '../clients/setlistfm';
...
export class VenueSyncService {
  ...
  async syncVenueFromTicketmaster(ticketmasterVenue: TicketmasterVenue): Promise<void> {
    try {
      await db
        .insert(venues)
        .values({
      ...
      console.error(`Failed to sync venue ${ticketmasterVenue.name}:`, error);
      throw error;
    }
  }
  ...
  async syncVenueFromSetlistFm(setlistFmVenue: SetlistFmVenue): Promise<void> {
    try {
      ...
      console.error(`Failed to sync venue ${setlistFmVenue.name}:`, error);
      throw error;
    }
  }

  async syncVenuesByCity(city: string, stateCode?: string, countryCode = 'US'): Promise<void> {
    try {
      ...
      console.error(`Failed to sync venues for ${city}:`, error);
      throw error;
    }
  }
  ...
  async syncMajorVenues(): Promise<void> {
    ...
    for (const { city, stateCode } of majorCities) {
      try {
        ...
        console.error(`Failed to sync venues for ${city}, ${stateCode}:`, error);
      }
    }
  }
