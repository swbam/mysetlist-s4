import { artists, db, eq, shows, sql, venues } from "@repo/database";
...
export class EnhancedShowVenueSync {
  ...
  async syncArtistShowsAndVenues(
    ...
    try {
      ...
      if (!artist || !artist.tmAttractionId) {
        throw new Error(
          `Artist not found or missing Ticketmaster ID: ${artistId}`,
        );
      }

      // Fetch shows with retry logic
      options.onProgress?.("Fetching shows from Ticketmaster...", 10);
      const showsData = await this.fetchShowsWithRetry(
      ...
      const now = new Date();
      const upcomingShows = showsData.filter(
      ...
      const venuesResult = await this.processVenuesInParallel(
        showsData,
        options,
      );
      ...
      await this.saveShowsWithVenueLinks(
        artistId,
        showsData,
        venuesResult.venueMap,
      );
  ...
  private async fetchShowsWithRetry(
    ...
    try {
      ...
      if (retryCount < this.maxRetries) {
        ...
        console.warn(
          `Retrying show fetch (attempt ${retryCount + 1}/${this.maxRetries}) after ${delay}ms...`,
        );

        await this.delay(delay);
        return this.fetchShowsWithRetry(
          tmAttractionId,
          options,
          retryCount + 1,
        );
  ...
  private async createSetlistsForShows(
    ...
    console.log(
      `Would create setlists for ${upcomingShows.length} upcoming shows`,
    );
  }
  ...
  async getArtistShowSyncStatus(artistId: string): Promise<{
    ...
    const hoursSinceSync = stats.last_synced_at
      ? (Date.now() - new Date(stats.last_synced_at).getTime()) /
        (1000 * 60 * 60)
  ...
  async syncVenuesFromShows(): Promise<{
    ...
    const venues: VenueData[] = venueDataList.map((data: any) => ({
      ...
      latitude: data.location?.latitude
        ? Number.parseFloat(data.location.latitude)
        : undefined,
      longitude: data.location?.longitude
        ? Number.parseFloat(data.location.longitude)
        : undefined,
