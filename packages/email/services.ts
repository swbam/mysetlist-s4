...
export async function sendShowReminderEmail({
  ...
  const html = await render(ShowReminderTemplate({ userName, show, appUrl, daysUntilShow }));
  
  const timeText = daysUntilShow === 0 ? 'today' : 
                   daysUntilShow === 1 ? 'tomorrow' : 
...
export async function sendNewShowNotificationEmail({
  ...
  const html = await render(NewShowNotificationTemplate({ userName, show, appUrl }));
...
export async function sendSetlistUpdateEmail({
  ...
  const html = await render(SetlistUpdateTemplate({ 
    userName, 
    show, 
    newSongs, 
    totalSongs, 
...
export async function sendWeeklyDigestEmail({
  ...
  const html = await render(WeeklyDigestTemplate({ 
    userName, 
    weekOf, 
    followedArtists, 
    upcomingShows, 
...
export async function sendPasswordResetEmail({
  ...
  const html = await render(PasswordResetTemplate({ name, resetUrl, appUrl, expirationHours }));
...
export async function sendEmailVerificationEmail({
  ...
  const html = await render(EmailVerificationTemplate({ name, verificationUrl, appUrl, expirationHours }));
...
export function validateUnsubscribeToken(token: string): { userId: string; emailType: string } | null {
  try {
