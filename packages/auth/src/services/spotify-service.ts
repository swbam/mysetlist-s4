...
export class SpotifyService {
   ...
   */
  async getUserTopArtists(accessToken: string, timeRange: 'short_term' | 'medium_term' | 'long_term' = 'medium_term', limit: number = 20): Promise<any[]> {
    try {
      const response = await fetch(`${this.baseUrl}/me/top/artists?time_range=${timeRange}&limit=${limit}`, {
        headers: {
          'Authorization': `Bearer ${accessToken}`,
          'Content-Type': 'application/json',
        },
   ...
   */
  async getUserTopTracks(accessToken: string, timeRange: 'short_term' | 'medium_term' | 'long_term' = 'medium_term', limit: number = 20): Promise<any[]> {
    try {
   ...
   */
  async getUserRecentlyPlayed(accessToken: string, limit: number = 20): Promise<any[]> {
    try {
   ...
   */
  async searchArtists(accessToken: string, query: string, limit: number = 20): Promise<any[]> {
    try {
      const encodedQuery = encodeURIComponent(query);
      const response = await fetch(`${this.baseUrl}/search?q=${encodedQuery}&type=artist&limit=${limit}`, {
        headers: {
          'Authorization': `Bearer ${accessToken}`,
          'Content-Type': 'application/json',
        },
