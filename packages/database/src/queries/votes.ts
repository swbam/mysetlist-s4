...
import { votes, setlistSongs, setlists, shows, users } from '../schema';
import { eq, sql, desc, asc, and, or, inArray, gte, lte } from 'drizzle-orm';
...
export async function getVoteCountsForSetlist(setlistId: string) {
  ...
  Object.keys(songVotes).forEach(songId => {
    songVotes[songId].netVotes = songVotes[songId].upvotes - songVotes[songId].downvotes;
  });

  return songVotes;
}

export async function getTopVotedSongsForShow(showId: string, options?: {
  limit?: number;
  minVotes?: number;
  ...
  const orderColumn = voteType === 'up' ? setlistSongs.upvotes 
                    : voteType === 'down' ? setlistSongs.downvotes 
...
export async function getUserVotingHistory(userId: string, options?: {
  limit?: number;
  showId?: string;
  voteType?: 'up' | 'down';
...
export async function getVotingStatistics(options?: {
  ...
  const [totalVotes, voteBreakdown, topVoters, mostVotedSongs] = await Promise.all([
    // Total votes
  ...
  return {
    ...
    voteBreakdown: voteBreakdown.reduce((acc, { voteType, count }) => {
      acc[voteType] = count;
      return acc;
...
export async function getVotingTrends(options?: {
  ...
  let query = db
    ...
    .innerJoin(setlists, eq(setlistSongs.setlistId, setlists.id))
    .where(
      gte(votes.createdAt, sql`NOW() - INTERVAL '${sql.raw(days.toString())} days'`)
    )
    .groupBy(dateFormat)
