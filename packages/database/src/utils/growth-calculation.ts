 ...
 */
export function calculateRealGrowth(
  current: number,
...
export function calculateArtistGrowth(artist: {
  ...
  const followersGrowth = calculateRealGrowth(
    artist.followers,
    artist.previousFollowers,
  );
  const popularityGrowth = calculateRealGrowth(
    artist.popularity,
    artist.previousPopularity,
  );
  const monthlyListenersGrowth = calculateRealGrowth(
    artist.monthlyListeners || 0,
    artist.previousMonthlyListeners,
  );
  const appFollowersGrowth = calculateRealGrowth(
    artist.followerCount,
    artist.previousFollowerCount,
  );
...
export function calculateShowGrowth(show: {
  ...
  const viewGrowth = calculateRealGrowth(
    show.viewCount,
    show.previousViewCount,
  );
  const attendeeGrowth = calculateRealGrowth(
    show.attendeeCount,
    show.previousAttendeeCount,
  );
  const voteGrowth = calculateRealGrowth(
    show.voteCount,
    show.previousVoteCount,
  );
  const setlistGrowth = calculateRealGrowth(
    show.setlistCount || 0,
    show.previousSetlistCount,
  );
...
export function calculateVenueGrowth(venue: {
  ...
  const totalShowsGrowth = calculateRealGrowth(
    venue.totalShows,
    venue.previousTotalShows,
  );
  const upcomingShowsGrowth = calculateRealGrowth(
    venue.upcomingShows,
    venue.previousUpcomingShows,
  );
  const attendanceGrowth = calculateRealGrowth(
    venue.totalAttendance || 0,
    venue.previousTotalAttendance,
  );
...
export function createHistoricalSnapshot<T extends Record<string, any>>(
  ...
  for (const field of fieldsToTrack) {
    const previousField =
      `previous${String(field).charAt(0).toUpperCase()}${String(field).slice(1)}` as keyof T;
