...
-- Function to update trending scores based on vote velocity
...
BEGIN
  ...
  SET 
    trending_score = GREATEST(0, (
      -- Recent vote activity (weighted by time)
      COALESCE((
        SELECT SUM(
          CASE 
            WHEN v.created_at > NOW() - time_window THEN 
              vote_weight * EXP(-EXTRACT(EPOCH FROM (NOW() - v.created_at)) / 3600) -- Exponential decay by hour
            ELSE 0 
          END
        )
        FROM votes v
  ...
  WHERE 
    ...
    AND last_synced_at > NOW() - INTERVAL '7 days'; -- Only update recently synced artists
