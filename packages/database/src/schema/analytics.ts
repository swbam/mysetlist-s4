...
export const pageViewStats = pgTable('page_view_stats', {
  ...
  periodIdx: index('idx_page_stats_period').on(table.period, table.periodStart),
...
export const artistAnalytics = pgTable('artist_analytics', {
  ...
  artistPeriodIdx: index('idx_artist_analytics_period').on(table.artistId, table.period, table.periodStart),
  trendingIdx: index('idx_artist_analytics_trending').on(table.trendingScore),
}));

export const showAnalytics = pgTable('show_analytics', {
  id: uuid('id').primaryKey().defaultRandom(),
  ...
  showPeriodIdx: index('idx_show_analytics_period').on(table.showId, table.period, table.periodStart),
}));
...
export const userRecommendations = pgTable('user_recommendations', {
  ...
  recommendationType: varchar('recommendation_type', { length: 50 }).notNull(), // 'artist', 'show', 'venue'
  ...
  userTypeIdx: index('idx_recommendations_user_type').on(table.userId, table.recommendationType),
...
export const experimentAssignments = pgTable('experiment_assignments', {
  ...
  experimentUserIdx: index('idx_exp_assignments').on(table.experimentId, table.userId),
...
export const experimentResults = pgTable('experiment_results', {
  ...
  experimentVariantIdx: index('idx_exp_results').on(table.experimentId, table.variant),
}));
...
export const anomalies = pgTable('anomalies', {
  ...
  entityIdx: index('idx_anomalies_entity').on(table.entityType, table.entityId),
}));