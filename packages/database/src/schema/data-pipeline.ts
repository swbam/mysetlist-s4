...
export const etlJobRuns = pgTable('etl_job_runs', {
  id: uuid('id').primaryKey().defaultRandom(),
  jobId: uuid('job_id').references(() => etlJobs.id).notNull(),
...
export const materializedViews = pgTable('materialized_views', {
  id: uuid('id').primaryKey().defaultRandom(),
...
export const factEvents = pgTable('fact_events', {
  id: uuid('id').primaryKey().defaultRandom(),
...
export const aggregatedMetrics = pgTable('aggregated_metrics', {
  ...
  metricEntityIdx: index('idx_agg_metrics_entity').on(table.metricName, table.entityType, table.entityId),
  periodIdx: index('idx_agg_metrics_period').on(table.periodType, table.periodStart),
}));
...
export const streamCheckpoints = pgTable('stream_checkpoints', {
  ...
  streamPartitionIdx: index('idx_checkpoint_stream').on(table.streamName, table.partitionKey),
}));
...
export const dataQualityMetrics = pgTable('data_quality_metrics', {
  ...
  tableColumnIdx: index('idx_quality_table_col').on(table.tableName, table.columnName),
