...
export const scalabilityPlans = pgTable(
  "scalability_plans",
  {
  ...
  (table) => ({
    ...
    currentUsersIdx: index("scalability_plans_current_users_idx").on(
      table.currentUsers,
    ),
    targetUsersIdx: index("scalability_plans_target_users_idx").on(
      table.targetUsers,
    ),
...
export const scalabilityMilestones = pgTable(
  "scalability_milestones",
  {
    id: serial("id").primaryKey(),
    planId: integer("plan_id")
      .references(() => scalabilityPlans.id, { onDelete: "cascade" })
      .notNull(),
  ...
  (table) => ({
    ...
    scheduledDateIdx: index("scalability_milestones_scheduled_date_idx").on(
      table.scheduledDate,
    ),
  }),
);
...
export const infrastructureConfigurations = pgTable(
  ...
  (table) => ({
    planIdIdx: index("infrastructure_configurations_plan_id_idx").on(
      table.planId,
    ),
    typeIdx: index("infrastructure_configurations_type_idx").on(table.type),
    environmentIdx: index("infrastructure_configurations_environment_idx").on(
      table.environment,
    ),
  }),
);
...
export const performanceBenchmarks = pgTable(
  ...
  {
    id: serial("id").primaryKey(),
    planId: integer("plan_id").references(() => scalabilityPlans.id, {
      onDelete: "cascade",
    }),
  ...
  (table) => ({
    ...
    userCountIdx: index("performance_benchmarks_user_count_idx").on(
      table.userCount,
    ),
    testDateIdx: index("performance_benchmarks_test_date_idx").on(
      table.testDate,
    ),
    environmentIdx: index("performance_benchmarks_environment_idx").on(
      table.environment,
    ),
  }),
);
...
export const scalabilityCosts = pgTable(
  "scalability_costs",
  {
    id: serial("id").primaryKey(),
    planId: integer("plan_id")
  ...
  (table) => ({
    ...
    periodStartIdx: index("scalability_costs_period_start_idx").on(
      table.periodStart,
    ),
  }),
);
...
export const scalabilityRisks = pgTable(
  "scalability_risks",
  {
    id: serial("id").primaryKey(),
    planId: integer("plan_id")
...
export const capacityPlanning = pgTable(
  ...
  (table) => ({
    ...
    resourceTypeIdx: index("capacity_planning_resource_type_idx").on(
      table.resourceType,
    ),
    environmentIdx: index("capacity_planning_environment_idx").on(
      table.environment,
    ),
  }),
);
...
export const scalabilityEvents = pgTable(
  "scalability_events",
  {
    id: serial("id").primaryKey(),
    planId: integer("plan_id").references(() => scalabilityPlans.id, {
  ...
  (table) => ({
    ...
    eventTypeIdx: index("scalability_events_event_type_idx").on(
      table.eventType,
    ),
    eventDateIdx: index("scalability_events_event_date_idx").on(
      table.eventDate,
    ),
...
export type InfrastructureConfiguration =
  typeof infrastructureConfigurations.$inferSelect;
export type NewInfrastructureConfiguration =
  typeof infrastructureConfigurations.$inferInsert;
