...
export const emailQueueEnhanced = pgTable('email_queue_enhanced', {
  id: uuid('id').primaryKey().defaultRandom(),
  ...
  scheduledIdx: index('idx_email_queue_enh_scheduled').on(table.scheduledFor, table.status),
  ...
  priorityIdx: index('idx_email_queue_enh_priority').on(table.priority, table.scheduledFor),
}));
...
export const digestContent = pgTable('digest_content', {
  id: uuid('id').primaryKey().defaultRandom(),
  ...
  userDigestIdx: index('idx_digest_content_user').on(table.userId, table.digestType, table.scheduledFor),
  includedIdx: index('idx_digest_content_included').on(table.includedInDigest),
}));
...
export const artistUpdateSubscriptions = pgTable('artist_update_subscriptions', {
  id: uuid('id').primaryKey().defaultRandom(),
  userId: uuid('user_id').references(() => users.id).notNull(),
...
export const showReminderSubscriptions = pgTable('show_reminder_subscriptions', {
  id: uuid('id').primaryKey().defaultRandom(),
  userId: uuid('user_id').references(() => users.id).notNull(),
