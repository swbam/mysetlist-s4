...
export class RealtimeManager {
    ...
    const channel = supabase
      ...
      .subscribe((status) => {
        if (status === 'SUBSCRIBED') {
          ...
          console.error(`❌ Failed to subscribe to setlist updates for show ${showId}`);
        }
      });
  ...
  subscribeToVoteUpdates(
    ...
    const channel = supabase
      ...
      .subscribe((status) => {
        if (status === 'SUBSCRIBED') {
          console.log(`✅ Subscribed to vote updates for setlist song ${setlistSongId}`);
        } else if (status === 'CHANNEL_ERROR') {
          console.error(`❌ Failed to subscribe to vote updates for setlist song ${setlistSongId}`);
        }
      });
    ...
    const channel = supabase
      ...
      .subscribe((status) => {
        if (status === 'SUBSCRIBED') {
          ...
          console.error(`❌ Failed to subscribe to attendance updates for show ${showId}`);
        }
      });
  ...
  subscribeToArtistFollowers(
    ...
    const channel = supabase
      ...
      .subscribe((status) => {
        if (status === 'SUBSCRIBED') {
          console.log(`✅ Subscribed to follower updates for artist ${artistId}`);
        } else if (status === 'CHANNEL_ERROR') {
          console.error(`❌ Failed to subscribe to follower updates for artist ${artistId}`);
        }
      });
    ...
    const channel = supabase
      ...
      .subscribe((status) => {
        if (status === 'SUBSCRIBED') {
          ...
          console.error(`❌ Failed to subscribe to setlist votes for ${setlistId}`);
        }
      });
...
export const subscribeToSetlistUpdates = (showId: string, callback: (payload: SetlistUpdatePayload) => void) =>
  realtimeManager.subscribeToSetlistUpdates(showId, callback);

export const subscribeToVoteUpdates = (setlistSongId: string, callback: (payload: VoteUpdatePayload) => void) =>
  realtimeManager.subscribeToVoteUpdates(setlistSongId, callback);

export const subscribeToShowUpdates = (showId: string, callback: (payload: any) => void) =>
  realtimeManager.subscribeToShowUpdates(showId, callback);

export const subscribeToAttendanceUpdates = (showId: string, callback: (payload: any) => void) =>
  realtimeManager.subscribeToAttendanceUpdates(showId, callback);

export const subscribeToArtistFollowers = (artistId: string, callback: (payload: any) => void) =>
...
export const subscribeToSetlistVotes = (setlistId: string, callback: (payload: any) => void) =>
