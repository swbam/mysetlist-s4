...
import { artists, venues, shows, songs, setlists, setlistSongs, users, showArtists } from '../schema';
...
export class DatabaseSeeder {
  ...
  async seedDatabase(options: SeedOptions = {}): Promise<void> {
    ...
    try {
      ...
      // Seed shows
      const seededShows = await this.seedShows(showCount, seededArtists, seededVenues);
  ...
  private async seedWithExternalApis(syncPopularArtists: boolean): Promise<void> {
  ...
  private async seedVenues(count: number): Promise<any[]> {
    const sampleVenues = Array.from({ length: count }, (_, i) => {
      const venueTypes = ['arena', 'theater', 'club', 'amphitheater', 'stadium', 'hall'];
      const cities = ['New York', 'Los Angeles', 'Chicago', 'Austin', 'Nashville', 'Seattle', 'Portland', 'Denver', 'Atlanta', 'Boston'];
      const states = ['NY', 'CA', 'IL', 'TX', 'TN', 'WA', 'OR', 'CO', 'GA', 'MA'];
  ...
  private async seedArtists(count: number): Promise<any[]> {
    const artistNames = [
      'The Electric Dreamers', 'Neon Nights', 'Crimson Tide', 'Silver Shadows', 'Golden Hour',
      'Midnight Express', 'Stellar Winds', 'Ocean Drive', 'Mountain Echo', 'Desert Storm',
      'Arctic Monkeys Revival', 'Cosmic Debris', 'Radio Static', 'Velvet Underground 2.0', 'Glass Animals Tribute',
      'The Strokes Cover Band', 'Modern Rock Collective', 'Indie Folk Project', 'Electronic Fusion', 'Alternative Edge',
      'Vinyl Records', 'Cassette Dreams', 'Digital Harmony', 'Analog Soul', 'Synthetic Love',
      'Retro Future', 'New Wave Revival', 'Post Punk Kids', 'Shoegaze Society', 'Dream Pop Collective',
      'Garage Rock Band', 'Psych Rock Ensemble', 'Prog Rock Orchestra', 'Math Rock Quartet', 'Art Rock Installation',
      'Ambient Soundscape', 'Noise Rock Experiment', 'Lo-Fi Hip Hop', 'Chillwave Collective', 'Synthwave Society',
      'Darkwave Underground', 'Newwave Tomorrow', 'Coldwave Winter', 'Minimal Techno', 'Deep House Project',
      'Acid Jazz Fusion', 'Nu Soul Revival', 'Future Funk', 'Vaporwave Aesthetic', 'Cyberpunk Orchestra'
    ];
    ...
    const sampleArtists = Array.from({ length: Math.min(count, artistNames.length) }, (_, i) => ({
      name: artistNames[i],
  ...
  private async seedSongs(count: number, artistList: any[]): Promise<any[]> {
    const songTitles = [
      'Electric Dreams', 'Neon Lights', 'Midnight Drive', 'City Skyline', 'Ocean Waves',
      'Mountain High', 'Desert Road', 'River Flow', 'Forest Deep', 'Sky Blue',
      'Starlight', 'Moonbeam', 'Sunshine', 'Rainbow', 'Thunder',
      'Lightning', 'Storm Clouds', 'Clear Skies', 'Windy Day', 'Calm Night',
      'Urban Jungle', 'Concrete Dreams', 'Steel and Glass', 'Neon Signs', 'Traffic Lights',
      'Subway Ride', 'Bus Stop', 'Train Station', 'Airport Terminal', 'Highway Exit',
      'Coffee Shop', 'Bookstore', 'Record Store', 'Art Gallery', 'Concert Hall',
      'Dance Floor', 'Rooftop Party', 'Beach Bonfire', 'Camping Trip', 'Road Trip',
      'Home Sweet Home', 'Familiar Faces', 'Old Friends', 'New Beginnings', 'Fresh Start',
      'Second Chances', 'Third Time', 'Lucky Number', 'Golden Ticket', 'Silver Lining'
    ];

    const sampleSongs = Array.from({ length: Math.min(count, songTitles.length) }, (_, i) => {
  ...
  private async seedShows(count: number, artistList: any[], venueList: any[]): Promise<any[]> {
    const showNames = [
      'Summer Festival', 'Winter Concert', 'Spring Tour', 'Fall Sessions',
      'Album Release Party', 'Acoustic Evening', 'Electric Night', 'Unplugged Session',
      'Greatest Hits Tour', 'New Album Showcase', 'Farewell Concert', 'Reunion Show',
      'Charity Benefit', 'Festival Headliner', 'Late Night Set', 'Afternoon Acoustic',
      'Birthday Celebration', 'Anniversary Show', 'Holiday Special', 'New Year Concert'
    ];
    ...
    const sampleShows = Array.from({ length: count }, (_, i) => {
      ...
      const showDate = new Date(baseDate.getTime() + daysOffset * 24 * 60 * 60 * 1000);
      ...
      return {
        ...
        slug: `${artist.name}-${showName}`.toLowerCase().replace(/[^a-z0-9]+/g, '-'),
  ...
  private async seedSetlists(showList: any[], songList: any[]): Promise<any[]> {
    ...
    for (const show of showList) {
      ...
      for (let i = 0; i < setlistCount; i++) {
        ...
        const insertedSetlist = await db.insert(setlists)
          .values({
            showId: show.id,
        ...
        if (insertedSetlist[0]) {
          ...
          const selectedSongs = this.shuffleArray([...songList]).slice(0, songCount);
