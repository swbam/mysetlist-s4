...
export function usePerformanceMonitor(options: PerformanceMonitorOptions = {}) {
    ...
    const observer = new PerformanceObserver((list) => {
      list.getEntries().forEach((entry) => {
        if (entry.entryType === 'paint') {
          ...
          updateMetric('domContentLoaded', navEntry.domContentLoadedEventEnd - navEntry.navigationStart);
          updateMetric('loadComplete', navEntry.loadEventEnd - navEntry.navigationStart);
        }
      });
    });
  ...
  const markComponentMount = useCallback((componentName: string) => {
...
export function useComponentPerformance(componentName: string, deps: any[] = []) {
  const { markComponentMount, measureRenderTime, metrics } = usePerformanceMonitor({
