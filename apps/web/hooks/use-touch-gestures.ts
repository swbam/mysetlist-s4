    ...
    if (pullStartRef.current !== null && onPullToRefresh) {
      ...
      if (pullDistance > 0 && window.scrollY === 0) {
        ...
        if (pullDistance > pullThreshold && !isPullingRef.current) {
          ...
          indicator.className = 'fixed top-0 left-0 right-0 h-16 bg-primary/10 flex items-center justify-center z-50';
          indicator.innerHTML = '<div class="animate-spin h-6 w-6 border-2 border-primary border-t-transparent rounded-full"></div>';
