...
export function useSocialInteractions({
  ...
  const { user } = useAuth();
  const [status, setStatus] = useState<SocialStatus>({ liked: false, saved: false });
  ...
  const handleAction = useCallback(async (
    ...
    try {
      ...
      if (response.ok) {
        ...
        setStatus(prev => ({
          ...prev,
          liked: action === 'like' ? true : action === 'unlike' ? false : prev.liked,
          saved: action === 'save' ? true : action === 'unsave' ? false : prev.saved,
        }));
...
export function useSocialInteractionsBatch(
  ...
  const updateStatus = useCallback((targetId: string, updates: Partial<SocialStatus>) => {
    setStatuses(prev => ({
      ...prev,
      [targetId]: {
        ...prev[targetId],
