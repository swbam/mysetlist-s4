...
export function useRealtimeShows({ 
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        {
          ...
          schema: 'public',
          table: 'shows',
          ...(Object.keys(filters).length > 0 && { filter: Object.entries(filters).map(([k, v]) => `${k}=${v}`).join(',') }),
        },
    ...
    channel.on(
      ...
      async (payload: RealtimePostgresChangesPayload<Show>) => {
        if (payload.new && 'id' in payload.new) {
          ...
          if (artistId && 'artist_id' in payload.new && payload.new.artist_id !== artistId) return;
          if (venueId && 'venue_id' in payload.new && payload.new.venue_id !== venueId) return;
          if (status && 'status' in payload.new && payload.new.status !== status) return;
  ...
  }, [artistId, venueId, status, limit, supabase, fetchShows, onNewShow, onShowUpdated]);
