...
export function useServiceWorker(options: ServiceWorkerOptions = {}) {
  ...
  const storeOfflineAction = useCallback(async (action: any) => {
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (data.type === 'CACHE_UPDATED') {
        ...
        setOfflineActions(prev => prev.filter(action => action.id !== data.action.id));
      }
    };
...
export function useOfflineManager() {
  ...
  const handleOfflineAction = useCallback(async (
    action: 'vote' | 'follow' | 'comment' | 'attend',
    data: any
  ) => {
    if (isOnline) {
