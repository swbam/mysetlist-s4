...
Cypress.Commands.add('voteForSong', (songId: string, voteType: 'up' | 'down') => {
...
Cypress.Commands.add('waitForSync', () => {
  cy.get('[data-testid="sync-indicator"]', { timeout: 30000 }).should('not.exist')
...
Cypress.Commands.add('interceptAPI', (alias: string, path: string, fixture?: string) => {
  if (fixture) {
...
declare global {
  namespace Cypress {
    interface Chainable {
      ...
      interceptAPI(alias: string, path: string, fixture?: string): Chainable<void>
