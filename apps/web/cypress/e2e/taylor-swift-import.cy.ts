...
describe('Taylor Swift Artist Import - Real Data Test', () => {
  ...
  it('should search for Taylor Swift and import her data', () => {
    ...
    cy.get('[data-testid="search-input"], input[placeholder*="Search"]').first().type('Taylor Swift');
    ...
    cy.get('[data-testid="artist-image"], img[alt*="Taylor Swift"]').should('be.visible');
    
    // Check for songs section
    cy.contains('Songs', { timeout: 30000 }).should('be.visible');
    
    // Verify songs are loaded (excluding live tracks)
    cy.get('[data-testid="song-item"], [data-testid="track-item"], div').then($songs => {
      const songTexts = Array.from($songs).map(el => el.textContent?.toLowerCase() || '');
      
      // Verify no live tracks
      const liveTrackCount = songTexts.filter(text => 
        text.includes('live') || 
        text.includes('acoustic') || 
        text.includes('session')
      ).length;
      
      cy.log(`Found ${$songs.length} songs, ${liveTrackCount} appear to be live tracks`);
    ...
    cy.get('[data-testid="show-item"], [data-testid="event-item"], div').should('have.length.greaterThan', 0);
...
describe('Artist Import System Validation', () => {
  it('should properly filter live tracks during import', () => {
    ...
    // Get all song titles
    cy.get('[data-testid="song-title"], [data-testid="track-title"], h3, h4').then($titles => {
      ...
      const liveIndicators = [
        ...
        '(Live)', '[Live]', 'Live Version',
        'Acoustic Session', 'MTV Unplugged', 'Radio Session'
      ];
  ...
  it('should create predicted setlists for upcoming shows', () => {
    ...
    cy.get('[data-testid="show-item"], [data-testid="event-item"]').first().click();
    ...
    cy.get('[data-testid="setlist-song"], [data-testid="track-item"]').should('have.length.greaterThan', 0);
