...
export function FollowButton({
  ...
  const getButtonContent = () => {
    if (loading) {
      return (
        <>
          <Loader2 className={cn("animate-spin", showText ? "mr-2" : "", size === 'sm' ? "h-3 w-3" : "h-4 w-4")} />
    ...
    if (following) {
      return (
        <>
          {showIcon && (
            <Heart className={cn("fill-current", showText ? "mr-2" : "", size === 'sm' ? "h-3 w-3" : "h-4 w-4")} />
          )}
          {showText && 'Following'}
        </>
      );
    ...
    return (
      <>
        {showIcon && (
          <Heart className={cn(showText ? "mr-2" : "", size === 'sm' ? "h-3 w-3" : "h-4 w-4")} />
        )}
        {showText && 'Follow'}
      </>
    );
  ...
  return (
    <Button
      ...
      className={cn(
        ...
        following && variant === 'default' && "border-red-200 text-red-600 hover:bg-red-50 hover:border-red-300",
        className
      )}
...
export function FollowButtonCompact(props: Omit<FollowButtonProps, 'showText' | 'size'>) {
  return (
    <FollowButton
      {...props}
      showText={false}
