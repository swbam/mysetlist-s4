...
import { ChevronUp, ChevronDown, Loader2, Wifi, WifiOff, Zap } from 'lucide-react';
...
export function RealtimeVoteButton({
  ...
  const [lastVoteAnimation, setLastVoteAnimation] = useState<'increase' | 'decrease' | null>(null);
  ...
  const { votes, isVoting, vote: optimisticVote, isOptimistic } = useOptimisticVoting({
    setlistSongId,
    userId,
  ...
  useEffect(() => {
    if (previousNetVotes.current !== netVotes) {
      const direction = netVotes > previousNetVotes.current ? 'increase' : 'decrease';
  ...
  const triggerHapticFeedback = useCallback((pattern: number[] = [10]) => {
    if (hapticFeedback && 'vibrate' in navigator) {
  ...
  if (variant === 'compact') {
    return (
      <TooltipProvider>
        <div className={cn('flex items-center gap-1 touch-manipulation', className)}>
          <Tooltip>
            <TooltipTrigger asChild>
              <motion.div
              ...
              >
                <Button
                  ...
                  disabled={isVoting || disabled || (voteLimits && !voteLimits.canVote)}
                  className={cn(
                    buttonSize,
                    'p-0 transition-all duration-150 touch-manipulation',
                    'min-h-[32px] min-w-[32px]',
                    'focus-visible:ring-2 focus-visible:ring-green-500/20',
                    userVote === 'up' && 'bg-green-100 text-green-700 hover:bg-green-200 dark:bg-green-900/30 dark:text-green-400',
                    isOptimistic && 'opacity-80'
                  )}
          ...
          <motion.span 
            className={cn(
              'text-sm font-medium min-w-[2rem] text-center tabular-nums transition-all duration-150',
              'select-none px-1',
              netVotes > 0 && 'text-green-600 dark:text-green-400',
              netVotes < 0 && 'text-red-600 dark:text-red-400',
              netVotes === 0 && 'text-muted-foreground',
              ...
              >
                <Button
                  ...
                  className={cn(
                    ...
                    userVote === 'down' && 'bg-red-100 text-red-700 hover:bg-red-200 dark:bg-red-900/30 dark:text-red-400',
                    isOptimistic && 'opacity-80'
                  )}
  ...
  return (
    <TooltipProvider>
      <div className={cn('flex flex-col items-center gap-1 touch-manipulation', className)}>
        <Tooltip>
          <TooltipTrigger asChild>
            <motion.div
              variants={buttonVariants}
