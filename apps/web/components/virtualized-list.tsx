...
export function VirtualizedList<T>({
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        'relative overflow-auto scrollbar-thin scrollbar-thumb-border scrollbar-track-background',
        className
      )}
    ...
    >
      ...
      <div
        className={cn(
          'absolute right-1 top-1 bottom-1 w-1 bg-muted rounded-full transition-opacity duration-300',
          isScrolling ? 'opacity-100' : 'opacity-0'
        )}
      >
        <div
          className="absolute right-0 w-1 bg-primary rounded-full transition-all duration-150"
          style={{
...
export function ArtistList({ artists, ...props }: { artists: any[] } & Omit<VirtualizedListProps<any>, 'items' | 'renderItem'>) {
  return (
    <VirtualizedList
      items={artists}
      itemHeight={80}
...
export function ShowList({ shows, ...props }: { shows: any[] } & Omit<VirtualizedListProps<any>, 'items' | 'renderItem'>) {
  return (
    <VirtualizedList
      items={shows}
      itemHeight={120}
        ...
        <div className="p-4 border-b hover:bg-muted/50 transition-colors">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-muted-foreground">{show.venue} • {show.date}</p>
            </div>
          </div>
        </div>
      )}
