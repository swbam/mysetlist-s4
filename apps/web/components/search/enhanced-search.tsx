  ...
  const handleSuggestionSelect = async (suggestion: SearchSuggestion) => {
    ...
    switch (suggestion.type) {
      ...
      case 'genre':
        ...
        break;
      case 'location':
        router.push(`/discover?location=${encodeURIComponent(suggestion.title)}`);
        break;
      case 'recent':
      case 'trending':
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          >
            <CardContent className="p-4 text-center">
              ...
              <h3 className="font-semibold mb-1">Shows</h3>
              <p className="text-sm text-muted-foreground">
      ...
      {!isSearching && recentSearches.length === 0 && (
        <Card className="mt-6">
          ...
          <CardContent>
            ...
            <div className="mt-4">
              <h4 className="font-medium mb-2">Popular Genres</h4>
              <div className="flex flex-wrap gap-2">
                {['Rock', 'Pop', 'Hip-Hop', 'Electronic', 'Country', 'Jazz', 'R&B', 'Indie'].map((genre) => (
                  <Badge
                    key={genre}
                    variant="secondary"
                    className="cursor-pointer hover:bg-secondary/80"
                    onClick={() => router.push(`/discover?genre=${encodeURIComponent(genre)}`)}
                  >
                    {genre}
                  </Badge>
                ))}
