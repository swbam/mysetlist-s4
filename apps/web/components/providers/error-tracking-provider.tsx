...
export function ErrorTrackingProvider({ 
  ...
  const addCustomContext = useCallback((key: string, data: Record<string, any>) => {
    try {
      setContext(key, {
        ...data,
        timestamp: new Date().toISOString(),
  ...
  if (!context) {
    ...
    return {
      ...
      trackPerformance: (operation: string, duration: number, metadata?: Record<string, any>) => {
        console.log('Performance (no tracking):', operation, duration, metadata);
      },
...
export function useComponentTracking(componentName: string) {
  ...
  return {
    ...
    trackComponentPerformance: (operation: string, duration: number, metadata?: Record<string, any>) =>
