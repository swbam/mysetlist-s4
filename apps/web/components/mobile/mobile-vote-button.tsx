...
import { ChevronUp, ChevronDown, Heart, Zap } from 'lucide-react';
...
export function MobileVoteButton({
  ...
  const handleVote = async (voteType: 'up' | 'down') => {
    ...
    try {
      ...
      if (newVote && (votes.upvotes + votes.downvotes) % 10 === 0) {
        toast.success(`Song hit ${votes.upvotes + votes.downvotes + 1} votes!`, {
          icon: '🎵',
        });
      }
    } catch (error) {
  ...
  const getButtonStyle = (type: 'up' | 'down') => {
    ...
    const activeClasses = votes.userVote === type ? (
      type === 'up' 
  ...
  const getCountStyle = () => {
    if (compact) {
      return cn(
        'text-xs font-medium min-w-[1.5rem] text-center tabular-nums',
    ...
    return cn(
      'text-sm font-medium min-w-[2.5rem] text-center tabular-nums md:text-xs md:min-w-[2rem]',
      netVotes > 0 && 'text-green-600 dark:text-green-400',
      netVotes < 0 && 'text-red-600 dark:text-red-400',
      netVotes === 0 && 'text-muted-foreground'
    );
  ...
  if (compact) {
    return (
      <div className={cn("flex items-center gap-1", className)}>
        ...
        >
          <motion.div
            animate={isVoting && votes.userVote === 'up' ? { scale: [1, 1.2, 1] } : {}}
        ...
        >
          <motion.div
            animate={isVoting && votes.userVote === 'down' ? { scale: [1, 1.2, 1] } : {}}
            transition={{ duration: 0.3 }}
          >
  ...
  return (
    <div className={cn("flex flex-col items-center gap-1 md:flex-row md:gap-1", className)}>
      <Button
        variant="ghost"
        size="sm"
        onClick={() => handleVote('up')}
        ...
        <AnimatePresence>
          {netVotes >= 50 && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -10 }}
              className="flex items-center gap-1 text-xs text-orange-500 font-medium mt-1 md:mt-0 md:ml-1"
            >
      ...
      <div className="text-xs text-muted-foreground mt-1 md:mt-0 md:ml-2">
