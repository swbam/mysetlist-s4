...
export function TouchCard({
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    if (swipeActions && Math.abs(deltaX) > Math.abs(deltaY)) {
      const maxSwipe = 120;
      const constrainedDistance = Math.max(-maxSwipe, Math.min(maxSwipe, deltaX));
      ...
      {swipeActions && (
        <>
          ...
          {swipeActions.left && (
            <motion.div
              className={cn(
                "absolute right-0 top-0 h-full flex items-center px-4",
                swipeActions.left.variant === 'destructive' && "bg-destructive",
                swipeActions.left.variant === 'secondary' && "bg-secondary",
                (!swipeActions.left.variant || swipeActions.left.variant === 'default') && "bg-primary"
              )}
              style={{
          ...
          {swipeActions.right && (
            <motion.div
              className={cn(
                ...
                swipeActions.right.variant === 'destructive' && "bg-destructive",
                swipeActions.right.variant === 'secondary' && "bg-secondary",
                (!swipeActions.right.variant || swipeActions.right.variant === 'default') && "bg-primary"
              )}
              style={{
                width: Math.max(0, swipeDistance),
