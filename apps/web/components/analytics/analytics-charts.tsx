...
interface ChartProps {
  type: 'growth' | 'engagement' | 'engagement-trends' | 'trending-performance' | 'user-growth' | 'retention' | 'user-segments';
  height?: number;
...
export function AnalyticsCharts({ type, height = 300, period = 'week' }: ChartProps) {
  ...
  const generateMockData = (chartType: string, periodType: string): ChartData[] => {
    ...
    return Array.from({ length: days }, (_, i) => {
      ...
      return {
        ...
        value,
        previousValue: i > 0 ? Math.floor(baseValue * trend ** (i-1) * (1 + (Math.random() - 0.5) * variance)) : baseValue,
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <div className="text-2xl font-bold">{getCurrentValue().toLocaleString()}</div>
