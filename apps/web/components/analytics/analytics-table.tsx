...
export function AnalyticsTable({ type, limit = 10 }: TableProps) {
  ...
  const generateMockData = (tableType: string, itemLimit: number): TableRow[] => {
    switch (tableType) {
      case 'engaged-users':
      ...
      case 'trending-shows':
        const shows = ['Eras Tour', 'After Hours Til Dawn', 'Happier Than Ever', 'Love On Tour', 'Sweetener World Tour'];
      ...
      case 'top-venues':
        const venueNames = ['Madison Square Garden', 'Staples Center', 'Wembley Stadium', 'Red Rocks Amphitheatre', 'The Forum'];
      ...
      case 'recent-activity':
        const activities = ['Voted on song', 'Followed artist', 'Attended show', 'Created setlist', 'Shared show'];
        return Array.from({ length: itemLimit }, (_, i) => ({
          id: `activity-${i}`,
          name: `User ${i + 1}`,
          avatar: `https://api.dicebear.com/7.x/avataaars/svg?seed=${i}`,
  ...
  if (loading) {
    return (
      <div className="space-y-2">
        {[...Array(limit)].map((_, i) => (
          <div key={i} className="flex items-center gap-3 p-3 bg-muted/50 rounded animate-pulse">
            ...
            <TableRow key={row.id}>
                ...
                <>
                  <TableCell>
                    <div className="text-sm">
                      <div className="font-medium">{row.metric} {row.metricLabel}</div>
                      <div className="text-muted-foreground">{row.secondaryMetric} {row.secondaryLabel}</div>
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <div className="text-sm text-muted-foreground">{row.lastActive}</div>
                  </TableCell>
                </>
              )}
              ...
              {type === 'trending-artists' && (
                <>
                  ...
                  <TableCell>
                    <span className="text-sm">{row.secondaryMetric?.toLocaleString()}</span>
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2">
