    ...
    <div className={cn('relative flex-shrink-0', sizeClasses[size], className)}>
      {src ? (
        <OptimizedImage
          ...
          fallback={
            <div className="flex items-center justify-center h-full w-full rounded-full bg-muted text-muted-foreground font-medium">
              {initials}
            </div>
          }
        />
  ...
  if (images.length === 1) {
    return (
      <div className={className}>
        ...
        />
        {images[0].caption && (
          <p className="text-sm text-muted-foreground mt-2">{images[0].caption}</p>
        )}
      </div>
    );
  }
