...
export const OptimizedArtistCard = React.memo(function OptimizedArtistCard({
  ...
  return (
    <Link 
      ...
      className={cn('group block focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 rounded-lg', className)}
      prefetch
    >
          ...
          <div className={cn(
            ...
            {artist.imageUrl ? (
              ...
              <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-muted/50 to-muted">
                ...
                <ScreenReaderOnly>No image available for {artist.name}</ScreenReaderOnly>
              </div>
            )}
            ...
            {showRank && isTopRanked && (
              ...
              >
                <Badge className="border-0 bg-gradient-to-r from-yellow-500 to-orange-500 px-2.5 py-1 shadow-lg">
                  <AccessibleIcon icon={Sparkles} label="Top ranked" decorative />
            ...
            {showStats && hasGrowth && (
              <div className="absolute bottom-2 left-2 flex items-center gap-1 rounded-full bg-black/70 px-2.5 py-1 backdrop-blur-sm">
                <AccessibleIcon icon={TrendingUp} label="Trending up" className="h-3 w-3 text-green-400" />
                <span className="font-medium text-green-400 text-xs">
                  +{artist.weeklyGrowth}%
                </span>
                <ScreenReaderOnly>Weekly growth: {artist.weeklyGrowth} percent</ScreenReaderOnly>
              </div>
            )}
              ...
              <div className="absolute inset-0 flex flex-col justify-end p-3 opacity-0 transition-opacity duration-300 group-hover:opacity-100 group-focus:opacity-100">
                <div className="space-y-2">
                  <div className="flex items-center gap-1.5 text-white/90">
                    <AccessibleIcon icon={Users} label="Followers" className="h-3.5 w-3.5" />
                  ...
                  {artist.recentShows > 0 && (
                    <div className="flex items-center gap-1.5 text-white/90">
                      <AccessibleIcon icon={Music} label="Recent shows" className="h-3.5 w-3.5" />
                      <span className="font-medium text-xs">
