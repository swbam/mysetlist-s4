...
export function ArtistGrid({
  ...
  const buildFetchUrl = (pageNum: number, reset: boolean = false) => {
  ...
  const fetchArtists = async (reset: boolean = false) => {
    setLoading(true);
    try {
  ...
  const handleFollow = async (artistId: string, currentlyFollowing: boolean) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.success(currentlyFollowing ? 'Unfollowed artist' : 'Following artist');
  ...
  const getEmptyState = () => {
    ...
    return {
      title: 'No artists found',
      description: 'Start by searching for your favorite artists or explore trending musicians.',
      action: {
