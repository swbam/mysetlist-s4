...
const generateMockShows = (count = 1000) => {
  return Array.from({ length: count }, (_, i) => ({
    id: `event${i}`,
    name: `Concert ${i + 1} - City ${i % 50}`,
    dates: {
      start: {
        localDate: new Date(Date.now() + i * 24 * 60 * 60 * 1000)
          .toISOString()
          .split("T")[0],
        localTime: "20:00:00",
      },
    ...
    priceRanges: [
      { min: 50 + (i % 50), max: 200 + (i % 100), currency: "USD" },
    ],
  }));
};

// Mock comprehensive track data (2000+ tracks for SLO testing)
const generateMockTracks = (count = 2000) => {
  ...
  const tracks = Array.from({ length: count }, (_, i) => ({
    ...
    artists: [
      { id: mockArtistData.spotify.id, name: mockArtistData.spotify.name },
    ],
...
class SSEEventCollector extends EventEmitter {
  ...
  waitForStage(stage: string, timeout = 5000): Promise<any> {
...
describe("End-to-End Import Flow Tests", () => {
  ...
  const setupComprehensiveMocks = () => {
    ...
    vi.mock("../../lib/services/adapters/SpotifyClient", () => ({
      ...
      listAlbumTracks: vi.fn().mockImplementation(async (albumId: string) => {
        const albumIndex = Number.parseInt(albumId.replace("album", ""));
  ...
  describe("Complete Import Flow with Performance SLOs", () => {
    it("should complete full import within GROK.md SLO targets", async () => {
      ...
      if (!sloValidation.allPassed) {
        console.log(
          "SLO Violations:",
          sloValidation.results.filter((r) => !r.passed),
        );
      }
    ...
    it("should stream progress via SSE within 200ms of phase changes", async () => {
      ...
      for (let i = 1; i < stages.length; i++) {
        const currentStageEvents = sseEvents.filter(
          (e) => e.event.stage === stages[i],
        );
        const previousStageEvents = sseEvents.filter(
          (e) => e.event.stage === stages[i - 1],
        );

        if (currentStageEvents.length > 0 && previousStageEvents.length > 0) {
          const timeBetween =
            currentStageEvents[0].timestamp -
            previousStageEvents[previousStageEvents.length - 1].timestamp;
    ...
    it("should maintain < 1% import failure rate", async () => {
      ...
      console.log(
        `Import Success Rate: ${100 - failureRate}% (${importCount - failureCount}/${importCount})`,
      );
    });
  });
  ...
  describe("Error Recovery and Resilience", () => {
    it("should recover from transient API failures", async () => {
      ...
      vi.mocked(
        require("../../lib/services/adapters/SpotifyClient").getAccessToken,
    ...
    it("should handle concurrent imports without data corruption", async () => {
      ...
      const [result1, result2] = await Promise.allSettled([
        (async () => {
          ...
          return {
            identity,
            import: await orchestrator1.runFullImport(identity.artistId),
          };
        })(),
        (async () => {
          ...
          return {
            identity,
            import: await orchestrator2.runFullImport(identity.artistId),
          };
        })(),
      ]);
      ...
      if (result1.status === "fulfilled" && result2.status === "fulfilled") {
        ...
        expect(
          artist1Songs.every(
            (as) => as.artistId === result1.value.identity.artistId,
          ),
        ).toBe(true);
        expect(
          artist2Songs.every(
            (as) => as.artistId === result2.value.identity.artistId,
          ),
        ).toBe(true);
      }
    });
  ...
  describe("Auto-Import Functionality", () => {
    it("should auto-import artist when not found in database", async () => {
      ...
      vi.mock("@repo/external-apis", () => ({
        SpotifyClient: class {
          ...
          async searchArtists() {
            return mockSpotifySearch();
          }
        },
        TicketmasterClient: class {
          async searchAttractions() {
            return mockTicketmasterSearch();
          }
        },
      }));
    ...
    it("should handle auto-import API failures gracefully", async () => {
      // Mock API failures
      const mockSpotifySearch = vi
        .fn()
        .mockRejectedValue(new Error("Spotify API Error"));
      const mockTicketmasterSearch = vi
        .fn()
        .mockRejectedValue(new Error("Ticketmaster API Error"));
