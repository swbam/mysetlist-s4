...
jest.mock('@hello-pangea/dnd', () => ({
  ...
  Droppable: ({ children }: any) => <div data-testid="droppable">{children({ provided: { innerRef: jest.fn(), droppableProps: {} }, snapshot: {} })}</div>,
  Draggable: ({ children }: any) => <div data-testid="draggable">{children({ provided: { innerRef: jest.fn(), draggableProps: {}, dragHandleProps: {} }, snapshot: {} })}</div>,
}));
...
describe('Show Page Components', () => {
  describe('ShowHeader', () => {
    it('renders show information correctly', () => {
      ...
      expect(screen.getByText('Doors: 19:00 • Show: 20:00')).toBeInTheDocument();
  ...
  describe('AttendeeList', () => {
    it('renders attendee list with count', async () => {
      render(<AttendeeList showId="show-1" currentUser={mockShow.currentUser} />);
  ...
  describe('ShowPageContent', () => {
    ...
    it('handles ongoing show status correctly', () => {
      ...
      expect(document.querySelector('[data-testid="live-indicator"]')).toBeInTheDocument();
    });
  });
