...
const mockSpotifyAlbums = [
  {
    ...
    images: [{ url: 'https://i.scdn.co/image/album1.jpg', height: 640, width: 640 }]
  },
  {
    id: 'album2', 
    name: 'folklore',
    ...
    images: [{ url: 'https://i.scdn.co/image/album2.jpg', height: 640, width: 640 }]
  }
];
...
const mockAudioFeatures = [
  ...
  { id: 'track2', liveness: 0.15, energy: 0.75, danceability: 0.65, valence: 0.85 },
  { id: 'track3', liveness: 0.05, energy: 0.3, danceability: 0.4, valence: 0.2 }
];
...
describe('API Integration Tests', () => {
  ...
  afterEach(async () => {
    // Clean up test data
    if (testArtistId) {
      await db.delete(artistSongs).where(eq(artistSongs.artistId, testArtistId));
  ...
  describe('Ticketmaster Integration', () => {
    ...
    it('should handle paginated event fetching correctly', async () => {
      ...
      for await (const eventBatch of iterateEventsByAttraction('tm-attraction-1')) {
  ...
  describe('Spotify Integration', () => {
    beforeEach(() => {
      ...
      vi.mock('../../lib/services/adapters/SpotifyClient', () => ({
        ...
        listAlbumTracks: vi.fn().mockImplementation(async (albumId: string) => {
          return mockSpotifyTracks[albumId as keyof typeof mockSpotifyTracks] || [];
        }),
        ...
        getAudioFeatures: vi.fn().mockImplementation(async (trackIds: string[]) => {
          return mockAudioFeatures.filter(feature => trackIds.includes(feature.id));
        })
      }));
    });
    ...
    it('should handle Spotify batch size limits', async () => {
      ...
      vi.mocked(getTracksDetails).mockImplementation(async (trackIds: string[]) => {
      ...
      vi.mocked(getAudioFeatures).mockImplementation(async (trackIds: string[]) => {
        // Verify batch size limits are respected
        expect(trackIds.length).toBeLessThanOrEqual(100);
        return largeMockFeatures.filter(feature => trackIds.includes(feature.id));
      });
  ...
  describe('Error Handling and Recovery', () => {
    it('should handle partial API failures gracefully', async () => {
      ...
      const apiError = result.errors.find(e => e.type === 'album_tracks_fetch');
    ...
    it('should maintain data consistency during failures', async () => {
      ...
      vi.mocked(iterateEventsByAttraction).mockImplementation(async function* () {
