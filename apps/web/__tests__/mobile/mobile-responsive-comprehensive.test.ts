...
const CRITICAL_DEVICES = [
  ...
  { name: 'iPhone SE', viewport: { width: 375, height: 667 }, category: 'phone' },
  { name: 'iPhone 15', viewport: { width: 393, height: 852 }, category: 'phone' },
  { name: 'iPhone 15 Pro Max', viewport: { width: 430, height: 932 }, category: 'phone' },
  { name: 'Samsung S24', viewport: { width: 384, height: 854 }, category: 'phone' },
  { name: 'Pixel 8', viewport: { width: 412, height: 915 }, category: 'phone' },
  { name: 'Small Android', viewport: { width: 320, height: 568 }, category: 'phone' },
  
  // TABLET DEVICES (Portrait & Landscape)
  { name: 'iPad Mini', viewport: { width: 768, height: 1024 }, category: 'tablet' },
  { name: 'iPad Air', viewport: { width: 820, height: 1180 }, category: 'tablet' },
  { name: 'iPad Pro 11"', viewport: { width: 834, height: 1194 }, category: 'tablet' },
  { name: 'iPad Pro 12.9"', viewport: { width: 1024, height: 1366 }, category: 'tablet' },
  
  // EDGE CASES
  { name: 'Very Small', viewport: { width: 320, height: 480 }, category: 'edge' },
  { name: 'Ultra Wide Phone', viewport: { width: 428, height: 926 }, category: 'edge' },
];

// LANDSCAPE variants
const LANDSCAPE_DEVICES = CRITICAL_DEVICES.filter(d => d.category === 'phone').map(device => ({
  ...device,
...
test.describe('Mobile Responsive - Core Functionality', () => {
  ALL_DEVICES.forEach((device) => {
    test(`${device.name} (${device.viewport.width}x${device.viewport.height}) - Core Mobile Experience`, async ({ page }) => {
...
test.describe('Mobile Responsive - Voting System', () => {
  test('Mobile Voting Experience - Touch Targets & Animations', async ({ page }) => {
...
test.describe('Mobile Responsive - Edge Cases', () => {
  test('Very Small Screen (320px) - Critical Functionality', async ({ page }) => {
...
test.describe('Mobile Responsive - Breakpoint Testing', () => {
  Object.entries(RESPONSIVE_BREAKPOINTS).forEach(([name, width]) => {
    test(`Breakpoint ${name} (${width}px) - Layout Integrity`, async ({ page }) => {
