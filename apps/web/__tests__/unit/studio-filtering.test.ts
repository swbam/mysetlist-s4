...
describe('Studio-Only Filtering - Core Functions', () => {
  ...
  describe('Liveness Threshold Logic', () => {
    ...
    it('should filter tracks above liveness threshold', () => {
      ...
      const studioTracks = mockTracks.filter(track => track.liveness <= LIVENESS_THRESHOLD);
  ...
  describe('ISRC Deduplication Logic', () => {
    it('should deduplicate tracks with same ISRC by popularity', () => {
      const tracksWithSameISRC = [
        { id: '1', name: 'Song (Original)', isrc: 'USUM71703861', popularity: 85 },
        { id: '2', name: 'Song (Remaster)', isrc: 'USUM71703861', popularity: 90 },
        { id: '3', name: 'Song (Deluxe)', isrc: 'USUM71703861', popularity: 88 }
      ];
    ...
    it('should handle fallback matching for tracks without ISRC', () => {
      ...
      expect(deduplicatedTracks.some(t => t.name === 'Different Song')).toBe(true);
    });
  });
  ...
  describe('Integration: Complete Studio Filtering Pipeline', () => {
    it('should correctly filter a mixed catalog to studio-only', () => {
      const mixedCatalog = [
        // Studio albums
        {
          ...
          tracks: [
            { id: '1', name: 'Shake It Off', liveness: 0.1, isrc: 'ISRC1', popularity: 95 },
            { id: '2', name: 'Blank Space', liveness: 0.15, isrc: 'ISRC2', popularity: 88 }
          ]
        },
        // Live album (should be filtered out)
        {
          ...
          tracks: [
            { id: '3', name: 'Love Story (Live)', liveness: 0.95, isrc: 'ISRC3', popularity: 82 }
          ]
        },
        // Studio album with mixed tracks
        {
          ...
          tracks: [
            { id: '4', name: 'cardigan', liveness: 0.05, isrc: 'ISRC4', popularity: 90 },
            { id: '5', name: 'the 1 (acoustic version)', liveness: 0.25, isrc: 'ISRC5', popularity: 78 },
            { id: '6', name: 'august (Live at Studio)', liveness: 0.85, isrc: 'ISRC6', popularity: 85 }
          ]
        }
      ];
    ...
    it('should maintain idempotency when run multiple times', () => {
      const catalog = [
        { id: '1', name: 'Studio Track', liveness: 0.1, isrc: 'ISRC1', popularity: 95 }
      ];

      // Run filtering pipeline multiple times
      const result1 = catalog.filter(t => 
        !isLikelyLiveTitle(t.name) && t.liveness <= 0.8
      );
      const result2 = result1.filter(t => 
        !isLikelyLiveTitle(t.name) && t.liveness <= 0.8
      );
      const result3 = result2.filter(t => 
        !isLikelyLiveTitle(t.name) && t.liveness <= 0.8
      );
