...
describe("Quality Validation Tests", () => {
  ...
  describe("GROK.md Quality Bar 1: Idempotency", () => {
    it("should produce identical results when import is re-run", async () => {
      ...
      // First import
      const identity = await orchestrator.initiateImport(
        mockComprehensiveData.artist.tmAttractionId,
      );
    ...
    it("should handle partial re-runs without creating duplicates", async () => {
      ...
      expect(countsAfterCatalogRerun.artistSongs).toBe(
        countsAfterCatalog.artistSongs,
      );
    });
  });
  ...
  describe("GROK.md Quality Bar 2: TM Completeness", () => {
    it("should ingest all Ticketmaster pages when pagination exists", async () => {
      ...
      const largeEventSet = Array.from({ length: 450 }, (_, i) => ({
        ...
        dates: {
          start: {
            localDate: `2024-${String((i % 12) + 1).padStart(2, "0")}-01`,
          },
        },
        _embedded: {
          venues: [
  ...
  describe("GROK.md Quality Bar 3: Catalog Purity (0 Live Tracks)", () => {
    ...
    it("should filter tracks with high liveness audio features", async () => {
      ...
      vi.mocked(
        require("../../lib/services/adapters/SpotifyClient").getAudioFeatures,
      ).mockResolvedValue(
        testTracks.map((t) => ({ id: t.id, liveness: t.liveness })),
      );

      vi.mocked(
        require("../../lib/services/adapters/SpotifyClient").getTracksDetails,
      ).mockResolvedValue(
        testTracks.map((t) => ({
          id: t.id,
          name: t.name,
      ...
      expect(result.liveFeaturesFiltered + result.studioTracksIngested).toBe(
        testTracks.length,
      );
    });
  });
  ...
  describe("GROK.md Quality Bar 4: ISRC Deduplication", () => {
    it("should keep only one row per ISRC with highest popularity", async () => {
      ...
      // Get all songs for this artist
      const artistSongsResult = await db
        .select({
      ...
      artistSongsResult.forEach(({ song }) => {
        if (song.isrc) {
      ...
      const totalSongs = artistSongsResult.length;
      const uniqueISRCs = new Set(
        artistSongsResult.map(({ song }) => song.isrc).filter(Boolean),
      );

      // All songs with ISRCs should have unique ISRCs
      const songsWithISRC = artistSongsResult.filter(({ song }) => song.isrc);
    ...
    it("should handle fallback deduplication for tracks without ISRC", async () => {
      ...
      vi.mocked(
        ...
        tracksWithoutISRC.map((t) => ({ id: t.id, liveness: 0.1 })),
      );
  ...
  describe("GROK.md Quality Bar 5: Progress SSE Timing", () => {
    it("should emit SSE events within 200ms of phase changes", async () => {
      ...
      for (const transition of phaseTransitions) {
        const fromEvents = progressEvents.filter(
          (e) => e.stage === transition.from,
        );
        const toEvents = progressEvents.filter(
          (e) => e.stage === transition.to,
        );
  ...
  async function getDatabaseCounts(artistId: string) {
    ...
    const [artistSongCount] = await db
      .select({ count: count() })
      .from(artistSongs)
  ...
  async function getSongsState(artistId: string) {
    ...
    return artistSongsResult.map(({ artistSong, song }) => ({
      artistSong: {
        ...artistSong,
