...
import {
  PerformanceMonitor,
  SLO_TARGETS,
} from "../../lib/services/monitoring/PerformanceMonitor";
// Import services
...
const generateArtistData = (scenario: ArtistTestScenario) => {
  ...
  for (let i = 0; i < scenario.expectedAlbums; i++) {
    ...
    albums.push({
      id: albumId,
      name: isLiveAlbum
        ? `Live at ${getCityName(i, scenario.type)}`
        : `Studio Album ${i + 1}`,
      album_type: "album",
    ...
    for (
      let j = 0;
      j < tracksPerAlbum && trackCounter < scenario.expectedTracks;
      j++, trackCounter++
    ) {
      ...
      const track = {
        ...
        external_ids: {
          isrc: `${scenario.type.toUpperCase()}${String(trackCounter).padStart(7, "0")}`,
        },
        ...
        preview_url:
          Math.random() > 0.3 ? `https://preview.spotify.com/${trackId}` : null,
...
const getVenueName = (index: number, type: string) => {
  ...
  return venues[type as keyof typeof venues][
    index % venues[type as keyof typeof venues].length
  ];
};
...
const getCityName = (index: number, type: string) => {
  ...
  return cities[type as keyof typeof cities][
    index % cities[type as keyof typeof cities].length
  ];
};
...
const getStateName = (index: number, type: string) => {
  ...
  return states[type as keyof typeof states][
    index % states[type as keyof typeof states].length
  ];
};
...
describe("Acceptance Tests - Realistic Artist Scenarios", () => {
  describe.each(ARTIST_SCENARIOS)("$name ($type artist)", (scenario) => {
    ...
    beforeEach(async () => {
      ...
      performanceMonitor = PerformanceMonitor.create(
        `acceptance-${scenario.type}`,
        scenario.tmAttractionId,
      );
    });
          ...
          .mockImplementation(async function* () {
            ...
            for (let i = 0; i < shows.length; i += pageSize) {
              ...
              await new Promise((resolve) =>
                setTimeout(resolve, getAPIDelay(scenario.type)),
              );
            }
          }),
      }));
      ...
      vi.mock("../../lib/services/adapters/SpotifyClient", () => ({
        ...
        getAudioFeatures: vi
          ...
          .mockImplementation(async (trackIds: string[]) => {
            ...
            return audioFeatures.filter((feature: any) =>
    ...
    it("should complete import within performance SLOs", async () => {
      ...
      const identityResult = await orchestrator.initiateImport(
        scenario.tmAttractionId,
      );
      ...
      expect(identityDuration).toBeLessThan(
        SLO_TARGETS.IDENTITY_PHASE.threshold,
      );

      // Adjust SLO expectations based on data volume
      const showsExpectedTime = Math.min(
        scenario.expectedShows * 30,
        SLO_TARGETS.SHOWS_PHASE.threshold,
      );
      const catalogExpectedTime = Math.min(
        scenario.expectedTracks * 20,
        SLO_TARGETS.CATALOG_PHASE.threshold,
      );
    ...
    it("should maintain studio-only catalog quality", async () => {
      ...
      const liveTracks = artistTracks.filter(
        ({ song }) => song.isLive === true,
      );
  ...
  describe("Cross-Artist Validation", () => {
    it("should handle imports of different artist types concurrently", async () => {
      ...
      const importPromises = orchestrators.map(
        async ({ orchestrator, scenario }) => {
          const artistData = generateArtistData(scenario);
          ...
          const importResult = await orchestrator.runFullImport(
            identityResult.artistId,
          );
      ...
      console.log(
        `Concurrent Import Results: ${successful.length}/${ARTIST_SCENARIOS.length} successful`,
      );
    });
    ...
    const setupConcurrentMocks = (scenario: ArtistTestScenario, data: any) => {
      ...
      vi.mocked(
        require("../../lib/services/adapters/TicketmasterClient")
      ...
      vi.mocked(
        require("../../lib/services/adapters/SpotifyClient").listAllAlbums,
