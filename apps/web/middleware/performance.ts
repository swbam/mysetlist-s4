...
export function performanceMiddleware(request: NextRequest) {
  ...
  response.headers.set('Server-Timing', `region;desc="iad1", environment;desc="production"`);
  response.headers.set('X-Vercel-Cache', request.headers.get('x-vercel-cache') || 'MISS');
  
  // Enable HTTP/2 Server Push for critical resources
  if (pathname === '/' || pathname.startsWith('/artists/')) {
    response.headers.append('Link', '</fonts/inter-var.woff2>; rel=preload; as=font; crossorigin');
