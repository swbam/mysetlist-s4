...
const checkIPRateLimit = (ip: string, limit: number = 1000, windowMs: number = 15 * 60 * 1000): boolean => {
  const now = Date.now();
...
export async function securityMiddleware(request: NextRequest): Promise<NextResponse | null> {
  ...
  if (detectSuspiciousActivity(request)) {
    console.warn(`Suspicious activity detected from IP: ${ip}, URL: ${request.url}`);
  ...
  if (pathname.startsWith('/api/')) {
    const rateLimitKey = Object.keys(rateLimitConfig).find(key => 
...
export const validateInput = (input: string, schema: keyof typeof validationSchemas): boolean => {
