...
async function runPerformanceAudit() {
  ...
  for (const testPage of pages) {
    ...
    try {
      ...
      const metrics = await page.evaluate(() => {
        return new Promise(resolve => {
            ...
            list.getEntries().forEach((entry) => {
              switch (entry.entryType) {
                ...
                case 'largest-contentful-paint':
                  ...
                  break;
                case 'first-input':
                  data.coreWebVitals.FID = entry.processingStart - entry.startTime;
                  break;
                case 'layout-shift':
                  if (!entry.hadRecentInput) {
                    data.coreWebVitals.CLS = (data.coreWebVitals.CLS || 0) + entry.value;
                  }
                  break;
              }
            });
          ...
          data.customMetrics = {
            ...
            // Bundle analysis
            scriptSizes: Array.from(document.querySelectorAll('script[src]')).map(script => ({
              src: script.src,
              async: script.async,
              defer: script.defer
            }))
      ...
      const resourceAnalysis = {
        ...
        javascriptSize: metrics.resources.filter(r => r.type === 'script').reduce((sum, r) => sum + r.size, 0),
        cssSize: metrics.resources.filter(r => r.type === 'stylesheet').reduce((sum, r) => sum + r.size, 0),
        imageSize: metrics.resources.filter(r => r.type === 'img').reduce((sum, r) => sum + r.size, 0),
      ...
      const scores = {
        FCP: metrics.coreWebVitals.FCP ? (
          metrics.coreWebVitals.FCP < 1800 ? 100 : 
          metrics.coreWebVitals.FCP < 3000 ? 50 : 0
        ) : null,
        LCP: metrics.coreWebVitals.LCP ? (
          metrics.coreWebVitals.LCP < 2500 ? 100 : 
          metrics.coreWebVitals.LCP < 4000 ? 50 : 0
        ) : null,
        FID: metrics.coreWebVitals.FID ? (
          metrics.coreWebVitals.FID < 100 ? 100 : 
          metrics.coreWebVitals.FID < 300 ? 50 : 0
        ) : null,
        CLS: metrics.coreWebVitals.CLS ? (
          metrics.coreWebVitals.CLS < 0.1 ? 100 : 
          metrics.coreWebVitals.CLS < 0.25 ? 50 : 0
        ) : null,
        TTFB: metrics.navigationTiming.ttfb ? (
          metrics.navigationTiming.ttfb < 800 ? 100 : 
          metrics.navigationTiming.ttfb < 1800 ? 50 : 0
        ) : null,
        bundleSize: resourceAnalysis.totalSize < 1000000 ? 100 : // < 1MB
                   resourceAnalysis.totalSize < 2000000 ? 50 : 0   // < 2MB
      };
      ...
      // Log key metrics
      const statusIcon = overallScore >= 80 ? '🚀' : overallScore >= 60 ? '⚡' : overallScore >= 40 ? '⚠️' : '🐌';
      console.log(`  ${statusIcon} Overall Score: ${overallScore}%`);
      console.log(`  📊 Core Web Vitals:`);
      console.log(`     FCP: ${metrics.coreWebVitals.FCP?.toFixed(0) || 'N/A'}ms`);
      console.log(`     LCP: ${metrics.coreWebVitals.LCP?.toFixed(0) || 'N/A'}ms`);
      console.log(`     FID: ${metrics.coreWebVitals.FID?.toFixed(0) || 'N/A'}ms`);
      console.log(`     CLS: ${metrics.coreWebVitals.CLS?.toFixed(3) || 'N/A'}`);
      console.log(`  📦 Bundle: ${(resourceAnalysis.totalSize / 1024).toFixed(0)}KB total`);
  ...
  successfulResults
    ...
    .forEach((result, idx) => {
      ...
      console.log(`  ${medal} ${result.page}: ${result.overallScore}% (${result.status})`);
    });
  ...
  if (successfulResults.length > 0) {
    ...
    const avgMetrics = {
      FCP: successfulResults.filter(r => r.coreWebVitals.FCP).map(r => r.coreWebVitals.FCP),
      LCP: successfulResults.filter(r => r.coreWebVitals.LCP).map(r => r.coreWebVitals.LCP),
      bundleSize: successfulResults.map(r => r.resourceAnalysis.totalSize)
    };

    if (avgMetrics.FCP.length > 0) {
      const avgFCP = avgMetrics.FCP.reduce((a, b) => a + b, 0) / avgMetrics.FCP.length;
      console.log(`  • Average First Contentful Paint: ${avgFCP.toFixed(0)}ms ${avgFCP < 1800 ? '✅' : '❌'}`);
    }
    
    if (avgMetrics.LCP.length > 0) {
      const avgLCP = avgMetrics.LCP.reduce((a, b) => a + b, 0) / avgMetrics.LCP.length;
      console.log(`  • Average Largest Contentful Paint: ${avgLCP.toFixed(0)}ms ${avgLCP < 2500 ? '✅' : '❌'}`);
    }
    
    const avgBundleSize = avgMetrics.bundleSize.reduce((a, b) => a + b, 0) / avgMetrics.bundleSize.length;
    console.log(`  • Average Bundle Size: ${(avgBundleSize / 1024).toFixed(0)}KB ${avgBundleSize < 1000000 ? '✅' : '❌'}`);
    ...
    successfulResults.forEach(result => {
      ...
      if (result.resourceAnalysis.totalSize > 1000000) issues.push('Large bundle size');
      if (result.resourceAnalysis.slowResources.length > 0) issues.push('Slow resources');
    });
