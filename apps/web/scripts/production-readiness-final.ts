  ...
  try {
    ...
    if (consoleCount > 0) {
      const fileList = runCommand(
        `find . -name "*.tsx" -o -name "*.ts" | grep -E "(app/|components/|lib/|hooks/)" | grep -v test | grep -v spec | xargs grep -l "console\\." | head -5`
      )
  ...
  try {
    ...
    for (const pattern of sensitivePatterns) {
      const result = runCommand(
        `grep -r "${pattern}" app/ components/ lib/ hooks/ 2>/dev/null || true`
      )
      if (result.trim()) {
        exposedKeys++
      }
...
function checkImageOptimization(): void {
  ...
  try {
    const output = runCommand(
      "tsx scripts/image-optimization-audit.ts 2>/dev/null"
    )
...
function checkNextJsConfig(): void {
  ...
  if (existsSync(nextConfigPath)) {
    ...
    const missingFeatures = productionFeatures.filter(
...
function checkSEO(): void {
  ...
  if (existsSync(layoutPath)) {
    ...
    const seoFeatures = ["metadataBase", "openGraph", "twitter", "robots"]

    const missingFeatures = seoFeatures.filter(
      (feature) => !content.includes(feature)
    )
...
async function main() {
  ...
  console.log(
    `   ❌ Failed: ${failCount} ${criticalFails > 0 ? `(${criticalFails} critical)` : ""}`
  )
  ...
  if (criticalFails > 0) {
    ...
    console.log(
      "   ⚠️  NEEDS ATTENTION - Non-critical issues should be addressed"
    )
  } else if (warningCount > 0) {
    console.log(
      "   ✅ READY WITH WARNINGS - Production ready but optimizations recommended"
    )
  } else {
  ...
  if (criticalFails > 0 || failCount > 0 || warningCount > 0) {
    ...
    const nonCriticalFails = checks.filter(
      (c) => c.status === "fail" && !c.critical
    )
    ...
    if (
      checks.some(
        (c) => c.name === "Console Statements" && c.status === "warning"
      )
    ) {
      console.log("   npm run tsx scripts/production-console-cleanup.ts")
    }
    if (
      checks.some((c) => c.name === "Image Optimization" && c.status !== "pass")
    ) {
