...
class SecurityAuditor {
  ...
  private async checkDependencyVulnerabilities(): Promise<void> {
    ...
    try {
      ...
      const criticalPackages = ['next', 'react', 'react-dom', '@supabase/supabase-js'];
  ...
  private async checkHardcodedSecrets(): Promise<void> {
    ...
    const secretPatterns = [
      ...
      { pattern: /xoxp-[0-9]{11}-[0-9]{11}-[0-9a-zA-Z]{24}/, type: 'Slack User Token' },
      { pattern: /password\\s*=\\s*[\"'][^\"'\\s]+[\"']/, type: 'Hardcoded Password' },
      { pattern: /secret\\s*=\\s*[\"'][^\"'\\s]+[\"']/, type: 'Hardcoded Secret' },
      { pattern: /api_key\\s*=\\s*[\"'][^\"'\\s]+[\"']/, type: 'Hardcoded API Key' },
    ];
          ...
          if (existsSync(file)) {
            ...
            if (content.includes('session') && !content.includes('secure')) {
              this.findings.push({
                ...
                recommendation: 'Ensure session cookies are secure and httpOnly',
                file,
              });
            }
  ...
  private async checkInputValidation(): Promise<void> {
    ...
    apiFiles.forEach(file => {
      if (existsSync(file)) {
        ...
        if (content.includes('request.json()') || content.includes('request.formData()')) {
        ...
        if (dbPasswordMatch) {
          ...
          if (password && (password.length < 12 || !/[A-Z]/.test(password) || !/[0-9]/.test(password))) {
            this.findings.push({
              ...
              description: 'Database password does not meet security requirements',
              recommendation: 'Use strong password with mixed case, numbers, and symbols',
              file: envFile,
            });
          }
        }
  ...
  private async checkDatabaseSecurity(): Promise<void> {
    ...
    const dbFiles = execSync('find . -name "*.ts" -exec grep -l "createClient\\|database" {} \\;', { 
      stdio: 'pipe',
      encoding: 'utf8'
    }).split('\n').filter(Boolean);
    ...
    dbFiles.forEach(file => {
      if (existsSync(file)) {
        ...
        if (content.includes('database') && !content.includes('ssl') && !content.includes('tls')) {
          this.findings.push({
            severity: 'medium',
            category: 'Database',
        ...
        if (content.includes('createClient') && !content.includes('pool')) {
          this.findings.push({
            ...
            recommendation: 'Configure connection pooling for better performance and security',
            file,
          });
        }
      }
    ...
    if (existsSync(nextConfig)) {
      ...
      if (!content.includes('Content-Security-Policy') && !content.includes('csp')) {
        this.findings.push({
          ...
          description: 'No Content Security Policy found in Next.js configuration',
  ...
  private async checkSessionSecurity(): Promise<void> {
    ...
    const authFiles = execSync('find . -name "*.ts" -exec grep -l "session\\|cookie" {} \\;', { 
      stdio: 'pipe',
      encoding: 'utf8'
    }).split('\n').filter(Boolean);
    ...
    authFiles.forEach(file => {
      if (existsSync(file)) {
        ...
        if (content.includes('session') && !content.includes('timeout') && !content.includes('expire')) {
          this.findings.push({
            severity: 'medium',
            category: 'Session Security',
  ...
  private async checkDataEncryption(): Promise<void> {
    ...
    const dataFiles = execSync('find . -name "*.ts" -exec grep -l "encrypt\\|decrypt\\|crypto" {} \\;', { 
      stdio: 'pipe',
      encoding: 'utf8'
    }).split('\n').filter(Boolean);
      ...
      if (existsSync(file)) {
        ...
        if (content.includes('md5') || content.includes('sha1')) {
          this.findings.push({
            ...
            recommendation: 'Use stronger encryption algorithms (AES-256, SHA-256)',
            file,
          });
        }
  ...
  private async checkLoggingSecurity(): Promise<void> {
    ...
    const logFiles = execSync('find . -name "*.ts" -exec grep -l "console.log\\|logger\\|log" {} \\;', { 
      stdio: 'pipe',
      encoding: 'utf8'
    }).split('\n').filter(Boolean);
  ...
  private async checkThirdPartyIntegrations(): Promise<void> {
    ...
    if (existsSync(packageJson)) {
      ...
      const highRiskPackages = Object.keys(dependencies).filter(pkg => 
        pkg.includes('fs') || pkg.includes('exec') || pkg.includes('child_process')
      );
  ...
  private generateSecurityReport(): void {
    ...
    // Generate score
    const score = Math.max(0, 100 - (report.summary.critical * 20 + report.summary.high * 10 + report.summary.medium * 5 + report.summary.low * 1));
  ...
  private generateRecommendations(): string[] {
    ...
    if (this.findings.some(f => f.category === 'Authentication')) {
      recommendations.push('Implement proper authentication and session management');
    }
