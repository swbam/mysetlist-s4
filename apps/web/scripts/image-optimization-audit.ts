...
function extractImageAttributes(imgTag: string): Record<string, string | undefined> {
  ...
  // Extract priority
  const priorityMatch = imgTag.match(/priority\s*(?:=\s*(?:true|['"]true['"]))?/i);
...
function isStaticImageImport(src: string): boolean {
  return src.startsWith('/') || src.startsWith('./') || src.startsWith('../') || !src.includes('://');
}
...
function checkImageOptimization(filePath: string): ImageIssue[] {
  ...
  const hasLucideImageImport = /import\s+.*\{\s*[^}]*Image[^}]*\}\s*from\s+['"]lucide-react['"]/.test(content);
  ...
  lines.forEach((line, index) => {
        ...
        if (attrs.src) {
          ...
          if (!attrs.alt || attrs.alt.trim() === '') {
            issues.push({
              ...
              severity: 'error',
              suggestion: 'Add alt text for accessibility: alt="Description of the image"'
            });
          }
          ...
          if (isStaticImageImport(attrs.src)) {
            if (!attrs.width || !attrs.height) {
              issues.push({
                ...
                severity: 'warning',
                suggestion: 'Add width and height attributes for better performance: width={500} height={300}'
              });
            }
          }
          ...
          if (attrs.src.includes('://') && !attrs.sizes && !attrs.width) {
            issues.push({
              ...
              severity: 'info',
              suggestion: 'Add sizes attribute for responsive images: sizes="(max-width: 768px) 100vw, 50vw"'
            });
          }
          ...
          if (
            ...
            if (!attrs.priority) {
              issues.push({
                ...
                severity: 'info',
                suggestion: 'Add priority attribute for LCP optimization: priority={true}'
              });
            }
          }
        }
    ...
    const hardcodedImageUrls = line.match(/['"]https?:\/\/[^'"]*\.(jpg|jpeg|png|gif|webp)['"]?/gi);
    if (hardcodedImageUrls) {
      hardcodedImageUrls.forEach(url => {
        // Skip known CDNs that are already optimized
        if (!url.includes('scdn.co') && !url.includes('ticketm.net') && !url.includes('unsplash.com')) {
          issues.push({
            filePath: relativePath,
            lineNumber,
            line: trimmedLine,
            ...
            severity: 'info',
            suggestion: 'Consider using Next.js Image component with this URL for optimization'
          });
        }
      });
    }
  ...
  if (issues.some(i => i.issue.includes('HTML img tag')) && !hasNextImageImport) {
    issues.push({
      filePath: relativePath,
      lineNumber: 1,
      line: 'File header',
...
async function main() {
  ...
  if (errorCount > 0) {
    console.log(`   1. Fix ${errorCount} critical image issues (missing alt text, HTML img tags)`);
  }
  
  const htmlImgCount = allIssues.filter(i => i.issue.includes('HTML img tag')).length;
  if (htmlImgCount > 0) {
    console.log(`   2. Replace ${htmlImgCount} HTML img tags with Next.js Image components`);
  }
  
  const altTextIssues = allIssues.filter(i => i.issue.includes('alt text')).length;
  if (altTextIssues > 0) {
    console.log(`   3. Add alt text to ${altTextIssues} images for accessibility`);
  }
  
  const dimensionIssues = allIssues.filter(i => i.issue.includes('width/height')).length;
  if (dimensionIssues > 0) {
    console.log(`   4. Add dimensions to ${dimensionIssues} images for better performance`);
  }
  ...
  if (totalIssues === 0) {
    ...
    console.log('   👀 https://nextjs.org/docs/app/api-reference/components/image');
  }
}
