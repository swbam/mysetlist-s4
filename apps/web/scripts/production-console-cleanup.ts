...
function processFile(filePath: string): ConsoleFix[] {
  ...
  lines.forEach((line, index) => {
    ...
    const hasConsole = Object.entries(CONSOLE_PATTERNS).some(([type, pattern]) => {
...
async function main() {
  ...
  for (const file of files) {
    ...
    if (fixes.length > 0) {
      ...
      fixes.forEach(fix => {
        ...
        if (fix.action !== 'keep') {
          console.log(`   Line ${fix.lineNumber}: ${fix.action} - ${fix.originalLine.trim()}`);
        }
      });
  ...
  console.log(`   Kept (development/debug): ${fixesByAction.keep} console statements`);
  console.log(`   Total files processed: ${files.length}`);
  console.log(`   Total fixes applied: ${totalFixes}`);

  if (totalFixes > 0) {
    console.log('\n✅ Production console cleanup completed!');
    console.log('💡 Remember to test the application to ensure no functionality was broken.');
  } else {
