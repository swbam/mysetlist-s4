...
class ImportOrchestrationTester {
  ...
  private async findAttractionId(artistName: string): Promise<string> {
    ...
    try {
      ...
      console.log(
        `✅ Found attraction: ${attraction.name} (ID: ${attraction.id})`,
      );
  ...
  private async testPhase1(
    ...
    try {
      ...
      if (phase.duration > 200) {
        console.warn(
          `⚠️  Phase 1 took ${phase.duration.toFixed(1)}ms (target: < 200ms)`,
        );
      }
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        const eventSource = new EventSource(
          `${BASE_URL}/api/artists/${artistId}/stream`,
        );
        let lastProgress = 0;
        ...
        eventSource.onmessage = (event) => {
          try {
            ...
            if (data.stage === "completed") {
              ...
              console.log(
                `✅ Full import completed in ${(phase.duration / 1000).toFixed(1)}s`,
              );
  ...
  private async validateImportResults(artistId: string): Promise<void> {
    ...
    try {
      ...
      const validations = [
        {
          check: artist.name && artist.name !== "Loading...",
          message: "Artist has real name",
        },
        {
          check: artist.slug && !artist.slug.startsWith("tm-"),
          message: "Artist has proper slug",
        },
        {
          check: artist.importStatus === "completed",
          message: "Import status is completed",
        },
        {
          check: artist.totalSongs > 0,
          message: `Has songs (${artist.totalSongs})`,
        },
      ];
  ...
  printReport(result: ImportTestResult): void {
    ...
    if (phase1?.duration) {
      ...
      console.log(
        `   Phase 1 (target < 200ms): ${phase1.duration.toFixed(1)}ms ${meetsTarget ? "✅" : "⚠️"}`,
      );
    }
