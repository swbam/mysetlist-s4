...
const DEVICE_PROFILES = {
  ...
  'Samsung Galaxy S21': { width: 384, height: 854, deviceScaleFactor: 2.75, isMobile: true },
      ...
      try {
        ...
        const tests = {
          ...
          mobileNavExists: device.isMobile ? 
            await page.isVisible('[data-testid="mobile-nav"], .mobile-nav, button[aria-label*="menu" i]') : 
            true,
            
          // Check if desktop nav is hidden on mobile
          desktopNavHidden: device.isMobile ? 
            await page.isHidden('.desktop-nav, nav.hidden\\:md\\:flex, .md\\:flex') || 
            !(await page.isVisible('.desktop-nav, nav.hidden\\:md\\:flex, .md\\:flex')) :
            true,
          ...
          textReadable: await page.evaluate(() => {
            ...
            return textElements.length === 0 || (readableCount / textElements.length >= 0.9);
          }),
        ...
        console.log(`  ${statusIcon} ${deviceName} (${device.width}x${device.height}): ${Math.round(score)}%`);
