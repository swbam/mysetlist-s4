...
export default async function AdminDashboard({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const [
    ...
    supabase.from('moderation_logs')
      .select('*, moderator:users!moderation_logs_moderator_id_fkey(display_name, email)')
      .order('created_at', { ascending: false })
      .limit(5),
    supabase.from('platform_stats')
      .select('*')
  ...
  const usersTrend = latestStats && weekAgoStats ? {
    value: calculateTrend(latestStats.total_users, weekAgoStats.total_users),
        ...
        <Card className="border-orange-200 bg-orange-50 dark:bg-orange-950/20">
          ...
          <CardContent className="space-y-2">
            {pendingReports! > 0 && (
              ...
              >
                <span className="font-medium">{pendingReports} pending reports</span>
            ...
            {pendingModeration! > 0 && (
              <Link
                ...
                className="flex items-center justify-between p-3 rounded-md bg-background hover:bg-accent transition-colors"
              >
                <span className="font-medium">{pendingModeration} items awaiting moderation</span>
                <span className="text-sm text-muted-foreground">Review now →</span>
              </Link>
            )}
          </CardContent>
                    ...
                    <div className="flex-1 space-y-1">
                      <p className="text-sm">
                        <span className="font-medium">
                          {activity.moderator?.display_name || activity.moderator?.email}
                        </span>
                        {' '}
                        {activity.action.replace('_', ' ')} {activity.target_type}
                      </p>
                      {activity.reason && (
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">Content Created Today</span>
                <span className="text-sm text-muted-foreground">
                  {((latestStats?.new_setlists ?? 0) + 
                    (latestStats?.new_reviews ?? 0) + 
                    (latestStats?.new_photos ?? 0)).toLocaleString()}
                </span>
              </div>
              <div className="flex items-center justify-between">
