...
export default function ContentActions({ type, item, locale }: ContentActionsProps) {
  ...
  const handleVerify = async () => {
    setLoading(true);
    try {
      const table = type === 'artist' ? 'artists' : 
                   type === 'venue' ? 'venues' : 'shows';
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      toast('Sync started', {
        description: 'Artist data is being synced with Spotify.',
      });
    } catch (error) {
      toast('Error', {
  ...
  const getViewUrl = () => {
    switch (type) {
      case 'artist': return `/${locale}/artists/${item.slug}`;
      case 'venue': return `/${locale}/venues/${item.slug}`;
      case 'show': return `/${locale}/shows/${item.slug}`;
    }
  };
