...
export default function UserManagementEnhanced() {
  ...
  const filteredUsers = users.filter(user => {
    ...
    const matchesStatus = 
      statusFilter === 'all' ||
      (statusFilter === 'active' && user.email_confirmed_at && !user.is_banned) ||
  ...
  const getUserStatusBadge = (user: User) => {
    ...
    return <Badge variant="default" className="text-green-600 bg-green-100">Active</Badge>;
  };
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {users.filter(u => u.warning_count && u.warning_count > 0).length} with warnings
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {users.filter(u => u.role === 'admin').length} admins, {users.filter(u => u.role === 'moderator').length} moderators
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Dialog open={showUserDialog} onOpenChange={setShowUserDialog}>
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Complete information about {selectedUser?.display_name || selectedUser?.email}
            </DialogDescription>
          </DialogHeader>
          ...
          {selectedUser && (
            <Tabs defaultValue="profile" className="w-full">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <div className="grid gap-2 text-sm">
                      ...
                      <div><strong>Display Name:</strong> {selectedUser.display_name || 'Not set'}</div>
                      <div><strong>Username:</strong> {selectedUser.username || 'Not set'}</div>
                      <div><strong>Role:</strong> {getRoleBadge(selectedUser.role)}</div>
                      <div><strong>Status:</strong> {getUserStatusBadge(selectedUser)}</div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid gap-2 text-sm">
                      <div><strong>Joined:</strong> {format(new Date(selectedUser.created_at), 'PPP')}</div>
                      <div><strong>Last Login:</strong> {selectedUser.last_login_at ? 
                        format(new Date(selectedUser.last_login_at), 'PPP') : 'Never'}</div>
                      <div><strong>Email Verified:</strong> {selectedUser.email_confirmed_at ? 
                        <CheckCircle className="inline w-4 h-4 text-green-500 ml-1" /> : 
                        <XCircle className="inline w-4 h-4 text-red-500 ml-1" />}</div>
                    </div>
                  </div>
                </div>
              </TabsContent>
                    ...
                    <Card>
                      ...
                      <CardContent className="space-y-2">
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">{userStats.setlists_created}</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Reviews Written:</span>
                          <span className="font-medium">{userStats.reviews_written}</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Votes Cast:</span>
                          <span className="font-medium">{userStats.votes_cast}</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Photos Uploaded:</span>
                          <span className="font-medium">{userStats.photos_uploaded}</span>
                        </div>
                      </CardContent>
                    </Card>
              ...
              <TabsContent value="content">
                <p className="text-muted-foreground">User-generated content will be displayed here.</p>
              </TabsContent>
                      ...
                      <CardContent className="pt-6">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium">
                            {selectedUser.warning_count} warning{selectedUser.warning_count > 1 ? 's' : ''} on record
                          </span>
                        </div>
                      </CardContent>
                    </Card>
      ...
      <Dialog open={showBanDialog} onOpenChange={setShowBanDialog}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button 
              variant="destructive"
              onClick={() => selectedUser && handleUserAction('ban', selectedUser.id, { reason: banReason })}
            >
              Ban User
            </Button>
          </DialogFooter>
