...
export default async function AnalyticsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  // Calculate trends
  const calculateTrend = (current: number, previous: number): AnalyticsData => {
    ...
    <div className="space-y-6">
            ...
            <CardContent>
              ...
              <div className="flex items-center gap-1 mt-1">
                <span className={`text-xs ${metric.isPositive ? 'text-green-500' : 'text-red-500'}`}>
                  {metric.isPositive ? '+' : ''}{metric.changePercent}%
                </span>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{totalShows?.toLocaleString() ?? 0}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{totalArtists?.toLocaleString() ?? 0}</div>
            <p className="text-xs text-muted-foreground">tracked artists</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">concert venues</p>
          </CardContent>
        </Card>
      </div>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Most popular artists on the platform</CardDescription>
              </CardHeader>
              <CardContent>
              ...
              <CardContent>
                <div className="space-y-4">
                  {topVenues?.slice(0, 8).map((venue, index) => {
                    const avgRating = venue.venue_reviews?.length > 0 
                      ? venue.venue_reviews.reduce((sum: number, review: any) => sum + review.rating, 0) / venue.venue_reviews.length 
                      : 0;
        ...
        <TabsContent value="users" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Latest actions taken by users on the platform</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                        ...
                        <div className="flex items-center gap-2">
                          <div className="h-6 w-6 rounded-full bg-muted flex items-center justify-center">
                            <span className="text-xs">
                              {activity.user?.display_name?.[0] || activity.user?.email?.[0] || '?'}
                            </span>
                          </div>
                          <span className="text-sm">
                            {activity.user?.display_name || activity.user?.email || 'Anonymous'}
                          </span>
                        </div>
                      </TableCell>
                      <TableCell>
                  ...
                  {platformStats?.slice(0, 7).map((stat, index) => (
                    <div key={stat.id} className="flex items-center justify-between">
                      ...
                      <div className="text-right text-sm">
                        ...
                        <p className="text-muted-foreground">
                          {stat.new_reviews || 0} reviews, {stat.new_photos || 0} photos
                        </p>
                      </div>
                    </div>
                  ))}
