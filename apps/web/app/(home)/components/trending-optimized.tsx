...
import { type TrendingItem, getWeeklyTrending } from "~/lib/trending-client";
      ...
      <div className="flex-1">
        ...
        {isShow && item.show_date && (
          <p className="text-gray-600 text-sm dark:text-gray-400">
            ...
            <TrendingDateDisplay date={item.show_date} />
          </p>
        )}
        {item.venue_name && (
          <p className="text-gray-600 text-sm dark:text-gray-400">
...
// Component to handle date formatting client-side
function TrendingDateDisplay({ date }: { date: string }) {
...
export function Trending() {
  ...
  useEffect(() => {
    const fetchTrending = async () => {
      try {
        const data = await getWeeklyTrending(10);
        setTrending(data);
      } catch (error) {
    ...
    return (
      <section className="py-12 bg-secondary/20">
        <div className="container mx-auto px-4">
          <h2 className="text-2xl sm:text-3xl font-bold mb-8">Trending This Week</h2>
  ...
  const shows = trending.filter(item => item.type === "show");
  const artists = trending.filter(item => item.type === "artist");
