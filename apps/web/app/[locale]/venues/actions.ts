...
import { venues, shows, venueReviews } from '@repo/database/src/schema';
import { and, eq, or, ilike, sql, gte, lte, inArray } from 'drizzle-orm';
...
export const getVenues = unstable_cache(
  async ({ search, types, capacity, userLat, userLng, limit = 50 }: GetVenuesParams) => {
    const conditions = [];
    ...
    if (capacity && capacity !== 'all') {
      switch (capacity) {
        case 'small':
          ...
          break;
        case 'medium':
          conditions.push(and(gte(venues.capacity, 1000), lte(venues.capacity, 5000)));
          break;
        case 'large':
          conditions.push(and(gte(venues.capacity, 5000), lte(venues.capacity, 20000)));
          break;
        case 'xlarge':
    ...
    // Apply sorting
    const queryWithSort = userLat && userLng
