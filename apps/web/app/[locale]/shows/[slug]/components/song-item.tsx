...
export function SongItem({ item, index, isEditing, canVote, onDelete }: SongItemProps) {
  ...
  const handleVote = (voteType: 'up' | 'down') => {
    ...
    startTransition(async () => {
      try {
        const result = await voteSong(item.id, voteType);
        ...
        if (result.removed) {
          ...
          setVotes(prev => ({
            ...
            netVotes: voteType === 'up' ? prev.netVotes - 1 : prev.netVotes + 1
          }));
          ...
          setVotes(prev => ({
            ...
            netVotes: voteType === 'up' ? prev.netVotes + 2 : prev.netVotes - 2
          }));
        } else {
          // New vote
          setUserVote(voteType);
        ...
        {canVote && !isEditing && (
          <div className="flex items-center gap-1">
            ...
            <span className={cn(
              ...
              votes.netVotes > 0 && "text-green-600",
              votes.netVotes < 0 && "text-red-600"
            )}>
