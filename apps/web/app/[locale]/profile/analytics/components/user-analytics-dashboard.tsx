...
export function UserAnalyticsDashboard({
  ...
  const musicTaste = useMemo(() => {
    ...
    attendedShows.forEach(show => {
      // Count genres
      const genres = show.shows?.artists?.genres || [];
      genres.forEach((genre: string) => {
        genreCount[genre] = (genreCount[genre] || 0) + 1;
      });
      
      // Count venues
      const venueName = show.shows?.venues?.name;
      if (venueName) {
        venueCount[venueName] = (venueCount[venueName] || 0) + 1;
      }
      
      // Count shows by month
      const month = format(parseISO(show.created_at), 'yyyy-MM');
    ...
    return { topGenres, topVenues, monthlyShows };
  }, [attendedShows]);
    ...
    <div className="space-y-8">
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              Member since {format(parseISO(stats.memberSince), 'MMMM yyyy')}
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <Tabs defaultValue="timeline" className="space-y-4">
        ...
        <TabsContent value="taste" className="space-y-4">
          <MusicTasteAnalysis
            topGenres={musicTaste.topGenres}
              ...
              <CardContent>
                <dl className="space-y-2">
                  ...
                  <div className="flex justify-between">
                    ...
                    <dd className="text-sm font-medium">
                      {new Set(attendedShows.map(s => s.shows?.venues?.id)).size}
                    </dd>
                  </div>
                </dl>
              </CardContent>
