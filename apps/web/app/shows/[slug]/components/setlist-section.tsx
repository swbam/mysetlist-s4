...
export function SetlistSection({ 
  ...
  const [setlistType, setSetlistType] = useState<'predicted' | 'actual'>('predicted');
  ...
  return (
    <div className="space-y-6">
      ...
      {hasAnySetlists ? (
        <Tabs defaultValue={hasActualSetlists ? "actual" : "predicted"} className="w-full">
          <TabsList className="grid w-full grid-cols-2">
            ...
            <TabsTrigger value="predicted" disabled={!hasPredictedSetlists}>
              Predicted {hasPredictedSetlists && `(${predictedSetlists.length})`}
            </TabsTrigger>
          </TabsList>
