    ...
    if (filterType !== 'all') {
      filtered = filtered.filter(activity => {
        switch (filterType) {
          ...
          case 'shows':
            return activity.type === 'show_attend' || activity.type === 'show_plan';
          case 'setlists':
        ...
        {stats && (
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
            ...
            <Card>
              <CardContent className="pt-4">
                <div className="text-center">
                  <div className="text-2xl font-bold">{stats.artistsFollowed}</div>
                  <div className="text-xs text-muted-foreground">Artists Followed</div>
                </div>
              </CardContent>
            </Card>
            <Card>
            ...
            <Card>
              <CardContent className="pt-4">
                <div className="text-center">
                  <div className="text-2xl font-bold">{stats.showsAttended}</div>
            ...
            <Card>
              <CardContent className="pt-4">
                <div className="text-center">
                  <div className="text-2xl font-bold">{stats.setlistsCreated}</div>
                  <div className="text-xs text-muted-foreground">Setlists Created</div>
                </div>
              </CardContent>
            </Card>
          </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      {activity.artistImage && (
                        <Avatar className="h-8 w-8 flex-shrink-0">
                          <AvatarImage src={activity.artistImage} alt={activity.artistName} />
                          <AvatarFallback>
                    ...
                    {(activity.showName || activity.venueName) && (
                      <div className="mt-2 text-xs text-muted-foreground">
                        {activity.showName && (
                          <span>{activity.showName}</span>
                        )}
                        {activity.venueName && (
                          <span> at {activity.venueName}</span>
                        )}
                        {activity.showDate && (
                          <span> • {new Date(activity.showDate).toLocaleDateString()}</span>
                        )}
                      </div>
                    )}
                  </div>
