...
export async function GET(request: NextRequest) {
  try {
    ...
    if (status && status !== 'all') {
      switch (status) {
        case 'active':
          query = query.not('email_confirmed_at', 'is', null).eq('is_banned', false);
          break;
        case 'banned':
    ...
    const [
      ...
      supabase.from('users').select('*', { count: 'exact', head: true })
        .not('email_confirmed_at', 'is', null).eq('is_banned', false),
      supabase.from('users').select('*', { count: 'exact', head: true }).eq('is_banned', true),
      supabase.from('users').select('*', { count: 'exact', head: true }).is('email_confirmed_at', null),
      supabase.from('users').select('*', { count: 'exact', head: true }).eq('role', 'admin'),
      supabase.from('users').select('*', { count: 'exact', head: true }).eq('role', 'moderator')
    ]);
