...
export async function GET(
  ...
  try {
    ...
    const totalContentCreated = (setlistsCreated || 0) + (reviewsWritten || 0) + (photosUploaded || 0);
    ...
    const stats = {
      ...
      engagement: {
        ...
        avg_review_rating: venueActivity?.length > 0 
          ? venueActivity.reduce((sum, v) => sum + v.rating, 0) / venueActivity.length 
          : 0
      },
...
function calculateCommunityEngagement(metrics: {
  ...
  const totalInteractions = metrics.votes + metrics.reviews * 2 + metrics.comments;
  
  if (totalInteractions < 5) return 'low';
  if (totalInteractions < 25) return 'medium';
  return 'high';
}
