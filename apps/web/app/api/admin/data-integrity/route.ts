...
import { artists, shows, venues, setlists, setlistSongs, votes, songs } from '@repo/database';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const addCheck = (name: string, status: 'pass' | 'fail' | 'warning', message: string, data?: any) => {
      ...
      report.summary[status === 'pass' ? 'passed' : status === 'fail' ? 'failed' : 'warnings']++;
    };
    ...
    if (artistsWithoutSpotify[0].count > 0) {
      ...
      addCheck('Artists without Spotify ID', 'pass', 'All artists have Spotify IDs');
    }
    ...
    if (showsWithoutVenues[0].count > 0) {
      ...
      addCheck('Shows without venues', 'pass', 'All shows have venue information');
    }
    ...
    if (orphanedSetlistSongs[0].count > 0) {
      ...
      addCheck('Orphaned setlist songs', 'pass', 'No orphaned setlist songs found');
    }
    ...
    if (voteCountMismatches.length > 0) {
      ...
      addCheck('Vote count consistency', 'pass', 'All vote counts are consistent');
    }
    ...
    addCheck('API error rates', 'pass', 'API error monitoring not yet implemented');
    ...
    if (queryTime > 1000) {
      ...
      addCheck('Database performance', 'pass', `Database responsive (${queryTime}ms)`);
    }
    ...
    switch (action) {
      case 'fix_vote_counts':
        ...
        return NextResponse.json({
          ...
          affectedRows: fixResults.rowCount
        });
      ...
      case 'cleanup_orphaned_votes':
        ...
        return NextResponse.json({
          ...
          deletedRows: orphanedResult.rowCount
        });
