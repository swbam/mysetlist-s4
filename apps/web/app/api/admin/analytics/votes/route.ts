...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: voteData, error } = await baseQuery.order('created_at', { ascending: false });
    ...
    for (let i = 0; i < dayCount; i++) {
      ...
      const dateKey = period === '1d' 
        ? format(date, 'HH:00') 
        : format(date, 'MMM dd');
      dailyVotes.set(dateKey, { date: dateKey, votes: 0, upvotes: 0, downvotes: 0 });
    }
    ...
    const engagementMetrics = {
      avgVotesPerUser: summary.uniqueUsers > 0 ? (summary.totalVotes / summary.uniqueUsers).toFixed(2) : 0,
      avgVotesPerSong: summary.uniqueSongs > 0 ? (summary.totalVotes / summary.uniqueSongs).toFixed(2) : 0,
      upvoteRatio: summary.totalVotes > 0 ? ((summary.upvotes / summary.totalVotes) * 100).toFixed(1) : 0,
      votingVelocity: period === '1d' ? summary.totalVotes : (summary.totalVotes / parseInt(period)).toFixed(1)
    };
    ...
    console.error('Error in vote analytics API:', error);
