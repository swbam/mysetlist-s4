...
import { votes, setlistSongs, songs, setlists, shows } from '@repo/database';
import { eq, and, desc, gte, sql } from 'drizzle-orm';
import { subHours, format, startOfHour, eachHourOfInterval } from 'date-fns';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const hourlyData = hourlyIntervals.map(intervalStart => {
      ...
      const hourVotes = votesData.filter(vote => 
    ...
    const totalDownvotes = votesData.filter(v => v.voteType === 'down').length;
    ...
    const peakHour = hourlyData.find(h => h.votes === maxVotes)?.hour || '00:00';
    const quietHour = hourlyData.find(h => h.votes === minVotes)?.hour || '00:00';
    ...
    const recentAvg = recentHours.reduce((sum, h) => sum + h.votes, 0) / Math.max(1, recentHours.length);
    const previousAvg = previousHours.reduce((sum, h) => sum + h.votes, 0) / Math.max(1, previousHours.length);
    ...
    const hourlyGrowth = hourlyData.slice(1).map((h, i) => h.votes - hourlyData[i].votes);
    ...
    const topMoments = hourlyData
      ...
      .map(h => ({
        ...
        votes: h.votes,
        impact: h.votes > averageVotes * 3 ? 'high' as const :
                h.votes > averageVotes * 2 ? 'medium' as const : 'low' as const,
      }))
    ...
    const trendMultiplier = currentTrend === 'up' ? 1.2 : currentTrend === 'down' ? 0.8 : 1.0;
    const timeOfDay = new Date().getHours();
    
    // Adjust for typical activity patterns (simplified)
    let timeMultiplier = 1.0;
    if (timeOfDay >= 18 && timeOfDay <= 23) timeMultiplier = 1.3; // Evening peak
    else if (timeOfDay >= 0 && timeOfDay <= 6) timeMultiplier = 0.3; // Night quiet
    else if (timeOfDay >= 12 && timeOfDay <= 17) timeMultiplier = 1.1; // Afternoon

    const predictedVotes = Math.round(
      (lastHourVotes * 0.4 + averageVotes * 0.6) * trendMultiplier * timeMultiplier
    );
    ...
    if (currentTrend !== 'stable') {
      predictionFactors.push(`${currentTrend === 'up' ? 'Upward' : 'Downward'} trend detected`);
    }
