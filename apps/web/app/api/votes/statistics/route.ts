...
export async function GET(request: NextRequest) {
  try {
    ...
    const averageNetVotes = setlistSongsData.length > 0 
      ? setlistSongsData.reduce((acc, song) => acc + song.netVotes, 0) / setlistSongsData.length 
      : 0;
    ...
    const recentActivity = allVotes
      ...
      .slice(0, 20)
      .map(vote => {
        const songData = setlistSongsData.find(s => s.id === vote.setlistSongId);
        return {
          id: vote.id,
    ...
    const votesLastHour = allVotes.filter(vote => vote.createdAt >= oneHourAgo).length;
