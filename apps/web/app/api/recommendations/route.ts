...
export async function GET(request: NextRequest) {
  try {
    ...
    const includeExplanations =
      searchParams.get("includeExplanations") === "true";
    const diversityFactor = Number.parseFloat(
      searchParams.get("diversityFactor") || "0.3",
    );
    const recencyWeight = Number.parseFloat(
      searchParams.get("recencyWeight") || "0.2",
    );
    const minSimilarity = Number.parseFloat(
      searchParams.get("minSimilarity") || "0.3",
    );
