...
import { processEmailAutomation, getPersonalizedEmailData, trackEmailEngagement } from '~/lib/email/automation-engine';
...
function isValidAutomationRequest(request: NextRequest): boolean {
  ...
  const automationSecret = process.env['EMAIL_AUTOMATION_SECRET'] || process.env['CRON_SECRET'];
    ...
    switch (action) {
      ...
      case 'track_engagement':
        if (!userId || !metadata?.emailId || !metadata?.action) {
          return NextResponse.json(
            { error: 'userId, emailId, and action are required for engagement tracking' },
            { status: 400 }
          );
        }
  ...
  try {
    ...
    switch (type) {
      ...
      case 'analytics':
        ...
        const totalSent = emailStats?.filter(e => e.status === 'sent').length || 0;
        ...
        const totalComplained = emailStats?.filter(e => e.complained_at).length || 0;
    ...
    if (token) {
      try {
        ...
        if (Date.now() - payload.timestamp > 30 * 24 * 60 * 60 * 1000) {
          return NextResponse.json({ error: 'Unsubscribe token expired' }, { status: 400 });
        }
      } catch {
