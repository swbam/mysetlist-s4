...
import { artists, venues, shows, songs, setlists, setlistSongs } from '@repo/database';
    ...
    switch (operation) {
      case 'stats': {
        ...
        return NextResponse.json({
          ...
          distribution: dataDistribution.rows.reduce((acc, row) => {
      ...
      case 'health': {
        ...
        try {
          ...
          if (Number(orphanedSetlists.rows[0]?.count) > 0) {
            issues.push(`${orphanedSetlists.rows[0]?.count} orphaned setlists found`);
          }
      ...
      default:
        return NextResponse.json(
          { error: 'Invalid operation. Valid operations: stats, health, table_sizes' },
          { status: 400 }
        );
    }
  } catch (error) {
    ...
    switch (operation) {
      ...
      case 'cleanup_orphaned_records': {
        ...
        const orphanedSetlistSongs = await db.execute(sql`
          ...
          RETURNING id
        `);
        results.push(`Removed ${orphanedSetlistSongs.rowCount} orphaned setlist songs`);
      ...
      case 'seed_sample_data': {
        ...
        const insertedArtists = await db.insert(artists)
      ...
      default:
        return NextResponse.json(
          { error: 'Invalid operation. Valid operations: vacuum, update_vote_counts, cleanup_orphaned_records, update_trending_scores, reindex, seed_sample_data' },
          { status: 400 }
        );
    }
  } catch (error) {
