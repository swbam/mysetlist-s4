  ...
  try {
    ...
    for (const setlist of setlists) {
      try {
        ...
        if (!artist) {
          results.errors.push(`Artist not found in database: ${setlist.artist.name}`);
          processTimer();
          continue;
        }
        ...
        const { data: show } = await supabase
          ...
          .lte('show_date', new Date(showDate.getTime() + 24 * 60 * 60 * 1000).toISOString().split('T')[0])
          .single();

        if (!show) {
          results.errors.push(`No matching show found for setlist: ${setlist.id}`);
          processTimer();
          continue;
        }
        ...
        if (setlistError) {
          results.errors.push(`Setlist creation error: ${setlistError.message}`);
          processTimer();
          continue;
        }
    ...
    monitor.trackExternalAPI('setlistfm', '/sync', duration, results.errors.length === 0);
...
async function syncVenue(supabase: any, venue: any) {
  try {
    ...
    return { venueId: null, error: error instanceof Error ? error.message : 'Unknown error' };
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const syncStatus = {
      ...
      showsWithSetlists: artist.shows?.filter(show => show.setlists?.length > 0).length || 0,
      totalSetlists: artist.shows?.reduce((sum, show) => sum + (show.setlists?.length || 0), 0) || 0,
