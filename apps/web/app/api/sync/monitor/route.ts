...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({
      ...
      healthScore,
      statistics: {
        artists: artistStats || { total: 0, synced: 0, syncedRecently: 0, needsSync: 0, withTrending: 0 },
        shows: showStats || { total: 0, upcoming: 0, completed: 0, withTrending: 0, createdRecently: 0 },
        venues: venueStats || { total: 0, withShows: 0, createdRecently: 0 },
      },
      syncSummary,
      trending: {
