...
async function handleLike(supabase: any, userId: string, targetType: string, targetId: string) {
  try {
...
async function handleUnlike(supabase: any, userId: string, targetType: string, targetId: string) {
  try {
    const { error } = await supabase
      .from('user_likes')
      .delete()
...
async function handleSave(supabase: any, userId: string, targetType: string, targetId: string) {
  try {
...
async function handleUnsave(supabase: any, userId: string, targetType: string, targetId: string) {
  try {
    const { error } = await supabase
      .from('user_saves')
      .delete()
...
export async function GET(request: NextRequest) {
  try {
    ...
    const status = targetIds.reduce((acc, id) => {
      acc[id] = {
