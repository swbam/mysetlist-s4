      ...
      switch (service.toLowerCase()) {
        ...
        default:
          return NextResponse.json(
            { error: 'Invalid service. Valid services: spotify, ticketmaster, setlistfm' },
            { status: 400 }
          );
      }
    } else {
        ...
        },
        {
          ...
          check: async () => {
            ...
            return checkAPIHealth('Setlist.fm', client, async () => {
              return await client.searchSetlists({ artistName: 'Radiohead', p: 1 });
            });
          },
        },
      ];
  ...
  try {
    ...
    switch (service?.toLowerCase()) {
      case 'spotify': {
        ...
        switch (action) {
          case 'search_artists':
            results.data = await client.searchArtists(params.query || 'test', params.limit || 5);
            break;
          case 'get_artist':
            if (!params.artistId) {
          ...
          case 'get_top_tracks':
            ...
            results.data = await client.getArtistTopTracks(params.artistId, params.market || 'US');
            break;
