...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const [requestDuration, requestCount, dbDuration, apiDuration] = performanceMetrics;
    ...
    if (includeSystemMetrics) {
      systemMetrics = {
        memory: {
          rss: await monitoringService.getMetrics('system_memory_rss', timeRange),
          heapUsed: await monitoringService.getMetrics('system_memory_heap_used', timeRange),
          heapTotal: await monitoringService.getMetrics('system_memory_heap_total', timeRange)
        },
        cpu: {
          user: await monitoringService.getMetrics('system_cpu_user', timeRange),
          system: await monitoringService.getMetrics('system_cpu_system', timeRange)
        }
      };
    }
    ...
    const stats = {
      requests: {
        ...
        avgDuration: requestDuration.length > 0 
          ? requestDuration.reduce((sum, m) => sum + m.value, 0) / requestDuration.length 
          : 0,
        errorRate: errorLogs.length / Math.max(requestCount.length, 1)
      },
      database: {
        totalQueries: dbDuration.length,
        avgDuration: dbDuration.length > 0 
          ? dbDuration.reduce((sum, m) => sum + m.value, 0) / dbDuration.length 
          : 0
      },
      externalAPIs: {
        totalCalls: apiDuration.length,
        avgDuration: apiDuration.length > 0 
          ? apiDuration.reduce((sum, m) => sum + m.value, 0) / apiDuration.length 
          : 0
      }
    };

    // Top endpoints by request count
    const endpointStats = requestCount.reduce((acc, metric) => {
