...
function isAuthorizedAnalyticsRequest(request: NextRequest): boolean {
  ...
  const analyticsSecret = process.env['ANALYTICS_SECRET'] || process.env['ADMIN_SECRET'];
...
export async function GET(request: NextRequest) {
  try {
    ...
    const cohortType = searchParams.get('cohortType') as 'monthly' | 'weekly' || 'monthly';

    switch (type) {
      case 'cohort':
        const cohortAnalysis = await getCohortAnalysis(startDate, endDate, cohortType);
      ...
      case 'predictive':
        const predictiveAnalytics = await getPredictiveAnalytics(startDate, endDate);
        return NextResponse.json({
          success: true,
      ...
      case 'funnel':
        ...
        const funnelAnalysis = await getFunnelAnalysis(funnelSteps, startDate, endDate);
        return NextResponse.json({
          success: true,
          data: funnelAnalysis,
          timestamp: new Date().toISOString()
...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (reportType) {
      case 'custom_cohort':
        const customCohort = await getCohortAnalysis(startDate, endDate, cohortType);
      ...
      case 'segment_analysis':
        const segmentAnalysis = await analyzeUserSegments(segments || [], startDate, endDate);
        return NextResponse.json({
          success: true,
      ...
      case 'custom_funnel':
        const customFunnel = await getFunnelAnalysis(
          customFunnelSteps || ['user_signup', 'first_vote', 'profile_complete'],
          startDate,
          endDate
        );
      ...
      case 'comparative_analysis':
        ...
        const prevEndDate = new Date(new Date(endDate).getTime() - periodDiff).toISOString();
        ...
        const comparison = {
          ...
          changes: calculatePeriodChanges(currentPeriodData, previousPeriodData)
        };
      ...
      case 'predictive_report':
        const predictiveReport = await generatePredictiveReport(startDate, endDate);
        return NextResponse.json({
          success: true,
  ...
  try {
    ...
    switch (updateType) {
      ...
      case 'predictive_settings':
        const { data: predictiveConfig, error: predictiveError } = await supabase
          .from('analytics_config')
          .upsert({
...
// Helper functions
async function generateAdvancedInsights(startDate: string, endDate: string): Promise<any> {
  ...
  const insights = {
    keyMetrics: {
      ...
      highValueUsers: rfm.segments.find(s => s.segment === 'Champions')?.userCount || 0
    },
    trends: {
      retentionTrend: retention.daySevenRetention > retention.dayOneRetention ? 'improving' : 'declining',
...
function generateRetentionRecommendations(retention: any): string[] {
  ...
  if (retention.churnRate > 20) {
    recommendations.push('Implement churn prevention campaigns - churn rate is above 20%');
  }
  
  if (retention.stickiness < 20) {
    recommendations.push('Improve daily engagement features - stickiness is below 20%');
  }
...
function generateChurnRecommendations(churnPrediction: any[]): string[] {
  ...
  if (highRiskUsers.length > 0) {
    recommendations.push(`${highRiskUsers.length} users have >70% churn probability - immediate intervention needed`);
  }
...
function generateAlerts(_cohort: any, retention: any, predictive: any, rfm: any): any[] {
  ...
  const hibernatingUsers = rfm.segments.find((s: any) => s.segment === 'Hibernating')?.userCount || 0;
...
async function generatePredictiveReport(startDate: string, endDate: string): Promise<any> {
  const predictive = await getPredictiveAnalytics(startDate, endDate);
  
  const report = {
    summary: {
      totalPredictions: predictive.userGrowthPrediction.length,
      highRiskUsers: predictive.churnPrediction.filter(p => p.churnProbability > 0.7).length,
    ...
    churnAnalysis: {
      ...
      interventionRecommendations: generateInterventionRecommendations(predictive.churnPrediction)
    },
    growthProjection: {
...
function generateInterventionRecommendations(churnPrediction: any[]): string[] {
  ...
  topRiskFactors.forEach(({ factor, count }) => {
    if (factor.includes('Inactive')) {
      recommendations.push(`${count} users inactive - send re-engagement campaigns`);
    } else if (factor.includes('Low session')) {
      recommendations.push(`${count} users with low engagement - improve onboarding`);
    } else if (factor.includes('New user')) {
      recommendations.push(`${count} new users at risk - optimize first-week experience`);
    }
  });
...
function generatePredictiveActionItems(predictive: any): string[] {
  ...
  if (predictive.seasonalityAnalysis.peakPeriods.length > 0) {
    actionItems.push(`Prepare for peak periods: ${predictive.seasonalityAnalysis.peakPeriods.join(', ')}`);
  }
...
function calculatePeriodChanges(current: any[], previous: any[]): any {
  const changes = {
    retention: {
      dayOneRetention: ((current[1].dayOneRetention - previous[1].dayOneRetention) / previous[1].dayOneRetention) * 100,
      daySevenRetention: ((current[1].daySevenRetention - previous[1].daySevenRetention) / previous[1].daySevenRetention) * 100,
      churnRate: ((current[1].churnRate - previous[1].churnRate) / previous[1].churnRate) * 100
    },
    cohorts: {
      averageRetention: current[0].averageRetention[0] - previous[0].averageRetention[0],
      cohortCount: current[0].cohorts.length - previous[0].cohorts.length
    },
    rfm: {
      champions: (current[2].segments.find((s: any) => s.segment === 'Champions')?.userCount || 0) - 
                 (previous[2].segments.find((s: any) => s.segment === 'Champions')?.userCount || 0),
      atRisk: (current[2].segments.find((s: any) => s.segment === 'At Risk')?.userCount || 0) - 
              (previous[2].segments.find((s: any) => s.segment === 'At Risk')?.userCount || 0)
    }
  };
