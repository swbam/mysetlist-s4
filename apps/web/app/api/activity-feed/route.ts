...
export async function GET(request: NextRequest) {
  try {
    ...
    const timeWindow = parseInt(searchParams.get('days') || '30');
    ...
    if (newShows) {
      newShows.forEach(show => {
        activities.push({
          ...
          artistId: show.artists.id,
          artistName: show.artists.name,
          artistImage: show.artists.image_url,
          title: `New show announced: ${show.artists.name}`,
          description: `${show.venues.name}, ${show.venues.city} - ${new Date(show.show_date).toLocaleDateString()}`,
    ...
    if (recentSetlists) {
      ...
      recentSetlists.forEach(item => {
        const setlistId = item.setlists.id;
      ...
      setlistMap.forEach((setlist, setlistId) => {
        activities.push({
          ...
          artistId: setlist.shows.artists.id,
          artistName: setlist.shows.artists.name,
          artistImage: setlist.shows.artists.image_url,
          title: `Setlist added for ${setlist.shows.artists.name}`,
          description: `${setlist.shows.venues.name}, ${setlist.shows.venues.city}`,
          link: `/shows/${setlist.shows.slug}`,
          metadata: {
            showDate: setlist.shows.show_date,
            venueName: setlist.shows.venues.name,
          }
        });
      });
    }
    ...
    if (activeShows) {
      ...
      showActivityMap.forEach((activity, showId) => {
        if (activity.voteCount >= 10) { // Threshold for "trending"
          activities.push({
            id: `trending-${showId}`,
            type: 'show_update',
