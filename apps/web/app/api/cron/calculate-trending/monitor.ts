  ...
  try {
    ...
    const [
      ...
      db.select({ count: sql<number>`count(*)::int` })
        ...
        .where(gte(userActivityLog.createdAt, new Date(Date.now() - 24 * 60 * 60 * 1000))),
    ...
    const performance = {
      ...
      calculationFrequency: getCalculationFrequency(lastCalculationResult[0]?.createdAt),
    };
    ...
    if (metrics.artistsWithTrendingScores === 0 && metrics.totalArtists > 0) {
      issues.push('No artists have trending scores - trending calculation may not be running');
      status = 'error';
    }
    
    if (metrics.showsWithTrendingScores === 0 && metrics.totalShows > 0) {
      issues.push('No shows have trending scores - trending calculation may not be running');
      if (status !== 'error') status = 'degraded';
    }

    // Check last calculation time
    const lastCalc = lastCalculationResult[0]?.createdAt;
    if (!lastCalc || Date.now() - lastCalc.getTime() > 25 * 60 * 60 * 1000) { // > 25 hours
      issues.push('Trending calculation has not run in the last 25 hours');
      if (status !== 'error') status = 'degraded';
    }

    // Check trending score coverage
    const artistCoverage = metrics.totalArtists > 0 ? 
      (metrics.artistsWithTrendingScores / metrics.totalArtists) * 100 : 0;
    
    if (artistCoverage < 50) {
      issues.push(`Low artist trending score coverage: ${artistCoverage.toFixed(1)}%`);
      if (status !== 'error') status = 'degraded';
    }

    // Check for recent activity
    if (metrics.recentActivityCount === 0) {
      recommendations.push('No user activity in last 24 hours - consider marketing initiatives');
    }

    // Check average trending score health
    if (avgTrendingScore < 10) {
      recommendations.push('Average trending score is low - consider adjusting calculation weights');
    }

    // Performance recommendations
    if (metrics.totalArtists > 10000) {
      recommendations.push('Large artist database detected - consider optimizing batch processing');
    }
    ...
    return {
      ...
      recommendations: ['Check database connection and trending calculation service'],
    };
  }
}
...
export async function getTrendingAnalytics(days = 7) {
  ...
  const calculationHistory = await db.select({
    createdAt: userActivityLog.createdAt,
...
if (require.main === module) {
  checkTrendingSystemHealth()
    .then(health => {
      ...
      console.log(`   Artists with Trending Scores: ${health.metrics.artistsWithTrendingScores}`);
      console.log(`   Total Shows: ${health.metrics.totalShows}`);
      console.log(`   Shows with Trending Scores: ${health.metrics.showsWithTrendingScores}`);
      console.log(`   Recent Activity (24h): ${health.metrics.recentActivityCount}`);
      console.log(`   Last Calculation: ${health.metrics.lastCalculationTime || 'Never'}\n`);
      
      console.log('🏆 Performance:');
      console.log(`   Avg Trending Score: ${health.performance.avgTrendingScore.toFixed(2)}`);
      console.log(`   Top Artist: ${health.performance.topTrendingArtist || 'None'}`);
      console.log(`   Top Show: ${health.performance.topTrendingShow || 'None'}`);
      console.log(`   Calculation Frequency: ${health.performance.calculationFrequency}\n`);
