...
export async function GET(request: NextRequest) {
  const results = {
    ...
    database: {
      configured: !!process.env.DATABASE_URL || !!process.env.NEXT_PUBLIC_SUPABASE_URL,
      connection_test: null as any
    }
  };

  // Test Spotify authentication
  if (results.apis.spotify.client_id_configured && results.apis.spotify.client_secret_configured) {
    try {
      ...
      results.apis.spotify.auth_test = {
        ...
        error: authResponse.ok ? null : `Failed with status ${authResponse.status}`
      };
    } catch (error) {
      results.apis.spotify.auth_test = {
        success: false,
  ...
  if (results.apis.setlistfm.api_key_configured) {
    try {
      ...
      results.apis.setlistfm.connection_test = {
        ...
        error: setlistResponse.ok ? null : `Failed with status ${setlistResponse.status}`
      };
    } catch (error) {
      results.apis.setlistfm.connection_test = {
        success: false,
  ...
  try {
    ...
    const testQuery = await db.execute(sql`SELECT 1 as test, current_timestamp`);
