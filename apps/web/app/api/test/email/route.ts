import { sendShowReminderEmail, sendWelcomeEmail } from '@repo/email';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      !authHeader ||
      authHeader !== `Bearer ${process.env['EMAIL_SYSTEM_TOKEN']}`
    ) {
    ...
    switch (action) {
      ...
      case 'test-notification': {
        ...
        const notificationResult = await sendEmailNotification({
          ...
          content:
            data.content ||
            '<h1>Test Email</h1><p>This is a test email from MySetlist.</p>',
        });
      ...
      case 'test-all': {
        ...
        try {
          const welcomeTest = await sendWelcomeEmail({
            to: [
              { email: data.email || 'test@example.com', name: 'Test User' },
            ],
            name: 'Test User',
          });
        ...
        try {
          const notificationTest = await sendEmailNotification({
            ...
            content:
              '<h1>All Systems Operational</h1><p>Email system is working correctly.</p>',
          });
      ...
      default:
        return NextResponse.json(
          {
            error:
              'Invalid action. Valid actions: test-welcome, test-reminder, test-notification, test-queue, process-queue, test-all',
          },
          { status: 400 }
        );
    }
