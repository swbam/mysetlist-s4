  ...
  try {
    ...
    if (!clientId || !clientSecret) {
      return NextResponse.json({
        ...
        solution: 'Please add SPOTIFY_CLIENT_ID and SPOTIFY_CLIENT_SECRET to your .env.local file'
      }, { status: 500 });
    }

    console.log(`✅ Credentials found: ${clientId.substring(0, 8)}... / ${clientSecret.substring(0, 8)}...`);
    ...
    if (!authResponse.ok) {
      ...
      throw new Error(`Spotify authentication failed: ${authResponse.status} ${authResponse.statusText} - ${errorText}`);
    }
    ...
    if (!searchResponse.ok) {
      ...
      throw new Error(`Artist search failed: ${searchResponse.status} ${searchResponse.statusText} - ${errorText}`);
    }
    ...
    if (searchData.artists.items.length > 0) {
      ...
      const artistResponse = await fetch(`https://api.spotify.com/v1/artists/${firstArtist.id}`, {
        headers: {
          'Authorization': `Bearer ${authData.access_token}`,
        },
      });
    ...
    return NextResponse.json({
      ...
      query,
      apiConfigured: !!(process.env.SPOTIFY_CLIENT_ID && process.env.SPOTIFY_CLIENT_SECRET),
