...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!artistResponse.ok) {
      ...
      throw new Error(`Artist search failed: ${artistResponse.status} ${artistResponse.statusText} - ${errorText}`);
    }
    ...
    if (!setlistResponse.ok) {
      ...
      throw new Error(`Setlist search failed: ${setlistResponse.status} ${setlistResponse.statusText} - ${errorText}`);
    }
    ...
    return NextResponse.json({
      ...
      response: {
        artistSearch: {
          ...
          artists: artistData.artist?.map((a: any) => ({
            mbid: a.mbid,
            name: a.name,
            sortName: a.sortName,
            url: a.url
        ...
        setlistSearch: {
          ...
          setlists: setlistData.setlist?.slice(0, 3).map((s: any) => ({
            id: s.id,
