...
// Test data
const TEST_ARTISTS = ['dispatch', 'our-last-night', 'metallica', 'taylor-swift', 'the-weeknd'];
...
export default function () {
  ...
  group('Voting System Performance', () => {
    ...
    const startTime = Date.now();
    const res = http.post(`${BASE_URL}/api/votes`, JSON.stringify(votePayload), params);
    ...
    check(res, {
      'vote API responds': (r) => r.status === 200 || r.status === 201 || r.status === 400,
