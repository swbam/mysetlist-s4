...
serve(async (req) => {
  ...
  try {
    ...
    for (const show of shows || []) {
      ...
      const score = calculateTrendingScore(votes, attendees, new Date(show.created_at));
    ...
    for (const artist of artists || []) {
      const recentShows = artist.shows.filter(
        show => new Date(show.created_at) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)
      );

      if (recentShows.length > 0) {
        const totalVotes = recentShows.reduce(
          (sum, show) => sum + (show.show_votes?.[0]?.count || 0), 0
        );
        const totalAttendees = recentShows.reduce(
          (sum, show) => sum + (show.show_attendees?.[0]?.count || 0), 0
        );
    ...
    const cleanupDate = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString();
...
function calculateTrendingScore(votes: number, attendees: number, createdAt: Date): number {
