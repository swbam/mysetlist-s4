...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const { type = 'daily', location, artistName } = await req.json() as SyncRequest;
    ...
    switch (type) {
      case 'daily':
        results = await syncDailyShows(supabase, ticketmasterApiKey, spotifyToken);
        break;
      case 'weekly':
        results = await syncWeeklyShows(supabase, ticketmasterApiKey, spotifyToken);
        break;
      case 'location':
        if (location) {
          results = await syncLocationShows(supabase, ticketmasterApiKey, spotifyToken, location);
        }
        break;
      case 'artist':
        if (artistName) {
          results = await syncArtistShows(supabase, ticketmasterApiKey, spotifyToken, artistName);
        }
        break;
    }
...
async function syncDailyShows(supabase: any, ticketmasterKey: string, spotifyToken: string) {
  ...
  if (data._embedded?.events) {
    for (const event of data._embedded.events) {
      ...
      if (event._embedded?.attractions?.[0]) {
        ...
        const spotifyArtist = await searchSpotifyArtist(attraction.name, spotifyToken);
        ...
        const { error: artistError } = await supabase
          ...
          .upsert({
            ...
            image_url: spotifyArtist?.images?.[0]?.url || attraction.images?.[0]?.url,
      ...
      const { error: showError } = await supabase
        .from('shows')
        .upsert({
          artist_id: await getArtistId(supabase, event._embedded?.attractions?.[0]?.name),
          venue_id: await getVenueId(supabase, event._embedded?.venues?.[0]?.name),
          show_date: event.dates.start.dateTime,
          slug: generateSlug(`${event._embedded?.attractions?.[0]?.name}-${event._embedded?.venues?.[0]?.name}-${event.dates.start.localDate}`),
          status: 'scheduled',
...
async function syncWeeklyShows(supabase: any, ticketmasterKey: string, spotifyToken: string) {
...
async function syncLocationShows(supabase: any, ticketmasterKey: string, spotifyToken: string, location: any) {
...
async function syncArtistShows(supabase: any, ticketmasterKey: string, spotifyToken: string, artistName: string) {
...
async function getArtistId(supabase: any, name: string): Promise<string | null> {
  if (!name) return null;
