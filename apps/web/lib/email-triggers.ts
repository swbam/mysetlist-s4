...
export async function triggerNewShowNotifications(showId: string) {
  console.log(`New show notification trigger disabled for show ${showId} - user follows not available`);
  return;
}
...
export async function triggerSetlistUpdateNotifications(
  ...
  console.log(`Setlist update notification trigger disabled for show ${showId} - user follows not available`);
  return;
}
