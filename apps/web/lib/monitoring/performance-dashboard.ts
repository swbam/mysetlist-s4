...
} from "../validation/performance-validator";
...
export function usePerformanceDashboard() {
  ...
  const refreshMetrics = useCallback(async () => {
    try {
      ...
      const [validationReport, databaseReport, systemMetrics, webMetrics] =
      ...
      const dashboard = await processDashboardData({
        validationReport:
          validationReport.status === "fulfilled"
            ? validationReport.value
            : null,
        databaseReport:
          databaseReport.status === "fulfilled" ? databaseReport.value : null,
        systemMetrics:
...
async function processDashboardData(data: {
  ...
  const systemHealth = processSystemHealth(
  ...
  const webPerformance = processWebPerformanceMetrics(
    data.webMetrics,
    data.validationReport,
  );
...
function processImportFlowMetrics(
  ...
  const importFlowResults =
    validationReport?.results.filter((r) => r.category === "Import Flow") || [];

  // Extract timing data from validation report
  const phase1Result = importFlowResults.find((r) =>
    r.testName.includes("Phase 1"),
  );
  const phase2Result = importFlowResults.find((r) =>
    r.testName.includes("Phase 2"),
  );
  const phase3Result = importFlowResults.find((r) =>
    r.testName.includes("Phase 3"),
  );
  ...
  const successRate =
    importFlowResults.filter((r) => r.passed).length /
    Math.max(importFlowResults.length, 1);
...
function processDatabaseHealthMetrics(
  ...
  const averageQueryTime =
    queryResults.length > 0
      ? queryResults.reduce(
          (sum, r) => sum + (r.executionTime > 0 ? r.executionTime : 0),
          0,
        ) / queryResults.length
      : 0;
...
function generateTrendData(
  ...
  const importPhase1 = validationReport.results.find((r) =>
    r.testName.includes("Phase 1"),
  );
  if (importPhase1) {
    trends.push({
