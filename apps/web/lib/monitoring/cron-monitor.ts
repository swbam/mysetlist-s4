...
interface AlertRule {
  id: string;
  jobName: string;
  condition: 'error_rate' | 'execution_time' | 'consecutive_failures' | 'missed_execution';
  threshold: number;
  enabled: boolean;
...
class CronJobMonitor {
  ...
  async recordExecution(
    ...
    const cutoff = new Date(Date.now() - MONITORING_CONFIG.METRIC_RETENTION_DAYS * 24 * 60 * 60 * 1000);
  ...
  private async updateHealthStatus(jobName: string, metric: CronJobMetric): Promise<void> {
  ...
  private async sendAlert(rule: AlertRule, message: string, health: CronJobHealth): Promise<void> {
  ...
  private async sendNotification(channel: string, message: string, health: CronJobHealth): Promise<void> {
  ...
  getHealthStatus(jobName?: string): CronJobHealth | CronJobHealth[] {
    if (jobName) {
      return this.healthStatus.get(jobName) || {
        jobName,
        isHealthy: false,
        lastExecution: null,
        consecutiveFailures: 0,
  ...
  getSystemHealth(): {
    ...
    return {
      ...
      averageErrorRate: allHealth.length > 0 ? totalErrorRate / allHealth.length : 0,
...
export function withMonitoring<T extends (...args: any[]) => Promise<any>>(
  ...
  return (async (...args: Parameters<T>) => {
    ...
    try {
      ...
      await cronMonitor.recordExecution(jobName, 'success', executionTime, { result });
