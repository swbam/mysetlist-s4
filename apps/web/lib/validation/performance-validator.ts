...
import {
  ...
  performanceMonitor,
} from "../optimizations/bundle-optimization";
import {
  ArtistImportOrchestrator,
  type ImportResult,
} from "../services/artist-import-orchestrator";
...
export class PerformanceValidator {
  ...
  async validateAllPerformance(): Promise<PerformanceValidationReport> {
    ...
    try {
      ...
      console.log(
        `✅ Performance validation completed - Overall Score: ${report.overallScore.toFixed(1)}%`,
      );
      return report;
    } catch (error) {
  ...
  private async validateImportFlowPerformance(): Promise<void> {
    ...
    this.addResult({
      ...
      recommendations:
        avgPhase1 > VALIDATION_TARGETS.PHASE_1_TARGET
          ? [
    ...
    this.addResult({
      ...
      recommendations:
        avgPhase2 > VALIDATION_TARGETS.PHASE_2_TARGET
          ? [
    ...
    this.addResult({
      ...
      recommendations:
        avgPhase3 > VALIDATION_TARGETS.PHASE_3_TARGET
          ? [
    ...
    this.addResult({
      ...
      score: this.calculateScore(
        avgTotal,
        VALIDATION_TARGETS.TOTAL_IMPORT_TARGET,
      ),
      recommendations:
        avgTotal > VALIDATION_TARGETS.TOTAL_IMPORT_TARGET
          ? [
  ...
  private async validateBundlePerformance(): Promise<void> {
    ...
    this.addResult({
      ...
      score: this.calculateScore(
        bundleSizes.homepage,
        VALIDATION_TARGETS.HOMEPAGE_BUNDLE,
        true,
      ),
      recommendations:
        bundleSizes.homepage > VALIDATION_TARGETS.HOMEPAGE_BUNDLE
          ? [
    ...
    this.addResult({
      ...
      score: this.calculateScore(
        bundleSizes.artistPage,
        VALIDATION_TARGETS.ARTIST_PAGE_BUNDLE,
        true,
      ),
      recommendations:
        bundleSizes.artistPage > VALIDATION_TARGETS.ARTIST_PAGE_BUNDLE
          ? [
    ...
    this.addResult({
      ...
      score: this.calculateScore(
        bundleSizes.showPage,
        VALIDATION_TARGETS.SHOW_PAGE_BUNDLE,
        true,
      ),
      recommendations:
        bundleSizes.showPage > VALIDATION_TARGETS.SHOW_PAGE_BUNDLE
          ? [
  ...
  private async validateWebVitals(): Promise<void> {
    ...
    this.addResult({
      ...
      recommendations:
        webVitals.lcp > VALIDATION_TARGETS.LCP_TARGET
          ? [
    ...
    this.addResult({
      ...
      recommendations:
        webVitals.fid > VALIDATION_TARGETS.FID_TARGET
          ? [
    ...
    this.addResult({
      ...
      recommendations:
        webVitals.cls > VALIDATION_TARGETS.CLS_TARGET
          ? [
    ...
    this.addResult({
      ...
      score: this.calculateScore(
        webVitals.ttfb,
        VALIDATION_TARGETS.TTFB_TARGET,
      ),
      recommendations:
        webVitals.ttfb > VALIDATION_TARGETS.TTFB_TARGET
          ? [
  ...
  private async validateDatabasePerformance(): Promise<void> {
    ...
    try {
      ...
      this.addResult({
        ...
        score: this.calculateScore(
          avgQueryTime,
          VALIDATION_TARGETS.DB_QUERY_TARGET,
        ),
        details: { queryCount: dbReport.queryResults.length },
        recommendations:
          avgQueryTime > VALIDATION_TARGETS.DB_QUERY_TARGET
            ? [
      ...
      this.addResult({
        ...
        score: this.calculateScore(
          connectionTime,
          VALIDATION_TARGETS.DB_CONNECTION_TARGET,
        ),
        recommendations:
          connectionTime > VALIDATION_TARGETS.DB_CONNECTION_TARGET
            ? [
      ...
      this.addResult({
        ...
        score: this.calculateScore(
          VALIDATION_TARGETS.DB_CACHE_HIT_RATIO - cacheHitRatio,
          0.1,
        ),
        recommendations:
          cacheHitRatio < VALIDATION_TARGETS.DB_CACHE_HIT_RATIO
            ? [
  ...
  private async validateConcurrentPerformance(): Promise<void> {
    ...
    this.addResult({
      ...
      score: this.calculateScore(
        concurrentImportTime,
        VALIDATION_TARGETS.PHASE_1_TARGET * 1.5,
      ),
      recommendations:
        concurrentImportTime > VALIDATION_TARGETS.PHASE_1_TARGET * 1.5
          ? [
  ...
  private async validateResourceUsage(): Promise<void> {
    ...
    this.addResult({
      ...
      score: this.calculateScore(
        ...
        VALIDATION_TARGETS.MEMORY_USAGE_TARGET,
        true,
      ),
      recommendations:
        memoryUsage.heapUsed > VALIDATION_TARGETS.MEMORY_USAGE_TARGET
          ? [
    ...
    this.addResult({
      ...
      recommendations:
        cpuUsage > VALIDATION_TARGETS.CPU_USAGE_TARGET
          ? [
  ...
  private calculateAverageQueryTime(
    dbReport: DatabasePerformanceReport,
  ): number {
    const validQueries = dbReport.queryResults.filter(
      (q) => q.executionTime > 0,
    );
  ...
  private addResult(
    result: Omit<ValidationResult, "score"> & { score?: number },
  ): void {
    this.results.push({
      score: 0,
      ...result,
  ...
  private categorizeResults(): PerformanceValidationReport["categories"] {
    ...
    for (const [displayName, key] of Object.entries(categoryMap)) {
      ...
      const criticalIssues = categoryResults
        ...
        .map(
          (r) =>
            `${r.testName}: ${r.actual}${r.unit} (target: ${r.target}${r.unit})`,
        );
  ...
  private identifyCriticalIssues(): string[] {
    ...
    for (const failure of criticalFailures) {
      issues.push(
        `${failure.category}: ${failure.testName} failed critically (${failure.actual}${failure.unit} vs ${failure.target}${failure.unit} target)`,
      );
    }

    // Check for Phase 1 failures (critical for user experience)
    const phase1Failure = this.results.find(
      (r) => r.testName.includes("Phase 1") && !r.passed,
    );
  ...
  private generateRecommendations(): string[] {
    ...
    if (failedCategories.includes("importFlow")) {
      recommendations.add(
        "Consider implementing a phased rollout strategy for import optimizations",
      );
    }

    if (failedCategories.includes("databasePerformance")) {
      recommendations.add(
        "Prioritize database optimization as it affects all other performance metrics",
      );
    }

    if (failedCategories.includes("bundlePerformance")) {
      recommendations.add(
        "Implement progressive loading strategy to improve initial page load",
      );
    }
