...
async function fetchSpotifyData(artistName: string): Promise<any> {
  try {
    ...
    const [topTracks, albums] = await Promise.all([
      spotify.getArtistTopTracks(spotifyArtist.id).catch(() => ({ tracks: [] })),
      spotify.getArtistAlbums(spotifyArtist.id, { include_groups: 'album,single' }).catch(() => ({ items: [] }))
    ]);
...
export async function upsertArtistEnhanced(tmId: string) {
  ...
  if (spotifyData) {
    console.log(`Fetched Spotify data for: ${spotifyData.artist.name} (${spotifyData.artist.popularity} popularity)`);
  }
  ...
  if (existingArtist.length > 0) {
    ...
    if (spotifyData?.artist) {
      ...
      updateData.externalUrls = JSON.stringify(spotifyData.artist.external_urls);
      ...
      if (spotifyData.artist.images?.[0]?.url) {
        ...
        updateData.smallImageUrl = spotifyData.artist.images.find((img: any) => img.width < 500)?.url || spotifyData.artist.images[spotifyData.artist.images.length - 1]?.url;
      }
      ...
      console.log(`Enhanced with Spotify: ${spotifyData.artist.followers.total} followers, ${allGenres.length} genres`);
    }
  ...
  if (spotifyData?.artist) {
    ...
    if (spotifyData.artist.images?.[0]?.url) {
      ...
      newArtist.smallImageUrl = spotifyData.artist.images.find((img: any) => img.width < 500)?.url || spotifyData.artist.images[spotifyData.artist.images.length - 1]?.url;
    }
    ...
    console.log(`Enhanced new artist with Spotify: ${spotifyData.artist.followers.total} followers, ${allGenres.length} genres`);
  }
