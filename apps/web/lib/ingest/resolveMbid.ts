...
export async function resolveMusicBrainzMbid(
  ...
  try {
    ...
    if (data.artists && data.artists.length > 0) {
      ...
      const normalizedSearchName = artistName
        .toLowerCase()
        .replace(/[^a-z0-9]/g, "");
      const normalizedResultName = artist.name
        .toLowerCase()
        .replace(/[^a-z0-9]/g, "");

      if (normalizedSearchName === normalizedResultName || artist.score >= 90) {
        // MusicBrainz provides a score
        return artist.id;
      }
    }
