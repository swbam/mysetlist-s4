...
import { artists, setlists, setlistSongs, songs, shows, venues } from '@repo/database';
...
export async function upsertSetlists(rawSetlists: any[], artistId: string) {
  ...
  for (const setlist of rawSetlists) {
    try {
      ...
      if (!showId) {
        console.warn(`Could not find matching show for setlist ${setlist.id}, skipping`);
        continue;
      }
  ...
  console.log(`Upserted ${upsertedSetlists.length} setlists for artist ${artistId}`);
...
async function findOrCreateVenueFromSetlistFm(sfVenue: any) {
  ...
  try {
    ...
    const newVenue = {
      ...
      latitude: sfVenue.city?.coords?.lat ? parseFloat(sfVenue.city.coords.lat) : null,
      longitude: sfVenue.city?.coords?.long ? parseFloat(sfVenue.city.coords.long) : null,
      createdAt: new Date(),
      updatedAt: new Date(),
    };
...
async function processSetlistSongs(setlistId: string, sets: any[], artistId: string) {
  let position = 0;
