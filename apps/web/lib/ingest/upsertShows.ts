...
export async function upsertShows(rawShows: any[], artistId: string) {
  ...
  for (const show of rawShows) {
    try {
      ...
      if (!show.venue) {
        console.warn(`Skipping show ${show.tm_event_id} - no venue information`);
        continue;
      }
      ...
      if (!venue) {
        console.warn(`Skipping show ${show.tm_event_id} - venue creation failed`);
        continue;
      }
      ...
      if (existingShow.length > 0) {
        ...
        await db
          ...
          .set({
            name: show.name,
            ...(show.event_date && { date: new Date(show.event_date).toISOString().split('T')[0] }),
            status: show.status,
