...
class EmailAutomationEngine {
  ...
  public async generatePersonalizedData(
    userId: string,
  ): Promise<PersonalizedEmailData> {
    ...
    return {
      shows:
        recommendedShows?.map((show) => ({
          ...
          date: show.date,
          venue: show.venues?.[0]
            ? `${show.venues[0].name}, ${show.venues[0].city}`
            : "",
        })) || [],
  ...
  private async calculateEngagementMetrics(userId: string) {
    ...
    const emailOpens =
      emailEngagement?.filter((e) => e.action === "open").length || 0;
    const emailClicks =
      emailEngagement?.filter((e) => e.action === "click").length || 0;

    // Simple engagement score calculation
    const score = Math.min(
      100,
      activityCount * 2 + emailOpens * 5 + emailClicks * 10,
    );
    ...
    try {
      ...
      for (const trigger of this.defaultTriggers) {
        ...
        try {
          ...
          results.failed++;
          results.errors.push(
            `Trigger ${trigger.id}: ${error instanceof Error ? error.message : "Unknown error"}`,
          );
        }
      }
      ...
      case "welcome-series":
        ...
        for (const user of newUsers || []) {
          ...
          if (emailResult.success) {
            ...
            results.failed++;
            results.errors.push(
              `Welcome email for ${user.email}: ${emailResult.error?.message}`,
            );
          }
        }
        break;
        ...
        for (const show of upcomingShows || []) {
          ...
          for (const userFollow of interestedUsers || []) {
            ...
            const emailResult = await sendShowReminderEmail({
              to: [
                {
                  email: userFollow.users?.[0]?.email,
                  name: personalizedData.userName,
                },
              ],
            ...
            if (emailResult.success) {
              ...
              results.failed++;
              results.errors.push(
                `Show reminder for ${userFollow.users?.[0]?.email}: ${emailResult.error?.message}`,
              );
            }
          }
        }
      ...
      case "engagement-recovery":
        ...
        for (const user of inactiveUsers || []) {
          ...
          const emailResult = await sendWeeklyDigestEmail({
            ...
            followedArtists: personalizedData.preferences.favoriteArtists.map(
              (a) => ({
                id: a.id,
                name: a.name,
            ...
            upcomingShows: personalizedData.recommendations.shows
              .slice(0, 3)
              .map((show) => ({
                ...show,
                artistName: "Various Artists", // Since we don't have artist info in recommendations
              })),
            newSetlists: personalizedData.recommendations.shows
              .slice(3, 6)
              .map((show) => ({
                ...show,
            ...
            totalFollowedArtists:
              personalizedData.preferences.favoriteArtists.length,
          });
          ...
          if (emailResult.success) {
            ...
            results.failed++;
            results.errors.push(
              `Re-engagement email for ${user.email}: ${emailResult.error?.message}`,
            );
          }
        }
        break;
            ...
            if (topVote) {
              const emailResult = await sendVoteMilestoneEmail({
                ...
                show: {
                  id: "show-id",
                  name:
                    topVote.setlist_songs?.[0]?.setlists?.[0]?.shows?.[0]
                      ?.name || "Unknown Show",
                  artistName: "Artist",
                  venue:
                    topVote.setlist_songs?.[0]?.setlists?.[0]?.shows?.[0]
                      ?.venues?.[0]?.name || "Unknown Venue",
                  date: new Date().toISOString(),
                },
                song: {
                  title:
                    topVote.setlist_songs?.[0]?.songs?.[0]?.title ||
                    "Unknown Song",
              ...
              if (emailResult.success) {
                ...
                results.failed++;
                results.errors.push(
                  `Milestone email for ${user.email}: ${emailResult.error?.message}`,
                );
              }
            }
          }
    ...
    for (const show of newShows || []) {
      ...
      for (const user of interestedUsers || []) {
        ...
        if (emailResult.success) {
          ...
          results.failed++;
          results.errors.push(
            `New show notification for ${user.email}: ${emailResult.error?.message}`,
          );
        }
      }
    }
  ...
  private async processWeeklyDigest(results: any) {
    ...
    for (const user of users || []) {
      ...
      const emailResult = await sendWeeklyDigestEmail({
        to: [
          { email: user.users?.[0]?.email, name: personalizedData.userName },
        ],
        userName: personalizedData.userName,
        weekOf: new Date().toLocaleDateString(),
        followedArtists: personalizedData.preferences.favoriteArtists.map(
      ...
      if (emailResult.success) {
        ...
        results.failed++;
        results.errors.push(
          `Weekly digest for ${user.users?.[0]?.email}: ${emailResult.error?.message}`,
        );
      }
    }
  }
  ...
  public async runABTest(
    ...
    if (existingAssignment) {
      return {
        ...
        template:
          campaign.abTest.variants.find(
            (v: any) => v.name === existingAssignment.variant,
          )?.template || "default",
      };
    }
...
export async function getPersonalizedEmailData(
  userId: string,
): Promise<PersonalizedEmailData> {
...
export async function trackEmailEngagement(
  ...
  return emailAutomationEngine.trackEmailEngagement(
    emailId,
    userId,
    action,
    metadata,
