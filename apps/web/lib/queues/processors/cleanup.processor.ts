...
async function cleanupOldJobs(maxAge: number): Promise<number> {
  ...
  for (const queueName of Object.values(QueueName)) {
    try {
      ...
      console.log(
        `Cleaned ${completedCleaned.length + failedCleaned.length} jobs from queue ${queueName}`,
      );
    } catch (error) {
...
async function cleanupFailedJobs(maxAge: number): Promise<number> {
  ...
  for (const queueName of Object.values(QueueName)) {
    try {
      ...
      console.log(
        `Cleaned ${failedCleaned.length} failed jobs from queue ${queueName}`,
      );
    } catch (error) {
      console.error(
...
async function cleanupImportLogs(
  maxAge: number,
  batchSize: number,
): Promise<number> {
  try {
