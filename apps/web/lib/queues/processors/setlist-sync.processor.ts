import { artists, db, setlistSongs, setlists, songs } from "@repo/database";
  ...
  try {
    ...
    for (let i = 0; i < setlistsData.length; i++) {
      ...
      if (setlistId) {
        ...
        const songCount = await processSongsInSetlist(
          setlistId,
          setlistData,
          artistId,
        );
    ...
    await job.log(
      `Setlist sync completed: ${setlistsCreated} setlists, ${songsCreated} songs`,
    );
...
async function processSongsInSetlist(
  setlistId: string,
  setlistData: any,
  artistId: string,
): Promise<number> {
