...
export async function processSpotifyCatalog(
  job: SimpleJob<{
    artistId: string;
    spotifyId: string;
    deep?: boolean;
  ...
  try {
    await job.log(
      `Starting ${deep ? "deep" : "standard"} catalog sync for artist ${artistId}`,
    );
    ...
    await job.log(
      `Catalog sync completed: ${result.totalSongs} songs imported`,
    );
