...
import { QueueName, type SpotifySyncJob, createWorker } from "../config";
...
async function syncArtistAlbums(artistId: string, spotifyId: string, job: Job) {
  const spotify = new SpotifyClient({});
  await spotify.authenticate();
...
async function syncFullCatalog(
  ...
  await job.updateProgress(30);
  const catalogResult = await syncArtistTracks(
    artistId,
    spotifyId,
    job,
    options,
