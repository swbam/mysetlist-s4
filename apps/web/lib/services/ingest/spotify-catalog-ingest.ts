 ...
 * Handles studio-only catalog with liveness filtering and ISRC deduplication
 */
...
// Live track patterns to filter out
const LIVE_PATTERNS = [
...
/**
 * Main Spotify catalog ingest service for robust data processing
 */
export class SpotifyCatalogIngest {
  ...
  /**
   * Ingest studio-only catalog for an artist with comprehensive filtering
   */
      ...
      await processBatch(
        ...
        },
        {
          ...
          onProgress: (completed, total) => {
            const progress = 15 + Math.floor((completed / total) * 20);
            reporter?.report(
              'importing-songs',
  ...
  private filterStudioTracks(
    ...
    for (const track of tracks) {
      ...
      // Filter by audio features (liveness threshold)
  ...
  /**
   * Ingest a single track into the database with robust error handling
   */
