...
async function testTicketmaster() {
  ...
  try {
    ...
    console.log(
      `Found ${searchResult.attractions.length} attractions for "Taylor Swift"`,
    );

    if (searchResult.attractions.length > 0) {
      const firstAttraction = searchResult.attractions[0];
      console.log(
        `First result: ${firstAttraction?.name} (ID: ${firstAttraction?.id})`,
      );
      ...
      try {
        for await (const events of iterateEventsByAttraction(
          firstAttraction?.id || "",
        )) {
          pageCount++;
          eventCount += events.length;
          console.log(
            `Page ${pageCount}: Retrieved ${events.length} events (total: ${eventCount})`,
          );
        ...
        console.log(
          `Total events found: ${eventCount} across ${pageCount} pages`,
        );
      } catch (error: any) {
...
async function testSpotify() {
  ...
  try {
    ...
    console.log(
      `Found ${searchResult.artists.length} artists for "Taylor Swift"`,
    );
    ...
    if (searchResult.artists.length > 0) {
      ...
      console.log(
        `Popularity: ${firstArtist.popularity}, Followers: ${firstArtist.followers.total}`,
      );
      ...
      if (albums.length > 0) {
        ...
        if (albumWithTracks) {
          console.log(
            `Testing with album: "${albumWithTracks.name}" (${albumWithTracks.total_tracks} tracks)`,
          );
          ...
          if (tracks.length > 0) {
            ...
            console.log(
              `Retrieved details for ${trackDetails.length}/${testTrackIds.length} tracks`,
            );
            ...
            console.log(
              `Retrieved audio features for ${audioFeatures.length}/${testTrackIds.length} tracks`,
            );
            ...
            if (audioFeatures.length > 0) {
              ...
              if (firstFeature) {
                console.log(
                  `Sample audio features - Liveness: ${firstFeature.liveness}, Energy: ${firstFeature.energy}, Danceability: ${firstFeature.danceability}`,
                );
              }
            }
          }
...
async function testErrorHandling() {
  ...
  try {
    ...
    const events: any[] = [];
    for await (const eventPage of iterateEventsByAttraction(
      "invalid-id-12345",
    )) {
  ...
  try {
    ...
    console.log(
      `Empty batches handled correctly: ${emptyDetails.length} details, ${emptyFeatures.length} features`,
    );
  } catch (error: any) {
...
export { testTicketmaster, testSpotify, testErrorHandling, runTests };
