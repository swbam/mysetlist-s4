...
export class ArtistImportOrchestrator {
  ...
  async initiateImport(
    ...
    try {
      ...
      const attraction = await getAttraction(
        tmAttractionId,
        this.config.ticketmasterApiKey,
      );

      if (!attraction) {
        throw new Error(
          `Attraction ${tmAttractionId} not found in Ticketmaster`,
        );
      }
      ...
      // Get best image
      const imageUrl =
        attraction.images?.find((img: any) => img.width && img.width >= 500)
          ?.url ||
      ...
      throw new Error(
        `Failed to initialize artist import: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
    ...
    try {
      ...
      if (this.progressReporter) {
        await this.progressReporter.report(
          "importing-shows",
          25,
          "Starting shows and venues import...",
        );
      }

      if (artist.tmAttractionId) {
        const showStats = await this.importShowsAndVenues(
          artistId,
      ...
      if (this.progressReporter) {
        await this.progressReporter.report(
          "importing-shows",
          70,
          `Imported ${stats.showsImported} shows`,
        );
      }
      ...
      if (this.progressReporter) {
        await this.progressReporter.report(
          "importing-songs",
          75,
          "Starting studio catalog import...",
        );
      }

      if (artist.spotifyId) {
        const catalogStats = await this.importStudioCatalog(
          artistId,
          artist.spotifyId,
        );
  ...
  private async importShowsAndVenues(
    ...
    try {
      ...
      for await (const events of this.iterateTicketmasterEvents(
        tmAttractionId,
        ticketmasterApiKey,
      )) {
        ...
        const showData = this.extractShowsFromEvents(
          events,
          venueMap,
          artistId,
        );
  ...
  private async importStudioCatalog(
    ...
    const { spotifyClientId, spotifyClientSecret, livenessThreshold } =
      this.config;
    ...
    try {
      ...
      const studioAlbums = albums.filter(
        (album) => !isLikelyLiveAlbum(album.name),
      );
      ...
      const trackIds = Array.from(new Set(allTracks.map((t) => t.id))).filter(
        Boolean,
      );
      ...
      const songsImported = await this.upsertSongs(
        artistId,
        deduplicatedTracks,
        featuresMap,
      );
   ...
   */
  private async runWrapUpPhase(
    artistId: string,
  ): Promise<{ setlistsCreated: number }> {
    try {
      if (this.progressReporter) {
        await this.progressReporter.report(
          "creating-setlists",
      ...
      const setlistResult = await setlistPreseeder.preseedSetlistsForArtist(
        artistId,
        {
          songsPerSetlist: 5,
      ...
      if (this.progressReporter) {
        await this.progressReporter.report(
          "creating-setlists",
          100,
          "Wrap-up phase completed successfully",
        );
      }
   ...
   */
  private async *iterateTicketmasterEvents(
    attractionId: string,
    apiKey: string,
  ) {
   ...
   */
  private extractShowsFromEvents(
    events: any[],
   ...
   */
  private async getSpotifyAlbums(
    artistId: string,
    token: string,
  ): Promise<any[]> {
   ...
   */
  private async getTracksDetails(
    trackIds: string[],
    token: string,
  ): Promise<any[]> {
    const tracks: any[] = [];
