...
let createRateLimiter: (props: { limit: number; window: string; prefix?: string }) => ReturnType<typeof createSimpleRateLimiter>;
...
try {
  ...
  console.warn('Database rate limiter not available, using in-memory rate limiter');
  createRateLimiter = (props: { limit: number; window: string; prefix?: string }) => {
    ...
    switch (unit) {
      case 'm': windowMs = num * 60000; break;
      case 'h': windowMs = num * 3600000; break;
      case 'd': windowMs = num * 86400000; break;
    }
...
export function getApiIdentifier(request: NextRequest, userId?: string): string {
