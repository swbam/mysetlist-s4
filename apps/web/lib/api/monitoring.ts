...
export class MonitoringService {
  ...
  metric(name: string, value: number, tags?: Record<string, string>, unit?: string) {
    ...
    this.cache.zadd(`metrics:${name}:timeseries`, Date.now(), JSON.stringify(entry));
  }
  ...
  async trackAnalytics(event: string, properties?: Record<string, any>, context?: Partial<AnalyticsEvent>) {
  ...
  trackRequest(req: any, res: any, duration: number) {
    ...
    this.metric('http_request_duration_ms', duration, {
      method: req.method,
      endpoint: req.url
    }, 'ms');
  }
  ...
  trackPerformance(name: string, value: number, rating: 'good' | 'needs-improvement' | 'poor') {
    this.metric(`performance_${name}`, value, { rating }, 'ms');
  }

  // Database query monitoring
  trackDatabaseQuery(query: string, duration: number, success: boolean) {
    this.metric('database_query_duration_ms', duration, {
  ...
  trackExternalAPI(service: string, endpoint: string, duration: number, success: boolean, statusCode?: number) {
    this.metric(`external_api_duration_ms`, duration, {
      service,
      endpoint,
      success: success.toString(),
  ...
  async getMetrics(name: string, timeRange: number = 3600000): Promise<MetricEntry[]> {
    try {
      const cutoff = Date.now() - timeRange;
      const results = await this.cache.zrange(`metrics:${name}:timeseries`, cutoff, Date.now());
...
export const monitor = {
  ...
  trackAnalytics: (event: string, properties?: Record<string, any>, context?: Partial<AnalyticsEvent>) => 
  ...
  trackExternalAPI: (service: string, endpoint: string, duration: number, success: boolean, statusCode?: number) => 
    monitoringService.trackExternalAPI(service, endpoint, duration, success, statusCode),
  trackSystemMetrics: () => 
