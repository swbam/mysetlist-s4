...
// Job configurations
const JOB_CONFIGS: Record<JobType, {
  maxAttempts: number;
  ...
  user_activity_analysis: { maxAttempts: 2, retryDelay: 240000, timeout: 300000 },
};
...
export class BackgroundJobQueue {
  ...
  private registerDefaultProcessors() {
    ...
    this.processors.set('analytics_processing', new AnalyticsProcessingProcessor());
    this.processors.set('cache_warming', new CacheWarmingProcessor());
    this.processors.set('cleanup', new CleanupProcessor());
    this.processors.set('trending_calculation', new TrendingCalculationProcessor());
    this.processors.set('user_activity_analysis', new UserActivityAnalysisProcessor());
  }
  ...
  private calculateJobScore(job: Job): number {
    ...
    const priorityWeight = job.priority === 'high' ? 1000 : job.priority === 'medium' ? 500 : 100;
  ...
  private async processJobs() {
    try {
      ...
      await Promise.all(
        concurrentJobs.map(jobId => this.processJob(jobId))
      );
  ...
  async getQueueStats() {
    const stats = {
      pending: await this.cache.pipeline([
        ['ZCARD', 'job_queue']
      ]),
      processing: 0,
      completed: 0,
