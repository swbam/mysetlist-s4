...
export const DB_PERFORMANCE_TARGETS = {
  ...
  CACHE_HIT_RATIO: 0.9, // 90% cache hit rate
...
export class DatabasePerformanceTester {
  ...
  private async testQuery(
    ...
    try {
      ...
      console.log(
        `Query ${queryName}: ${executionTime.toFixed(2)}ms (target: ${targetTime}ms) - ${meetsTarget ? "PASS" : "FAIL"}`,
      );
    } catch (error) {
    ...
    if (executionTime > targetTime) {
      ...
      if (slownessFactor > 3) {
        recommendations.push(
          "Query is critically slow - consider major optimization",
        );
      } else if (slownessFactor > 2) {
        recommendations.push("Query performance needs improvement");
      } else {
        recommendations.push(
          "Query slightly exceeds target - minor optimization needed",
        );
      }
      ...
      if (queryName.includes("with_stats")) {
        recommendations.push(
          "Consider denormalizing frequently accessed statistics",
        );
  ...
  private async analyzeIndexes(): Promise<IndexAnalysisResult[]> {
    try {
      ...
      for (const row of (indexStats as any).rows || []) {
        const effectiveness =
          row.tuples_fetched > 0
            ? row.tuples_fetched / Math.max(row.tuples_read, 1)
            : 0;
        ...
        if (row.scans === 0) {
          ...
          recommendations.push(
            "Low index effectiveness - review query patterns",
          );
  ...
  private async getConnectionMetrics(): Promise<ConnectionMetrics> {
    try {
      ...
      return {
        totalConnections: Number.parseInt(row.total_connections) || 0,
        activeConnections: Number.parseInt(row.active_connections) || 0,
        idleConnections: Number.parseInt(row.idle_connections) || 0,
  ...
  private generateReport(
    ...
    const overallScore =
      totalQueries > 0 ? (passedQueries / totalQueries) * 100 : 0;

    const criticalIssues: string[] = [];
    const recommendations: string[] = [];

    // Analyze results for critical issues
    const criticallySlowQueries = this.results.filter(
      (r) => r.executionTime > 0 && r.executionTime > 1000, // Over 1 second
    );

    if (criticallySlowQueries.length > 0) {
      criticalIssues.push(
        `${criticallySlowQueries.length} queries are critically slow (>1s)`,
      );
    }

    if (overallScore < 70) {
      criticalIssues.push(
        "Overall database performance is below acceptable threshold",
      );
    }

    if (
      connectionMetrics.totalConnections >
      DB_PERFORMANCE_TARGETS.MAX_CONNECTIONS
    ) {
...
export class DatabaseOptimizer {
    ...
    try {
      ...
      for (const indexQuery of indexQueries) {
        try {
          ...
          const indexName =
            indexQuery.match(/idx_[a-zA-Z0-9_]+/)?.[0] || "unknown";
  ...
  async analyzeQueryPlans(): Promise<void> {
    try {
      ...
      console.warn(
        "Could not analyze query plans (pg_stat_statements may not be enabled):",
        error,
      );
    }
  }
  ...
  async optimizeSettings(): Promise<string[]> {
    ...
    try {
      ...
      optimizations.push(
        "Database settings optimization recommendations generated",
      );
