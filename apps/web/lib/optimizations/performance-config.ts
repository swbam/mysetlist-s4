    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      try {
        ...
        performanceMonitor.recordMetric({
          ...
          errorRate:
            (batchResults.length - successfulResults.length) /
            batchResults.length,
        });
      } catch (error) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        if (attempt < maxRetries) {
          const delay = backoffMs * 2 ** attempt; // Exponential backoff
...
export function checkBundleSize(
  ...
  const target =
    PERFORMANCE_CONFIG.BUNDLE_TARGETS[
      bundleName as keyof typeof PERFORMANCE_CONFIG.BUNDLE_TARGETS
    ];
