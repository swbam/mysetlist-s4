...
class MLRecommendationEngine {
  ...
  public async computeUserVector(userId: string): Promise<UserVector> {
    if (this.userVectorCache.has(userId)) {
      ...
      if (Date.now() - cached.lastUpdated.getTime() < 5 * 60 * 1000) { // 5 minutes
        return cached;
      }
    }
    ...
    const genrePreferences = this.computeGenrePreferences(votes || [], attendance || [], follows || []);
    
    // Process artist preferences
    const artistPreferences = this.computeArtistPreferences(votes || [], attendance || [], follows || []);
    
    // Process venue preferences
    const venuePreferences = this.computeVenuePreferences(votes || [], attendance || []);
    
    // Process temporal preferences
    const timePreferences = this.computeTimePreferences(votes || [], attendance || []);
    
    // Process location preferences
    const locationPreferences = this.computeLocationPreferences(attendance || []);
    
    // Process seasonal preferences
    const seasonalPreferences = this.computeSeasonalPreferences(votes || [], attendance || []);
  ...
  public async getCollaborativeRecommendations(
    ...
    const similarUsers = await this.findSimilarUsers(userId, config.minSimilarity);
          ...
          if (show && !seenItems.has(show.id)) {
            ...
            recommendations.push({
              ...
              explanation: {
                ...
                confidence: score,
                factors: [
                  { factor: 'user_similarity', weight: 0.7, contribution: similarUser.similarity },
                  { factor: 'vote_value', weight: 0.3, contribution: item.vote_value / 5 }
                ]
              },
        ...
        for (const show of genreShows) {
          ...
          recommendations.push({
            ...
            explanation: {
              ...
              confidence: score,
              factors: [
                { factor: 'genre_match', weight: 0.5, contribution: score * 0.5 },
                { factor: 'artist_preference', weight: 0.3, contribution: score * 0.3 },
                { factor: 'venue_preference', weight: 0.2, contribution: score * 0.2 }
              ]
            },
            diversity_score: this.calculateDiversityScore(show, userVector),
            freshness_score: this.calculateFreshnessScore(show.date)
  ...
  public async getHybridRecommendations(
    ...
    const [collaborative, contentBased] = await Promise.all([
      this.getCollaborativeRecommendations(userId, { ...config, limit: config.limit * 2 }),
      this.getContentBasedRecommendations(userId, { ...config, limit: config.limit * 2 })
    ]);
    ...
    for (const rec of contentBased) {
      if (combinedRecommendations.has(rec.id)) {
        ...
        existing.explanation.confidence = Math.max(existing.explanation.confidence, rec.explanation.confidence);
      } else {
        combinedRecommendations.set(rec.id, {
          ...rec,
          score: rec.score * 0.4,
    ...
    return this.rankAndDiversify(Array.from(combinedRecommendations.values()), config);
  }
        ...
        if (score > config.minSimilarity) {
          recommendations.push({
            ...
            explanation: {
              ...
              reason: 'Discovered through latent factor analysis of your preferences',
              confidence: score,
              factors: [
                { factor: 'latent_factor_1', weight: 0.4, contribution: score * 0.4 },
                { factor: 'latent_factor_2', weight: 0.3, contribution: score * 0.3 },
                { factor: 'latent_factor_3', weight: 0.3, contribution: score * 0.3 }
              ]
            },
            diversity_score: this.calculateDiversityScore(show, userVector),
            freshness_score: this.calculateFreshnessScore(show.date)
  ...
  // Helper methods
  private computeVotingBehavior(votes: any[]): UserVector['features']['votingBehavior'] {
    if (votes.length === 0) {
      return { avgVoteValue: 3, voteFrequency: 0, popularityBias: 0.5 };
    }

    const avgVoteValue = votes.reduce((sum, vote) => sum + vote.vote_value, 0) / votes.length;
  ...
  private computeGenrePreferences(votes: any[], attendance: any[], follows: any[]): Record<string, number> {
  ...
  private computeArtistPreferences(votes: any[], attendance: any[], follows: any[]): Record<string, number> {
    ...
    votes.forEach(vote => {
      ...
      if (artistId) {
        artistCounts[artistId] = (artistCounts[artistId] || 0) + vote.vote_value;
      }
    });
  ...
  private computeVenuePreferences(votes: any[], attendance: any[]): Record<string, number> {
  ...
  private computeTimePreferences(votes: any[], attendance: any[]): Record<string, number> {
  ...
  private computeLocationPreferences(attendance: any[]): Record<string, number> {
  ...
  private computeSeasonalPreferences(votes: any[], attendance: any[]): Record<string, number> {
  ...
  private async findSimilarUsers(userId: string, minSimilarity: number): Promise<SimilarityMatrix[]> {
    ...
    return [
      { userA: userId, userB: 'user1', similarity: 0.85, sharedItems: 15, lastCalculated: new Date() },
      { userA: userId, userB: 'user2', similarity: 0.72, sharedItems: 12, lastCalculated: new Date() },
      { userA: userId, userB: 'user3', similarity: 0.68, sharedItems: 8, lastCalculated: new Date() }
  ...
  private calculateContentScore(show: any, userVector: UserVector): number {
    ...
    const genreScore = showGenres.reduce((sum: number, genre: string) => {
  ...
  private calculateDiversityScore(show: any, userVector: UserVector): number {
    ...
    const genreOverlap = showGenres.filter((genre: string) => topUserGenres.includes(genre)).length;
  ...
  private calculateFreshnessScore(date: string): number {
    ...
    const now = new Date();
    const daysDiff = (showDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24);
  ...
  private rankAndDiversify(recommendations: MLRecommendation[], config: MLRecommendationConfig): MLRecommendation[] {
    // Sort by score
    ...
    for (const rec of recommendations) {
      ...
      const diversityPenalty = (artistCount + venueCount) * config.diversityFactor;
...
export async function getMLRecommendations(
  ...
  switch (config.algorithm) {
    case 'collaborative':
      return mlRecommendationEngine.getCollaborativeRecommendations(userId, config);
    case 'content':
      return mlRecommendationEngine.getContentBasedRecommendations(userId, config);
    case 'matrix_factorization':
      return mlRecommendationEngine.getMatrixFactorizationRecommendations(userId, config);
    case 'hybrid':
    default:
