...
// Mock moderator user
export const createMockModeratorUser = (overrides: Partial<MockUser> = {}): MockUser =>
  createMockUser({
    role: 'moderator',
    user_metadata: {
...
export const createMockSupabaseAuth = (authState: 'authenticated' | 'unauthenticated' | 'loading' = 'authenticated') => {
  const states = {
...
export const mockAuthError = (type: 'invalid_credentials' | 'email_not_confirmed' | 'network_error' = 'invalid_credentials') => {
  const errors = {
    invalid_credentials: {
...
export const mockAuthProvider = (authState: 'authenticated' | 'unauthenticated' | 'loading' = 'authenticated') => {
...
export const mockFailedLogin = (errorType: 'invalid_credentials' | 'email_not_confirmed' = 'invalid_credentials') => {
...
export const mockUserWithRole = (role: 'user' | 'admin' | 'moderator' = 'user') => {
  const roleConfigs = {
    user: createMockUser(),
...
export const mockAuthRequest = (method: 'GET' | 'POST' | 'PUT' | 'DELETE' = 'POST', authenticated = true) => {
...
export const mockAuthMiddleware = (authenticated = true, role: 'user' | 'admin' | 'moderator' = 'user') => {
  if (!authenticated) {
