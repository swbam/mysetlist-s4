  ...
  return new Promise((resolve, reject) => {
    ...
    child.stdout.on("data", (data) => {
      ...
      if (
        ...
        stdout.includes("started server on")
      ) {
...
async function benchmarkTurbopack() {
  ...
  try {
    ...
    const improvement = (
      ((webpackResult.duration - turbopackResult.duration) /
        webpackResult.duration) *
      100
    ).toFixed(1);
    ...
    if (improvement > 0) {
      ...
      console.log(
        "   This may indicate Turbopack configuration needs optimization",
      );
    }
  } catch (error) {
...
async function main() {
  ...
  if (process.argv.includes("--benchmark")) {
    ...
    console.log(
      "To run performance benchmark, use: node scripts/benchmark-turbopack.js --benchmark",
    );
