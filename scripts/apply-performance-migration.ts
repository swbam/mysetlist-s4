...
async function main() {
  ...
  try {
    ...
    const migrationPath = join(
      ...
      "database",
      "migrations",
      "0009_comprehensive_performance_optimization.sql",
    );
    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      try {
        ...
        console.log(
          `✅ [${i + 1}/${statements.length}] Success: ${preview}${statement.length > 80 ? "..." : ""}`,
        );
        ...
        const preview = statement.substring(0, 80).replace(/\s+/g, " ").trim();
        console.error(
          `❌ [${i + 1}/${statements.length}] Error: ${preview}${statement.length > 80 ? "..." : ""}`,
        );
    ...
    console.log("\n📋 Next steps:");
    console.log(
      "1. Run refresh_performance_caches() every 15 minutes via cron",
    );
