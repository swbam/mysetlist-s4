...
class DisasterRecoveryManager {
    ...
    try {
      ...
      for (const table of tables) {
        ...
        try {
          ...
          console.warn(`⚠️  Warning: Error backing up table ${table}:`, tableError);
      ...
      backupData.metadata.checksum = await this.calculateChecksum(JSON.stringify(backupData));
      ...
      console.log(`💾 Size: ${(backupData.metadata.size / 1024 / 1024).toFixed(2)} MB`);
      ...
      for (const table of tables) {
        ...
        try {
          ...
          if (data && data.length > 0) {
            backupData.data[table] = data;
            totalRecords += data.length;
            console.log(`✅ Backed up ${data.length} recent records from ${table}`);
          }
        } catch (tableError) {
          console.warn(`⚠️  Warning: Error backing up table ${table}:`, tableError);
        }
   ...
   */
  async restoreFromBackup(backupId: string, options: { dryRun?: boolean; tables?: string[] } = {}): Promise<void> {
    const backupPath = path.join(this.config.backupDirectory, `${backupId}.json`);
    ...
    try {
      ...
      const calculatedChecksum = await this.calculateChecksum(JSON.stringify(backup));
      ...
      const tablesToRestore = options.tables || Object.keys(backup.data).filter(key => !key.endsWith('_error'));
      ...
      for (const table of tablesToRestore) {
        ...
        console.log(`🔄 Restoring table: ${table} (${backup.data[table].length} records)`);
        ...
        try {
          ...
          if (deleteError) {
            console.warn(`⚠️  Warning: Could not clear table ${table}: ${deleteError.message}`);
          }
          ...
          for (let i = 0; i < records.length; i += batchSize) {
            ...
            if (insertError) {
              console.error(`❌ Error inserting batch for table ${table}:`, insertError);
              throw insertError;
            }
          }
  ...
  async validateSystem(): Promise<{ healthy: boolean; issues: string[] }> {
    ...
    try {
      ...
      const criticalTables = ['users', 'artists', 'venues', 'shows', 'setlists'];
      ...
      for (const table of criticalTables) {
        try {
          ...
          if (error) {
            issues.push(`Critical table ${table} is not accessible: ${error.message}`);
    ...
    try {
      ...
      for (const file of backupFiles) {
        try {
          ...
          console.warn(`⚠️  Warning: Could not process file ${file}:`, fileError);
        }
      }
  ...
  private async getAllTables(): Promise<string[]> {
    ...
    if (error) {
      ...
      return [
        'users', 'artists', 'venues', 'shows', 'setlists', 'songs', 'setlist_songs',
        'votes', 'user_follows_artists', 'user_show_attendance', 'artist_stats',
        'venue_stats', 'show_stats', 'backup_logs', 'health_logs'
      ];
    }
  ...
  private async getTablesWithTimestamps(): Promise<string[]> {
    ...
    return [
      'users', 'artists', 'venues', 'shows', 'setlists', 'songs', 'setlist_songs',
      'votes', 'user_follows_artists', 'user_show_attendance'
    ];
  }
  ...
  private async logRestoreOperation(backupId: string, tables: string[]): Promise<void> {
    try {
      await this.supabase
        .from('restore_logs')
        .insert({
...
program
  ...
  .option('-t, --type <type>', 'Backup type (full|incremental|schema)', 'incremental')
