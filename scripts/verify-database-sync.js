  ...
  for (const [tableName, description] of Object.entries(tables)) {
    try {
      ...
      if (error) {
        ...
        results[tableName] = { count: recordCount, hasData, status: hasData ? '✅' : '⚠️ ' };
        console.log(`${hasData ? '✅' : '⚠️ '} ${description}: ${recordCount} records`);
      }
    } catch (error) {
      results[tableName] = { error: error.message, status: '❌' };
...
async function runVerification() {
  ...
  if (tableCheck.hasAnyData) {
    ...
    console.log('   1. Run artist import: node scripts/test-artist-import-simple.js');
