...
async function testRLSEnabled(): Promise<void> {
  ...
  const { data, error } = await serviceClient
    ...
    .in('tablename', ['user_activity_log', 'trending_artists', 'trending_shows', 'pipeline_jobs', 'schema_migrations']);
...
async function testServiceRoleAccess(): Promise<void> {
  ...
  const tables = ['user_activity_log', 'trending_artists', 'trending_shows', 'pipeline_jobs', 'schema_migrations'];
...
async function testUserActivityLogPolicies(): Promise<void> {
  ...
  const { data: { user }, error: signUpError } = await anonClient.auth.signUp({
    email: `test-${Date.now()}@example.com`,
    password: 'testpassword123'
  });

  if (signUpError || !user) {
    console.log('⚠️  Skipping user activity log tests (cannot create test user)');
    return;
  }
  ...
  await runTest('User cannot view other users activity logs', async () => {
    ...
    if (error) throw error;
    if (data && data.length > 0) throw new Error('Should not see other users logs');
  });
}
