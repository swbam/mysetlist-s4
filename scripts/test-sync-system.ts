...
async function testArtistImport() {
  ...
  try {
    ...
    if (success && result.artistId) {
      ...
      await logResult('Artist Database Creation', dbSuccess, { artist: artist?.name });
    ...
    await logResult('Artist Import API', false, { error: error instanceof Error ? error.message : 'Unknown error' });
    return { success: false };
  }
}
...
async function testCronJobTriggering() {
  ...
  for (const endpoint of cronEndpoints) {
    try {
      ...
      await logResult(`Cron Job ${endpoint}`, false, { error: error instanceof Error ? error.message : 'Unknown error' });
    }
  }
...
async function testOrchestratedSync() {
  ...
  try {
    ...
    await logResult('Orchestrated Sync', false, { error: error instanceof Error ? error.message : 'Unknown error' });
    return { success: false };
  }
}
...
async function testDatabaseDataCreation() {
  ...
  for (const table of tables) {
    try {
      ...
      results[table] = { error: error instanceof Error ? error.message : 'Unknown error' };
    }
  }
...
async function testCronJobLogs() {
  ...
  try {
    ...
    await logResult('Cron Job Logs', false, { error: error instanceof Error ? error.message : 'Unknown error' });
    return { success: false };
  }
}
...
async function testSyncFunctions() {
  ...
  try {
    ...
    const { triggerManualSync, triggerTrendingUpdate } = await import('../apps/web/lib/sync-functions');
    ...
    await logResult('Sync Functions - Trending Update', trendingSuccess, trendingResult);
    
    return { success: trendingSuccess };
  } catch (error) {
    await logResult('Sync Functions', false, { error: error instanceof Error ? error.message : 'Unknown error' });
    return { success: false };
  }
}
