...
async function checkDataStatus() {
  try {
    ...
    if (response.ok) {
      ...
      console.log(`  Artists with trending scores: ${data.stats.trendingArtists}`);
      ...
      if (data.stats.topArtists?.length > 0) {
        ...
        data.stats.topArtists.forEach((artist: any, index: number) => {
          console.log(`    ${index + 1}. ${artist.name} (Score: ${artist.trendingScore?.toFixed(2)})`);
        });
      }
...
async function initializeData() {
  ...
  try {
    ...
    if (result.summary.topTrending.artists?.length > 0) {
      console.log('\n🔥 Top Trending Artists:');
      result.summary.topTrending.artists.forEach((artist: any, index: number) => {
        console.log(`  ${index + 1}. ${artist.name} (Score: ${artist.trendingScore?.toFixed(2)}, Popularity: ${artist.popularity})`);
      });
    }
    ...
    if (result.summary.topTrending.shows?.length > 0) {
      ...
      result.summary.topTrending.shows.forEach((show: any, index: number) => {
        ...
        console.log(`  ${index + 1}. ${show.name} on ${date} (Score: ${show.trendingScore?.toFixed(2)})`);
      });
    }
...
async function syncRealArtists() {
  ...
  try {
    ...
    if (!response.ok) {
      console.log('⚠️  Could not sync real artists (API keys may not be configured)');
      return;
    }
    ...
    if (result.success && result.syncedCount > 0) {
      ...
      if (result.trendingArtists?.length > 0) {
        ...
        result.trendingArtists.slice(0, 5).forEach((artist: any, index: number) => {
          console.log(`    ${index + 1}. ${artist.name} (${artist.upcomingShows} shows)`);
        });
      }
    }
  } catch (error) {
...
async function main() {
  ...
  if (hasData) {
    ...
    const answer = await new Promise<string>((resolve) => {
      rl.question('\nDo you want to sync real artists from Ticketmaster? (y/N): ', resolve);
    });
    rl.close();
  ...
  console.log(`    curl -X POST ${APP_URL}/api/admin/initialize-data${ADMIN_KEY ? ' -H "Authorization: Bearer $ADMIN_API_KEY"' : ''}`);
  console.log('\n  Sync real artists:');
  console.log(`    curl ${APP_URL}/api/artists/sync`);
  console.log('\n  Update trending scores:');
  console.log(`    curl -X POST ${APP_URL}/api/admin/calculate-trending${ADMIN_KEY ? ' -H "Authorization: Bearer $ADMIN_API_KEY"' : ''}`);
}
