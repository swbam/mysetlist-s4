...
async function testExternalAPIs() {
  ...
  // Test Spotify API
  if (
    ENV_VARS.SPOTIFY_CLIENT_ID.value &&
    ENV_VARS.SPOTIFY_CLIENT_SECRET.value
  ) {
    try {
...
async function runAllChecks() {
  ...
  console.log("=".repeat(50));
  console.log(
    `Environment Variables: ${envCheck.allValid ? "✅ Valid" : "❌ Issues"}`,
  );
  ...
  console.log(
    `Supabase Connection: ${supabaseCheck ? "✅ Working" : "❌ Failed"}`,
  );

  const apiResults = Object.entries(apiChecks)
    .map(([api, success]) => `${api}: ${success ? "✅" : "❌"}`)
    .join(", ");
  ...
  const allGood =
    envCheck.allValid &&
    supabaseCheck &&
    Object.values(apiChecks).some(Boolean);
  ...
  if (allGood) {
    ...
    console.log(
      "⚠️  Some configuration issues found. Please check the details above.",
    );
  }
