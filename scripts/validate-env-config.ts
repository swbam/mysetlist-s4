...
function validatePackage(packageName: string, validationFn: () => { errors: string[]; warnings: string[] }) {
...
validatePackage('@repo/database', () => {
  ...
  if (!process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY) {
    errors.push('NEXT_PUBLIC_SUPABASE_ANON_KEY is required for Supabase client');
  }
...
validatePackage('@repo/auth', () => {
  ...
  if (!process.env.SPOTIFY_CLIENT_SECRET) {
    warnings.push('SPOTIFY_CLIENT_SECRET not set - Spotify OAuth will not work');
  }
  if (!process.env.NEXT_PUBLIC_SPOTIFY_CLIENT_ID) {
    warnings.push('NEXT_PUBLIC_SPOTIFY_CLIENT_ID not set - Spotify OAuth will not work');
  }
...
validatePackage('@repo/external-apis', () => {
  ...
  if (!process.env.NEXT_PUBLIC_SPOTIFY_CLIENT_ID) {
    warnings.push('NEXT_PUBLIC_SPOTIFY_CLIENT_ID not set - Spotify API will not work');
  }
  if (!process.env.TICKETMASTER_API_KEY) {
    warnings.push('TICKETMASTER_API_KEY not set - Ticketmaster API will not work');
  }
...
validatePackage('@repo/observability', () => {
  ...
  if (!process.env.BETTERSTACK_API_KEY) {
    warnings.push('BETTERSTACK_API_KEY not set - BetterStack monitoring disabled');
  }
...
validatePackage('@repo/feature-flags', () => {
  ...
  if (!process.env.FLAGSMITH_ENVIRONMENT_KEY) {
    warnings.push('FLAGSMITH_ENVIRONMENT_KEY not set - Feature flags will use defaults');
  }
...
validatePackage('@repo/rate-limit', () => {
  ...
  if (!process.env.REDIS_URL && !process.env.UPSTASH_REDIS_REST_URL) {
    warnings.push('Redis configuration missing - Rate limiting will use memory store');
  }
...
validatePackage('@repo/webhooks', () => {
  ...
  if (!process.env.TICKETMASTER_WEBHOOK_SECRET) {
    warnings.push('TICKETMASTER_WEBHOOK_SECRET not set - Ticketmaster webhooks disabled');
  }
...
if (failed > 0) {
  ...
  if (totalWarnings > 0) {
    console.log('⚠️  Some optional features may not work due to missing configuration.');
  }
  process.exit(0);
}