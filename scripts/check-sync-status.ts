...
interface SyncStatus {
  ...
  syncSummary: {
    masterSync: { runs: number; success: number; errors: number; lastRun: Date | null };
    autonomousSync: { runs: number; success: number; errors: number; lastRun: Date | null };
    trendingCalc: { runs: number; success: number; errors: number; lastRun: Date | null };
...
async function checkSyncStatus(hours = 24) {
  ...
  try {
    ...
    console.log(`  Synced: ${data.statistics.artists.synced.toLocaleString()} (${getPercentage(data.statistics.artists.synced, data.statistics.artists.total)}%)`);
    console.log(`  Need Sync: ${data.statistics.artists.needsSync.toLocaleString()}`);
    console.log(`  With Trending: ${data.statistics.artists.withTrending.toLocaleString()}`);
    
    console.log(`\nShows:`);
    console.log(`  Total: ${data.statistics.shows.total.toLocaleString()}`);
    console.log(`  Upcoming: ${data.statistics.shows.upcoming.toLocaleString()}`);
    console.log(`  Completed: ${data.statistics.shows.completed.toLocaleString()}`);
    console.log(`  Recently Added: ${data.statistics.shows.createdRecently.toLocaleString()}`);
    
    console.log(`\nVenues:`);
    console.log(`  Total: ${data.statistics.venues.total.toLocaleString()}`);
    console.log(`  With Shows: ${data.statistics.venues.withShows.toLocaleString()}`);
    ...
    if (data.trending.artists.length > 0 || data.trending.shows.length > 0) {
      ...
      if (data.trending.artists.length > 0) {
        ...
        data.trending.artists.slice(0, 5).forEach((artist, i) => {
          console.log(`  ${i + 1}. ${artist.name} (score: ${artist.trendingScore.toFixed(2)})`);
        });
      }
      ...
      if (data.trending.shows.length > 0) {
        ...
        data.trending.shows.slice(0, 5).forEach((show, i) => {
          console.log(`  ${i + 1}. ${show.name} (score: ${show.trendingScore.toFixed(2)})`);
        });
      }
    }
    ...
    if (data.healthScore.overall < 50) {
      ...
      console.log("   - Running a full sync: pnpm trigger-sync master-sync --mode full");
      ...
      if (data.healthScore.syncCoverage < 80) {
        console.log("   - Low sync coverage: Run discovery to find new artists");
      }
      if (data.healthScore.syncFreshness < 80) {
        console.log("   - Stale data: Increase sync frequency or run manual sync");
      }
    ...
    if (data.statistics.artists.needsSync > data.statistics.artists.total * 0.3) {
...
function displaySyncOperation(name: string, data: any) {
  ...
  const successRate = data.runs > 0 ? Math.round((data.success / data.runs) * 100) : 0;
  const status = successRate >= 80 ? "✅" : successRate >= 50 ? "⚠️ " : "❌";
  
  console.log(`\n${name}:`);
  console.log(`  Runs: ${data.runs} (${data.success} success, ${data.errors} errors)`);
