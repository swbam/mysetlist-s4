...
async function addDiscoveredArtists(discoveredArtists, source) {
  ...
  console.log(`📥 Adding ${discoveredArtists.length} ${source} artists to database...`);
...
async function getTrendingData() {
  ...
  try {
    ...
    trendingShows.forEach((show, i) => {
      console.log(`  ${i + 1}. ${show.name} by ${show.artist_name} (score: ${show.trending_score?.toFixed(2)})`);
    });
...
async function main() {
  try {
    ...
    console.log(`📈 Trending: ${trendingData.artists} artists, ${trendingData.shows} shows`);
