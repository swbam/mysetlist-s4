...
async function triggerSync(options: SyncOptions) {
  ...
  try {
    ...
    if (options.endpoint === "autonomous-discovery" && result.discovery) {
      ...
      if (result.sync) {
        console.log("\n🔄 Sync Results:");
        console.log(`   Artists: ${result.sync.artists.processed} processed, ${result.sync.artists.updated} updated`);
        console.log(`   Shows: ${result.sync.shows.processed} processed, ${result.sync.shows.added} added`);
      }
      ...
      console.log(`   Popular Artists: ${result.phases.popularArtists.synced}/${result.phases.popularArtists.processed} synced`);
...
// Check environment
if (!CRON_SECRET) {
  console.warn("⚠️  Warning: CRON_SECRET not set - requests may be unauthorized");
}
