...
async function executeMigration() {
  ...
  try {
    ...
    const migrationPath = join(process.cwd(), 'supabase/migrations/20250128_security_fixes.sql')
    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      if (error) {
        ...
        console.error('')
        console.error('Please execute the migration manually in the Supabase SQL Editor:')
