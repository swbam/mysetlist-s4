...
async function deployFunctions() {
  ...
  for (const func of EDGE_FUNCTIONS) {
    ...
    if (!existsSync(funcPath)) {
      console.warn(`⚠️  Function ${func.name} directory not found, skipping...`);
      continue;
    }
    ...
    try {
      ...
      if (func.secrets && Object.keys(func.secrets).length > 0) {
        ...
        execSync(
          `supabase secrets set ${secretsCmd} --function ${func.name}`,
...
`;
  ...
  try {
    ...
    const migrationPath = join(process.cwd(), 'supabase', 'migrations', `${timestamp}_setup_cron_jobs.sql`);
