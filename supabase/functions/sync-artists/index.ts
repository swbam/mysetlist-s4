...
async function fetchSpotifyArtist(spotifyId: string, accessToken: string): Promise<SpotifyArtist> {
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch artist from Spotify: ${response.statusText}`);
  }

  return await response.json();
}

async function searchSpotifyArtist(query: string, accessToken: string): Promise<SpotifyArtist[]> {
  ...
  if (!response.ok) {
    throw new Error(`Failed to search artists on Spotify: ${response.statusText}`);
  }
...
serve(async (req) => {
  ...
  try {
    ...
    const artistData = {
      ...
      slug: spotifyArtist.name.toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-|-$/g, ''),
      image_url: spotifyArtist.images[0]?.url || null,
      small_image_url: spotifyArtist.images[2]?.url || spotifyArtist.images[1]?.url || null,
    ...
    return new Response(
      JSON.stringify({ artist, cached: false }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
  } catch (error) {
