...
async function fetchSetlistFmSetlists(params: {
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch setlists from Setlist.fm: ${response.statusText}`);
  }
...
async function fetchSetlistById(setlistId: string): Promise<SetlistFmSetlist> {
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch setlist from Setlist.fm: ${response.statusText}`);
  }
  ...
  try {
    ...
    if (setlistId) {
      ...
      if (setlists.length === 0) {
        return new Response(
          JSON.stringify({ error: 'No setlist found for the specified criteria' }),
      ...
      return new Response(
        JSON.stringify({ error: 'Either setlistId or both artistName and date are required' }),
    ...
    if (songs.length === 0) {
      return new Response(
        JSON.stringify({ error: 'Setlist has no songs' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
