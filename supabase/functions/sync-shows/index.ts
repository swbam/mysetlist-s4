...
async function fetchTicketmasterEvents(params: {
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch events from Ticketmaster: ${response.statusText}`);
  }
  ...
  try {
    ...
    for (const event of events) {
      ...
      if (!dbArtist) {
        ...
        const { data: newArtist } = await supabase
          ...
          .insert({
            ...
            slug: mainArtist.name.toLowerCase()
              .replace(/[^a-z0-9]+/g, '-')
              .replace(/^-|-$/g, ''),
          })
          .select()
      ...
      const showData = {
        ...
        status: new Date(event.dates.start.localDate) > new Date() ? 'upcoming' : 'completed',
      };
          ...
          if (!dbSupportingArtist) {
            const { data: newArtist } = await supabase
              ...
              .insert({
                ...
                slug: supportingArtist.name.toLowerCase()
                  .replace(/[^a-z0-9]+/g, '-')
                  .replace(/^-|-$/g, ''),
              })
              .select()
