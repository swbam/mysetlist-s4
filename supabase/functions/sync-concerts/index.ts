...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const endDate = new Date(Date.now() + options.days_ahead * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
        ...
        for (const event of events) {
          try {
            ...
            if (venue) {
              ...
              const { data: existingVenue, error: venueSelectError } = await supabase
                .from('venues')
                .select('id')
              ...
              if (venueSelectError && venueSelectError.code !== 'PGRST116') {
                console.error(`Error checking venue: ${venueSelectError.message}`);
                continue;
              }
              ...
              if (!existingVenue) {
                ...
                if (venueInsertError) {
                  console.error(`Error creating venue: ${venueInsertError.message}`);
                  continue;
                }
            ...
            const { data: existingShow, error: showSelectError } = await supabase
              .from('shows')
              .select('id')
            ...
            if (!existingShow) {
              ...
              if (showInsertError) {
                console.error(`Error creating show: ${showInsertError.message}`);
                errors++;
              } else {
                synced++;
              ...
              if (showUpdateError) {
                console.error(`Error updating show: ${showUpdateError.message}`);
                errors++;
              }
            }

          } catch (eventError) {
            console.error(`Error processing event for ${artist.name}:`, eventError);
            errors++;
          }
        }
