...
async function simulateArtistsAPI() {
  ...
  try {
    ...
    console.log(
      `  📈 Top artists: ${transformedArtists
        .slice(0, 3)
        .map((a) => a.name)
        .join(", ")}`,
    );
...
async function testTrendingPageFlow() {
  ...
  try {
    ...
    console.log(
      `  ✅ Page stats: ${stats.trendingArtists} artists, ${stats.hotShows} shows, ${stats.searchVolume} views, ${stats.activeUsers} active users`,
    );
...
async function runFullTest() {
  ...
  const [artistsResult, showsResult, liveResult, pageResult] =
    await Promise.all([
      simulateArtistsAPI(),
      simulateShowsAPI(),
      simulateLiveAPI(),
  ...
  console.log(
    `✅ Artists API: ${hasArtistsData ? "WORKING" : "NO DATA"} (${artistsResult.artists.length} items)`,
  );
  console.log(
    `✅ Shows API: ${hasShowsData ? "WORKING" : "NO DATA"} (${showsResult.shows.length} items)`,
  );
  console.log(
    `✅ Live API: ${hasLiveData ? "WORKING" : "NO DATA"} (${liveResult.trending.length} items)`,
  );
  ...
  if (hasArtistsData && hasShowsData && hasLiveData && hasPageData) {
    console.log(
      "🟢 FULLY FUNCTIONAL - All trending APIs are working with real data!",
    );
    ...
    console.log(
      `   - ${pageResult.trendingArtists} artists with trending scores`,
    );
  ...
  console.log(
    "   ✅ Simplified trending API queries to avoid DB schema issues",
  );
  ...
  console.log(
    "   4. If UI still shows no data, check browser dev tools for client-side errors",
  );
