  ...
  try {
    ...
    recentArtists.forEach((artist, i) => {
      const daysSinceSync = artist.lastSyncedAt
        ? Math.floor(
            (Date.now() - new Date(artist.lastSyncedAt).getTime()) /
              (1000 * 60 * 60 * 24),
          )
        : "Never";
      ...
      console.log(
        `     - Followers: ${artist.followers?.toLocaleString() || 0}`,
      );
      console.log(
        `     - Trending Score: ${artist.trendingScore?.toFixed(1) || 0}`,
      );
    ...
    const withFollowers = recentArtists.filter(
      (a) => a.followers && a.followers > 0,
    ).length;
    ...
    console.log(
      `   - With Spotify Data: ${withSpotifyData}/${syncedArtists} (${Math.round((withSpotifyData / syncedArtists) * 100)}%)`,
    );
    console.log(
      `   - With Real Followers: ${withFollowers}/${syncedArtists} (${Math.round((withFollowers / syncedArtists) * 100)}%)`,
    );

    const healthScore =
      ((withSpotifyData + withFollowers) / (syncedArtists * 2)) * 100;
