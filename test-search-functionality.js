...
async function testSearchAPI() {
  ...
  try {
    ...
    if (mainData.results && mainData.results.length > 0) {
      ...
      const nonArtists = mainData.results.filter(result => result.type !== 'artist');
      if (nonArtists.length > 0) {
        console.log(`\n❌ ERROR: Found ${nonArtists.length} non-artist results!`);
        nonArtists.forEach(result => {
    ...
    if (artistsResponse.ok) {
      ...
      console.log(`📊 Artists results count: ${artistsData.artists?.length || 0}`);
