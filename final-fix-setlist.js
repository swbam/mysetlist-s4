...
async function finalFixSetlist() {
  try {
    ...
    if (!arcticSongs || arcticSongs.length === 0) {
      console.log('No Arctic Monkeys songs found. Please run the previous script first.');
      return;
    }
    ...
    finalSetlist?.forEach(item => {
      ...
      console.log(`${String(item.position).padStart(2)}. ${item.songs?.title}${notes} ${votes}`);
