...
async function POST(request) {
    try {
        ...
        try {
            await database_1.db.execute((0, database_1.sql) `SELECT refresh_trending_data();`);
        ...
        try {
            await database_1.db.execute((0, database_1.sql) `
        SELECT log_cron_run(
          'calculate-trending', 
          'success',
...
async function calculateArtistTrending() {
    try {
        const result = await database_1.db.execute((0, database_1.sql) `
...
async function calculateShowTrending() {
    try {
        const result = await database_1.db.execute((0, database_1.sql) `
...
async function getTopTrendingArtists(limit = 10) {
    try {
        const topArtists = await database_1.db.execute((0, database_1.sql) `
      SELECT 
        id,
        name,
      ...
      LIMIT ${limit}
    `);
        return topArtists.rows.map(row => ({
            id: row.id,
            name: row.name,
            score: row.score,
        }));
...
async function HEAD(request) {
    try {
        ...
        await database_1.db.execute((0, database_1.sql) `SELECT 1`);
