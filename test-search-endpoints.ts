...
async function testEndpoint(endpoint: string, query: string): Promise<TestResult> {
...
async function runSearchTests() {
  ...
  for (const endpoint of endpoints) {
    ...
    for (const query of testQueries) {
      ...
      if (result.success) {
        console.log(`✅ ${result.dataCount} results (${result.responseTime}ms)`);
      } else {
  ...
  for (const [endpoint, endpointResults] of Object.entries(groupedResults)) {
    ...
    const avgResponseTime = endpointResults
      .filter(r => r.success)
    ...
    const sampleData = endpointResults.find(r => r.success && r.sampleData)?.sampleData;
    if (sampleData) {
      console.log(`  Sample Data Structure:`, JSON.stringify(sampleData, null, 2).slice(0, 200) + '...');
    }
  ...
  const artistsSearchResults = results.filter(r => r.endpoint === '/api/artists/search' && r.success);
  const generalSearchResults = results.filter(r => r.endpoint === '/api/search' && r.success);
  ...
  if (artistsSearchResults.length > 0 && generalSearchResults.length > 0) {
    ...
    if (artistsSample && generalSample) {
      ...
      console.log(`  /api/artists/search fields: [${artistsFields.join(', ')}]`);
      console.log(`  /api/search fields: [${generalFields.join(', ')}]`);
      
      const commonFields = artistsFields.filter(f => generalFields.includes(f));
  ...
  console.log(`Overall Success Rate: ${totalSuccess}/${totalTests} (${overallSuccessRate}%)`);

  if (overallSuccessRate >= 95) {
    console.log('🎉 EXCELLENT: All search endpoints are working properly!');
  } else if (overallSuccessRate >= 80) {
    console.log('✅ GOOD: Most endpoints are working, but some issues detected');
  ...
  if (results.some(r => r.endpoint === '/api/search/suggestions' && !r.success)) {
    console.log('• Fix the suggestions endpoint to use Ticketmaster API consistently');
  }

  if (avgResponseTime > 1000) {
    console.log('• Consider implementing response caching to improve performance');
  }

  const ticketmasterFailures = results.filter(r => !r.success && r.error?.includes('Ticketmaster'));
  if (ticketmasterFailures.length > 0) {
    console.log('• Check Ticketmaster API configuration and rate limits');
  }

  console.log('• Ensure all artist search components use /api/artists/search for consistency');
