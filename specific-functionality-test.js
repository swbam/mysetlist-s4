...
async function testSearchFunctionality() {
  ...
  // Test 1: Basic search
  try {
    const response = await makeRequest(`${BASE_URL}/api/search?q=test&type=artists`);
    console.log(`✓ Basic search: ${response.statusCode} - ${response.data?.total || 0} results`);
  } catch (error) {
  ...
  try {
    const response = await makeRequest(`${BASE_URL}/api/artists/search?q=Beatles`);
    console.log(`✓ Artist search: ${response.statusCode} - Found ${Array.isArray(response.data) ? response.data.length : 0} artists`);
  } catch (error) {
  ...
  try {
    const response = await makeRequest(`${BASE_URL}/api/search/suggestions?q=rock`);
    console.log(`✓ Search suggestions: ${response.statusCode} - ${Array.isArray(response.data) ? response.data.length : 0} suggestions`);
  } catch (error) {
...
async function testVotingSystem() {
  ...
  try {
    ...
    if (response.statusCode === 400 && response.data?.error?.includes('setlistSongId')) {
      console.log('✓ Voting API properly validates parameters');
    } else {
      console.log(`? Voting API response: ${response.statusCode} - ${JSON.stringify(response.data)}`);
    }
  } catch (error) {
...
async function testRealtimeFeatures() {
  ...
  try {
    const response = await makeRequest(`${BASE_URL}/api/realtime/subscriptions`);
  ...
  for (const api of apis) {
    try {
      ...
      if (response.statusCode === 200) {
        ...
        console.log(`? ${api} integration: ${response.statusCode} - ${response.data?.error || 'Unknown error'}`);
      }
    } catch (error) {
  ...
  for (const endpoint of adminEndpoints) {
    try {
      ...
      if (response.statusCode === 401 || response.statusCode === 403) {
        ...
        console.log(`? ${endpoint}: ${response.statusCode} - May need authentication setup`);
      }
    } catch (error) {
...
async function testPerformanceFeatures() {
  ...
  try {
    ...
    if (response.statusCode === 207) {
      console.log(`✓ Health check: Multi-status response (detailed health info)`);
    } else {
  ...
  for (const page of pages) {
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 400) {
        const hasReactContent = response.raw.includes('__next') || response.raw.includes('React');
        console.log(`✓ ${page.name}: ${response.statusCode} ${hasReactContent ? '(React app)' : ''}`);
      } else {
